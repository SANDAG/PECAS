...
def executePostgreSQLQuery(query, database, port, host, user):
    return subprocess.call([ps.pgpath + "psql", "-c", query, "--dbname=" + database, "--port=" + str(port), "--host=" + host, "--username=" + user])

def executeSQLServerQuery(query, database, user, password):
    return subprocess.call([ps.sqlpath + "sqlcmd", "-S", ps.sd_host, "-U", user, "-P", password, "-d", database, "-Q", query])
...
# Function to calculate the base year ratio between the prices that AA is producing and the prices that SD has been calibrated to
...
def applyAAtoSDPriceCorrection():
        ...
        moveReplace(ps.scendir+"/"+str(year)+"/ExchangeResults.csv", ps.scendir+"/"+str(year)+"/AAExchangeResults.csv")
        shutil.copy(ps.scendir+"/"+str(year)+"/SDPrices.csv", ps.scendir+"/"+str(year)+"/ExchangeResults.csv")

        
def loadAAOutputsForYear(year,command):
        logging.info("Loading data for year:" + str(year))
        if ps.isScenDirOnDatabaseServer :  # the easy way making PostgreSQL do all the work, can do this if the files are on the same machine as the database server
            ...
            sqlstr = "SET search_path="+ps.mapit_schema+"; SELECT load_aa_output('"+ folder_path +"' , "+str(year)+", '"+ ps.scenario +"' ); "
            ...
            cur = conn.cursor()
            cur.execute('SELECT * FROM %s.aa_output_files;' % ps.mapit_schema) # get the list of aa output files we are interested in
            ...
            for row in filesrows: # for each output file
                ...
                cur.copy_from(f, "%s.%s" %(ps.mapit_schema,temp_table_name), sep=',', null='') # use the psycopg2 fast copy command to copy the data into the temporary table
                ...
                sqlcmd= "INSERT INTO %s.%s ( SELECT %d, '%s', tbl.* FROM %s.%s as tbl )" %(ps.mapit_schema, all_table_name, year, ps.scenario, ps.mapit_schema, temp_table_name)
                cur.execute(sqlcmd)
            ...
            sqlstr = "INSERT INTO "+ps.mapit_schema+".all_floorspace_taz(year_run, scenario, taz, commodity, quantity) \
                    SELECT all_floorspacei.year_run, all_floorspacei.scenario, mgra_to_taz.taz, all_floorspacei.aa_commodity AS commodity, \
                    sum(all_floorspacei.quantity) AS quantity \
                    FROM output.all_floorspacei, public.mgra_to_taz \
                    WHERE all_floorspacei.taz = mgra_to_taz.mgra and all_floorspacei.scenario='"+ps.scenario+"' and all_floorspacei.year_run ="+str(year)+\
                    " GROUP BY all_floorspacei.year_run, all_floorspacei.scenario, mgra_to_taz.taz, all_floorspacei.aa_commodity;"
            retcode = subprocess.check_call([command, "-c",sqlstr, "--host="+ps.mapit_host, "--port="+str(ps.mapit_port) , "--dbname="+ps.mapit_database, "--username="+ps.pguser]) 
            logResultsFromExternalProgram("loaded all_floorspace_taz", "Failed to load all_floorspace_taz", (retcode,))
...
def loadSqlAAOutputs(year):
    ...
    logResultsFromExternalProgram("AA results loaded into SQL Server", "Failed in loading AA outputs to SQL Server ", (retcode,))
...
def writeFloorspaceI(year):
    ...
    if hasC:
        ...
        shutil.copy(ps.scendir+"/"+str(year-1)+"/FloorspaceDelta.csv", ps.scendir+"/"+str(year)+"/FloorspaceDelta.csv")
    ...
    if hasC:
        ...
        shutil.copy(ps.scendir+"/"+str(year)+"/FloorspaceCalc.csv", ps.scendir+"/"+str(year)+"/FloorspaceI.csv")
    else:     
...
# Function to calculate and update total imports and exports in ActivityTotalsI in future years (base year has to be set before running.
def updateImportersOrExporters(year, activitiesToExclude, activitiesToUpdate, updateUsingMorU):
	...
	( \n\
		...
		where scenario='"+ps.scenario+"' and year_run="+str(year-1)+" and moru='"+updateUsingMorU+"' and activity not like '"+activitiesToExclude+"' \n\
	group by commodity, moru \n\
     ) im, output.all_makeuse mu \n\
    where mu.scenario='"+ps.scenario+"' and \n\
    mu.year_run="+str(year-1)+" and \n\
    -- next two lines find the exporting activity that used (or importing commodity that made) the commodity we measured in the inner query \n\
...
def loadDistances(skimFileName, skimyear):
    ...
    if ps.sql_system == ps.postgres:
        ...
        raise ValueError
    logResultsFromExternalProgram("Deleted old distance data from database", "Problem deleting old distance data from database", (retcode,))
...
def loadDevelopmentEvents(year):
    ...
    if ps.sql_system == ps.postgres:
        ...
        conn = psycopg2.connect(database=ps.sd_database, host=ps.sd_host, port=ps.sd_port, user=ps.pguser,  password=ps.pgpassword ) 
        cur = conn.cursor()
        cur.execute('TRUNCATE %s.%s' % (ps.sd_schema, "development_events")) # empty the temporary table
        f = open(csv_file, 'r') # open the AA output file
        f.readline() # skip the first line for header
        cur.copy_from(f, "%s.%s" %(ps.sd_schema,"development_events"), sep=',', null='') # use the psycopg2 fast copy command to copy the data into the temporary table
        ...
        f = open(csv_file, 'r') # open the AA output file
        f.readline() # skip the first line for header
        query = "BULK INSERT %s.%s FROM "%(ps.sd_schema, "development_events")+"'"+csv_file+"' WITH (FIELDTERMINATOR = ',',ROWTERMINATOR = '0x0a', FIRSTROW = 2)" 
        executeSQLServerQuery(query, ps.sd_database, ps.sd_user, ps.sd_password)
...
def replayDevelopmentEvents():
    ...
    sqlstr1 = "UPDATE "+ps.sd_schema+'''.parcels SET	space_quantity   = development_events.new_space_quantity, 
...
(      	 event_type = 'CS'
    ...
    if ps.sql_system == ps.postgres:
        retcode1 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+sqlstr1, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        retcode2 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+sqlstr2, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        retcode3 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+sqlstr3, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        retcode4 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+sqlstr4, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        retcode5 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+sqlstr5, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        retcode6 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+sqlstr6, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
    elif ps.sql_system == ps.sqlserver:
        ...
        logging.error("Invalid database system: " + ps.sql_system)
        raise ValueError
    logResultsFromExternalProgram("Replayed development events into parcels table", "Problem replaying development events into parcels table",(retcode1,retcode2,retcode3,retcode4,retcode5,retcode6))
...
def insertDevelopmentEventsIntoHistory(year):
    ...
    if year==ps.baseyear:
        if ps.sql_system == ps.postgres:
            query = "SET search_path="+ps.sd_schema+"; TRUNCATE development_events_history; INSERT INTO development_events_history (select "+str(year)+" as year_run,* from development_events);"
            retcode = executePostgreSQLQuery(query, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        ...
        if ps.sql_system == ps.postgres:
            query = "SET search_path="+ps.sd_schema+"; INSERT INTO development_events_history (select "+str(year)+" as year_run,* from development_events);"
            retcode = executePostgreSQLQuery(query, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
            logging.info("Completed Inserting Development Events to History: "+str(year))
        elif ps.sql_system == ps.sqlserver:
...
def writeFloorspaceSummaryFromParcelFile(sd_output_year):
    ...
    if ps.sql_system == ps.postgres:
        sqlstr = "\\copy (select * from "+ps.sd_schema+".floorspacei_view) to '"+ ps.EXACTPATH + str(sd_output_year) +"/FloorspaceI.csv' csv header"
        print sqlstr
        retcode = executePostgreSQLQuery(sqlstr, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        logResultsFromExternalProgram("Wrote FloorspaceI from Parcel File for use in year "+str(sd_output_year), "Problem writing FloorspaceI from Parcel File for use in year "+str(sd_output_year),(retcode,))
    elif ps.sql_system == ps.sqlserver:
        folder_path = os.path.join(ps.EXACTPATH, str(sd_output_year))
        file_path = os.path.join(folder_path,'FloorspaceI.csv')
        file_path = file_path.replace('/', '\\')
        sqlstr = "select 'TAZ' as taz, 'Commodity' as commodity, 'Quantity' as quantity union all select cast(TAZ as nvarchar), cast(Commodity as nvarchar(50)), cast(Quantity as nvarchar) from "+ps.sd_database+"."+ps.sd_schema+".floorspacei_view"
...
def resetParcelDatabase():
    ...
    query1 = "DELETE from local_effect_distances WHERE pecas_parcel_num >(SELECT MAX(pecas_parcel_num) FROM parcels_backup);"
    ...
    query4 = "DELETE from parcel_zoning_xref WHERE pecas_parcel_num >(SELECT MAX(pecas_parcel_num) FROM parcels_backup);"
    ...
    query6 = "UPDATE parcels  SET year_built= parcels_backup.year_built, space_type_id=parcels_backup.space_type_id, space_quantity=parcels_backup.space_quantity, land_area=parcels_backup.land_area, available_services_code=parcels_backup.available_services_code, is_derelict=parcels_backup.is_derelict, is_brownfield=parcels_backup.is_brownfield FROM parcels_backup  WHERE parcels.pecas_parcel_num = parcels_backup.pecas_parcel_num;"
    query7 = "TRUNCATE TABLE "+ps.sd_schema+".parcels_snapshot";
    
    retcode5A = 0
    if ps.sql_system == ps.postgres:
        retcode1 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+query1, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        retcode2 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+query2, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        retcode3 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+query3, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        retcode4 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+query4, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        retcode5 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+query5, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        retcode5A = executePostgreSQLQuery(query5A, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        retcode6 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+query6, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        retcode7 = executePostgreSQLQuery("SET search_path="+ps.sd_schema+"; "+query7, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
        logResultsFromExternalProgram("Reset parcels", "Problem resetting parcels", (retcode1,retcode2,retcode3,retcode4,retcode5,retcode5A,retcode6,retcode7))
    elif ps.sql_system == ps.sqlserver:
        ...
        logResultsFromExternalProgram("Reset parcels", "Problem resetting parcels", (retcode1,retcode2,retcode3,retcode4,retcode5,retcode6,retcode7))
    else:
        logging.error("Invalid database system: " + ps.sql_system)
        raise ValueError
...
def runTravelModelForYear(year):
	...
	fout = open(ps.scendir+"/"+str(year)+"/"+str(year)+"skim.csv", "a") # open in "append" mode, so we can append the external skims 
...
def prepareTravelModelInputs(year, scenario):
    ...
    retcode = executePostgreSQLQuery("select input.generate_tm_inputs("+str(year)+",'"+scenario+"');", ps.mapit_database, ps.mapit_port, ps.mapit_host, ps.pguser)
...
def prepareTravelModelInputsSANDAG(year): 
    ...
    sql = "EXEC travel.tm_generate_4step_inputs '%s', %s, '%s', %s" % (os.path.abspath(ps.local_unc_path)+os.sep, ps.aa_scenario_id, ps.sd_schema, year)
    ...
    retcode = subprocess.call(["bcp",sqlstr1,"queryout",filepath1,"-S",ps.sd_host,"-d","pecas_sr13","-U",ps.sd_user,"-P",ps.sd_password,"-c","-t,"])
    ...
    retcode = subprocess.call(["bcp",sqlstr2,"queryout",filepath2,"-S",ps.sd_host,"-d","pecas_sr13","-U",ps.sd_user,"-P",ps.sd_password,"-c","-t,"])
    ...
    retcode = subprocess.call(["bcp",sqlstr3,"queryout",filepath3,"-S",ps.sd_host,"-d","pecas_sr13","-U",ps.sd_user,"-P",ps.sd_password,"-c","-t,"])
    ...
    retcode = subprocess.call(["bcp",sqlstr4,"queryout",filepath4,"-S",ps.sd_host,"-d","pecas_sr13","-U",ps.sd_user,"-P",ps.sd_password,"-c","-t,"])
    logResultsFromExternalProgram(None, "Problem preparing tm_hhdata travel model inputs in database", (retcode,))  

    # Get final values to use in travel model (integers that sum to proper total). Order is important because PASEF relies on HH totals
...
def prepareTravelModelOutputsSANDAG(year):
    ...
    sqlstr1 = "EXEC aa.load_skims '%s', %s, %s" % (os.path.abspath(ps.tm_output_dir)+os.sep, ps.aa_scenario_id, str(year-1))
