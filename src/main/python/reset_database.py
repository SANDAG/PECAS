...
def resetdatabase():
    ...
    classpath = pr.buildClassPath(ps.codepath + "/" + ps.pecasjar, 
                                  ps.codepath + "/" + ps.commonbasejar,
                                  ps.codepath + "/or124.jar", 
                                  ps.codepath + "/mtj.jar", 
                                  ps.codepath + "/log4j-1.2.9.jar",
                                  ps.codepath + "/postgresql-8.4-701.jdbc4.jar", 
                                  ps.codepath + "/sqljdbc4.jar", 
                                  ps.codepath + "/" + ps.simpleormjar,
                                  ps.codepath + "/")
    ...
    """
    remove AA outputs from SQL Server for year > ps.startyear. Need to change along with other scripts 
    to not reset when replay.
    ...
    pr.logResultsFromExternalProgram(None, "Problem removing results for "+ps.scenario+" 
    from SQL Server database",(retcode,))
    """
    # remove outputs from mapit
    logging.info("Removing results from scenario " + ps.scenario + " from MapIt database " \
                 + ps.mapit_database + " on host " + ps.mapit_host)
    ...
    #    "--port="+str(ps.mapit_port) , "--dbname="+ps.mapit_database, "--username="+ps.pguser])
    # pr.logResultsFromExternalProgram(None, 
    #    "Problem removing results for "+ps.scenario+" from MapIt database",(retcode,))

    # remove records from all_floorspace_taz
    # sqlstr= "SET search_path=%s; DELETE FROM all_floorspace_taz WHERE scenario='%s'; " 
    #    % (ps.mapit_schema, ps.scenario)
    # retcode = subprocess.check_call([command, "-c", sqlstr, "--host="+ps.mapit_host, 
    #    "--port="+str(ps.mapit_port) , "--dbname="+ps.mapit_database, "--username="+ps.pguser])
    # pr.logResultsFromExternalProgram(None, 
    #    "Problem removing all_floorspace_taz for "+ps.scenario+" from MapIt database",(retcode,))
