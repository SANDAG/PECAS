...
def executePostgreSQLQuery(query, database, port, host, user):
    return subprocess.call([ps.pgpath + "psql", "-c", query, "--dbname=" + database, "--port=" + str(port), 
                            "--host=" + host, "--username=" + user])

def executeSQLServerQuery(query, database, user, password):
    return subprocess.call([ps.sqlpath + "sqlcmd", "-S", ps.sd_host, "-U", user, "-P", password, "-d", database, 
                            "-Q", query])
...
def loadAAOutputsForYear(year, command):
    ...
    if ps.isScenDirOnDatabaseServer :  
        ...
        # Abdel wrote all the compexity into the load_aa_output routine on the database, 
        # so this is simple here in python
        sqlstr = "SET search_path=" + ps.mapit_schema +  "; SELECT load_aa_output('" \
                + folder_path + "' , " + str(year) + ", '" + ps.scenario + "' ); "
        retcode = subprocess.check_call([command, "-c", sqlstr, 
                                         "--host=" + ps.mapit_host, 
                                         "--port=" + str(ps.mapit_port) , 
                                         "--dbname=" + ps.mapit_database, 
                                         "--username=" + ps.pguser])
        ...
        for row in filesrows:  # for each output file
            ...
            # now insert the records from the temporary table 
            # into the full table which contains data for each year/scenario
        ...
        sqlstr = "INSERT INTO " + ps.mapit_schema + ".all_floorspace_taz(year_run, scenario, taz, commodity, quantity) \
                ...
                all_floorspacei.aa_commodity AS commodity, sum(all_floorspacei.quantity) AS quantity \
                FROM output.all_floorspacei, public.mgra_to_taz \
                WHERE all_floorspacei.taz = mgra_to_taz.mgra \
                and all_floorspacei.scenario='" + ps.scenario + \
                "' and all_floorspacei.year_run =" + str(year) + \
    ...
    set total_amount=(case when '" + updateUsingMorU + "'='U' then -im.amount else im.amount end) from \n\
        ...
        ( \n\
               -- inner query is amount made (or used in the case of updating imports) internally last year \n\
                ...
                where scenario='" + ps.scenario + "' and year_run=" + str(year - 1) + \
                " and moru='" + updateUsingMorU + "' and activity not like '" + activitiesToExclude + "' \n\
    ...
    for row in exresultsReader:
        ...
        if (counter >= 250):
            ...
            query = "insert into " + ps.sd_schema + \
                    ".exchange_results  (yr, commodity, luz, price, internal_bought) values "
            first = True
            counter = 0
    if not first:
        query += ";"
...
def loadDevelopmentEvents(year):
    ...
    if ps.sql_system == ps.postgres:
        ...
        conn = psycopg2.connect(database = ps.sd_database, 
                                host = ps.sd_host, 
                                port = ps.sd_port, 
                                user = ps.pguser, 
                                password = ps.pgpassword)
...
def replayDevelopmentEvents():
    ...
    sqlstr4 = "INSERT INTO " + ps.sd_schema + '''.parcel_fee_xref
                 SELECT development_events.new_pecas_parcel_num, parcel_fee_xref.fee_schedule_id, \
                 parcel_fee_xref.year_effective
    ...
    sqlstr5 = '''INSERT INTO ''' + ps.sd_schema + '''.parcel_zoning_xref
                 SELECT development_events.new_pecas_parcel_num, parcel_zoning_xref.zoning_rules_code, \
    ...
    if year == ps.baseyear:
        if ps.sql_system == ps.postgres:
            query = "SET search_path=" + ps.sd_schema + "; TRUNCATE development_events_history; \
                ...
                (select " + str(year) + " as year_run,* from development_events);"
            retcode = executePostgreSQLQuery(query, ps.sd_database, ps.sd_port, ps.sd_host, ps.pguser)
...
def resetParcelDatabase():
    ...
    query4 = "DELETE from parcel_zoning_xref WHERE pecas_parcel_num >(SELECT MAX(pecas_parcel_num) \
        FROM parcels_backup);"
    ...
    query6 = "UPDATE parcels  SET year_built= parcels_backup.year_built, space_type_id=parcels_backup.space_type_id, \
        ...
        is_brownfield=parcels_backup.is_brownfield FROM parcels_backup  \
        WHERE parcels.pecas_parcel_num = parcels_backup.pecas_parcel_num;"
...
def loadAATotals():
    ...
    folder_path = folder_path.replace('\\', '/')
    conn = psycopg2.connect(database=ps.mapit_database, 
                            host=ps.mapit_host, 
                            port=ps.mapit_port, 
                            user=ps.pguser,  
                            password=ps.pgpassword )
    ...
    f.readline() # skip the first line for header
    # use the psycopg2 fast copy command to copy the data into the temporary table
    cur.copy_from(f, "%s.%s" %('input', "activity_totals_temp"), sep=',', null='') 
    cur.execute("DELETE FROM input.activity_totals WHERE scenario= '%s';" %(ps.scenario))
    cur.execute("INSERT INTO input.activity_totals (select year_run, '%s', activity, \
        total_amount from input.activity_totals_temp);" %(ps.scenario))
...
def runTravelModelForYear(year):
    ...
    skimyear = year
    # specifies where to find the network files for the travel model, 
    #this demo always uses the 2007 network but in general the network closest
    # in time to the current year should be used.
    networkyear = 2007  
    # if the travel model does not use different file system directories to represent different scenarios 
    #there would be additional code to select the appropriate scenario as well as the appropriate year
    ...
    # our travel model is a simple frank-wolfe assignment 
    # (trip generation is actually done within the AA code for software performance reasons)
    retcode = subprocess.call([javaRunCommand, "-Xmx3000M", 
                               "-Dlog4j.configuration=log4j.xml", "-DNETWORKYEAR=" + str(networkyear),
...
def prepareTravelModelInputsSANDAG(year):
    ...
    sql = "EXEC travel.tm_generate_4step_inputs '%s', %s, '%s', %s" % (os.path.abspath(ps.local_unc_path) + os.sep, 
                                                                       ps.aa_scenario_id, ps.sd_schema, year)
    ...
    retcode = subprocess.call(["bcp", sqlstr1, "queryout", filepath1, "-S", ps.sd_host, "-d", 
                               "pecas_sr13", "-U", ps.sd_user, "-P", ps.sd_password, "-c", "-t,"])
    ...
    retcode = subprocess.call(["bcp", sqlstr2, "queryout", filepath2, "-S", ps.sd_host, "-d", 
                               "pecas_sr13", "-U", ps.sd_user, "-P", ps.sd_password, "-c", "-t,"])
    ...
    retcode = subprocess.call(["bcp", sqlstr3, "queryout", filepath3, "-S", ps.sd_host, "-d", 
                               "pecas_sr13", "-U", ps.sd_user, "-P", ps.sd_password, "-c", "-t,"])
    ...
    retcode = subprocess.call(["bcp", sqlstr4, "queryout", filepath4, "-S", ps.sd_host, "-d", 
                               "pecas_sr13", "-U", ps.sd_user, "-P", ps.sd_password, "-c", "-t,"])
    logResultsFromExternalProgram(None, "Problem preparing tm_hhdata travel model inputs in database", (retcode,))
...
def prepareTravelModelOutputsSANDAG(year):
    ...
    sqlstr1 = "EXEC aa.load_skims '%s', %s, %s" % (os.path.abspath(ps.tm_output_dir) \
                                                   + os.sep, ps.aa_scenario_id, str(year - 1))
    ...
    retcode = subprocess.call(["bcp", sqlstr1, "queryout", filepath1, "-S", 
                               ps.sd_host, "-d", "pecas_sr13", "-U", ps.sd_user, "-P", ps.sd_password, "-c", "-t,"])
    logResultsFromExternalProgram(None, "Problem preparing loading skims into database", (retcode,))
