...
def runAA(year, skimfile, skimyear):
    ...
    logging.info("Running AA with -Dscendir=" + ps.scendir + " -DYEAR=" + str(year) + " -DSKIMYEAR=" \
                 + str(skimyear) + " -DSKIMFILE=" + str(skimfile) + " -DPREVYEAR=" + str(prevyear) \
                 + " -DCONSTRAINED=" + str(constrained))
    logging.info("classpath is " + classpath)
    retcode = subprocess.call([ps.javaRunCommand, "-Xmx2500M", "-Dlog4j.configuration=log4j.xml", 
                               "-DSCENDIR=" + ps.scendir, "-DYEAR=" + str(year), "-DSKIMYEAR=" + str(skimyear), 
                               "-DSKIMFILE=" + str(skimfile),"-DPREVYEAR=" + str(prevyear), 
                               "-DCONSTRAINED=" + str(constrained), "-cp", 
                               pr.buildClassPath(ps.scendir + "AllYears/Inputs", classpath),
                               "com.hbaspecto.pecas.aa.control.AAControl"])
    pr.logResultsFromExternalProgram("AA model finished", "AA model did not run successfully in year " + str(year), 
                                     (retcode,))
    ...
    if year<> ps.baseyear:
        ...
        retcode = pr.executePostgreSQLQuery("\\copy input.tm_input to '"+ps.EXACTPATH+"/"+str(year)+"/tm_inputs.csv' \
            CSV HEADER", ps.mapit_database, ps.mapit_port, ps.mapit_host, ps.pguser)
        pr.logResultsFromExternalProgram("Wrote out travel model inputs", "Problem writing travel model land-use \
            inputs for "+str(year), (retcode,))'''
