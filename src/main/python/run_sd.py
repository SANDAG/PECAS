...
def runSD(year, skimfile, skimyear):
    ...
    logging.info("running SD model for " + str(year) + " with -DSCENDIR=" + ps.scendir \
                 + " -Ddatabase=" + ps.sd_database + " -Dport=" + str(ps.sd_port) + " -Ddatabaseuser=" + ps.sd_user \
                 + "-Dhost=" + ps.sd_host + " -DSD_SCHEMA=" + ps.sd_schema)
    retcode = subprocess.call([ps.javaRunCommand, 
                               "-Xmx5000M", 
                               "-DSCENDIR=" + ps.scendir, 
                               "-Ddatabase=" + ps.sd_database, 
                               "-Dport=" + str(ps.sd_port),
                               "-Ddatabaseuser=" + ps.sd_user,
                               "-Dhost=" + ps.sd_host,
                               "-DSDPASSWORD=" + ps.sd_password,
                               "-DSD_SCHEMA=" + ps.sd_schema,
                               "-DEXACTPATH=" + ps.EXACTPATH,
                               "-DDEVEVENTPATH=" + ps.DEVEVENTPATH,
                               "-DLOGFILEPATH=" + ps.LOGFILEPATH,
                               "-cp", 
    ...
    pr.logResultsFromExternalProgram("SD model Finished", 
                                     "SD model did not run successfully in year " + str(year), (retcode,))
