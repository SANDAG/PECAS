...
public class LogitModel extends DiscreteChoiceModel implements
	...
	public void allocateQuantity(double amount)
		...
		for (int i = 0; i < probs.length; i++) {
			...
			((AggregateAlternative) a).setAggregateQuantity(amount * probs[i], amount
					* probs[i] * (1 - probs[i]) * dispersionParameter);
		}
	}
	...
	public double getUtilityNoSizeEffect() throws ChoiceModelOverflowException {
		...
		while (i < alternatives.size()) {
			sum += Math.exp(getDispersionParameter()
					* alternatives.get(i).getUtilityNoSizeEffect());
			i++;
		}
		if (sum == 0) {
	...
	public double getUtility(double higherLevelDispersionParameter)
		...
		if (Double.isNaN(bob) || Double.isInfinite(bob)) {
			logger
					.error("Choice model overflow error in composite utility calculation, denominator term is "
							+ sum);
			...
			while (j < alternatives.size()) {
				...
				logger.error("Alt " + a + " has utility "
						+ a.getUtility(getDispersionParameter()));
				j++;
			}
			throw new ChoiceModelOverflowException(
					"Overflow getting composite utility");
		}
		if (caching) {
			utilityCached = true;
		}
	 ...
	 * this parameter was set using a coefficient object. Otherwise returns null.
	 * 
		...
		synchronized (alternatives) {
			...
			while (it.hasNext()) {
				...
				if (Double.isInfinite(weights[i])) {
					throw new ChoiceModelOverflowException(weights[i]
							+ " weight for alternative " + a + ", Dispersion: "
							+ getDispersionParameter() + ", Utility: " + utility);
				}
				sum += weights[i];
				i++;
			}
			if (sum == 0) {
				throw new NoAlternativeAvailable("Denominator in " + this
						+ " model is 0");
			}
			if (Double.isInfinite(sum)) {
				throw new ChoiceModelOverflowException(
						"Denominator in logit model is infinite");
			}
	 ...
	 * @throws ChoiceModelOverflowException
	 */
	public double[][] choiceProbabilityDerivatives()
			throws ChoiceModelOverflowException {
		final double[] weights = new double[alternatives.size()];
		...
		synchronized (alternatives) {
			...
			for (int a = 0; a < derivatives.length; a++) {
				...
				for (int p = 0; p < derivatives.length; p++) {
					derivatives[a][p] -= getDispersionParameter() * weights[a]
							* weights[p];
				}
			}
			return derivatives;
	 ...
	 */
	@Override
	public Alternative monteCarloChoice(double randomNumber)
	...
	public String toString() {
		...
		if (alternatives.size() > 5) {
			altsString.append("LogitModel with " + alternatives.size()
					+ "alternatives {");
		}
		else {
	...
	public static int arrayCoefficientSimplifiedChoice(
			double[][] theCoefficients, double[] theAttributes) {
	...
	public double[] choiceProbabilityDerivativesWRTDispersion()
			throws ChoiceModelOverflowException {
		final double[] weights = new double[alternatives.size()];
	...
	public Vector getUtilityDerivativesWRTParameters(List<Coefficient> cs)
		...
		while (it.hasNext()) {
			final ParameterSearchAlternative da = (ParameterSearchAlternative) it
					.next();
		...
		if (dispIndex >= 0) {
			final double term1 = sumWeightsTimesUtilities
					/ (getDispersionParameter() * sumWeights);
	...
	public Matrix getExpectedTargetDerivativesWRTParameters(
		...
		for (final Alternative a : getAlternatives()) {
			...
			if (prob > 0.0) {
				...
				final Matrix dmdc = da
						.getExpectedTargetDerivativesWRTParameters(ts, cs);
