 ...
 * in particular
 * <p>
 * y = y0 + delta*(exp(lambda(x-x0))-1)/(1+exp(lambda(x-x0))) + slope*(x-x0)
 * <p>
 * A linear function can be set up by setting delta to zero and slope to
 * nonzero. A logistic function can be set up by setting detla to nonzero and
 * slope to zero.
 * 
 * @author J. Abraham
 */
public class LogisticPlusLinearFunction implements SingleParameterFunction {
	...
	public LogisticPlusLinearFunction(double y0, double x0, double lambda,
	...
	public double evaluate(double x) {
		final double temp = Math.exp(lambda * (x - x0));
