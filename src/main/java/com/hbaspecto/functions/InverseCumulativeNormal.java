...
public class InverseCumulativeNormal {
	...
	private static final double ERF_A[] = { 3.16112374387056560E00,
			1.13864154151050156E02, 3.77485237685302021E02, 3.20937758913846947E03,
			1.85777706184603153E-1 };

	private static final double ERF_B[] = { 2.36012909523441209E01,
			2.44024637934444173E02, 1.28261652607737228E03, 2.84423683343917062E03 };

	// C------------------------------------------------------------------
	// C Coefficients for approximation to erfc in second interval
	// C------------------------------------------------------------------
	private static final double ERF_C[] = { 5.64188496988670089E-1,
			8.88314979438837594E0, 6.61191906371416295E01, 2.98635138197400131E02,
			8.81952221241769090E02, 1.71204761263407058E03, 2.05107837782607147E03,
			1.23033935479799725E03, 2.15311535474403846E-8 };

	private static final double ERF_D[] = { 1.57449261107098347E01,
			1.17693950891312499E02, 5.37181101862009858E02, 1.62138957456669019E03,
			3.29079923573345963E03, 4.36261909014324716E03, 3.43936767414372164E03,
	...
	private static double calerf(double X, int jint) {
		 ...
		 * *******************************************
		 * C------------------------------------------------------------------ C C
		 * THIS PACKET COMPUTES THE ERROR AND COMPLEMENTARY ERROR FUNCTIONS C FOR
		 * REAL ARGUMENTS ARG. IT CONTAINS TWO FUNCTION TYPE C SUBPROGRAMS, ERF AND
		 * ERFC (OR DERF AND DERFC), AND ONE C SUBROUTINE TYPE SUBPROGRAM, CALERF.
		 * THE CALLING STATEMENTS C FOR THE PRIMARY ENTRIES ARE C C Y=ERF(X) (OR
		 * Y=DERF(X) ) C AND C Y=ERFC(X) (OR Y=DERFC(X) ). C C THE ROUTINE CALERF IS
		 * INTENDED FOR INTERNAL PACKET USE ONLY, C ALL COMPUTATIONS WITHIN THE
		 * PACKET BEING CONCENTRATED IN THIS C ROUTINE. THE FUNCTION SUBPROGRAMS
		 * INVOKE CALERF WITH THE C STATEMENT C CALL CALERF(ARG,RESULT,JINT) C WHERE
		 * THE PARAMETER USAGE IS AS FOLLOWS C C FUNCTION PARAMETERS FOR CALERF C
		 * CALL ARG RESULT JINT C ERF(ARG) ANY REAL ARGUMENT ERF(ARG) 0 C ERFC(ARG)
		 * ABS(ARG) .LT. XMAX ERFC(ARG) 1 C C THE MAIN COMPUTATION EVALUATES NEAR
		 * MINIMAX APPROXIMATIONS C FROM
		 * "RATIONAL CHEBYSHEV APPROXIMATIONS FOR THE ERROR FUNCTION" C BY W. J.
		 * CODY, MATH. COMP., 1969, PP. 631-638. THIS C TRANSPORTABLE PROGRAM USES
		 * RATIONAL FUNCTIONS THAT THEORETICALLY C APPROXIMATE ERF(X) AND ERFC(X) TO
		 * AT LEAST 18 SIGNIFICANT C DECIMAL DIGITS. THE ACCURACY ACHIEVED DEPENDS
		 * ON THE ARITHMETIC C SYSTEM, THE COMPILER, THE INTRINSIC FUNCTIONS, AND
		 * PROPER C SELECTION OF THE MACHINE-DEPENDENT CONSTANTS. C C AUTHOR: W. J.
		 * CODY C MATHEMATICS AND COMPUTER SCIENCE DIVISION C ARGONNE NATIONAL
		 * LABORATORY C ARGONNE, IL 60439 C C LATEST MODIFICATION: JANUARY 8, 1985 C
		 * C------------------------------------------------------------------
		 */
		double result = 0;
		double Y = Math.abs(X);
		...
		if (Y <= THRESHOLD) {
			...
			if (jint != 2) {
				...
				final double del = (Y - Y_SQ) * (Y + Y_SQ);
		...
		if (jint == 0) {
			...
			if (X < 0) {
				if (X < X_NEG) {
					...
					final double del = (X - Y_SQ) * (X + Y_SQ);
	...
	public static double refine(double x, double d) {
		if (d > 0 && d < 1) {
			final double e = 0.5D * erfc(-x / Math.sqrt(2.0D)) - d;
			final double u = e * Math.sqrt(2.0D * Math.PI) * Math.exp(x * x / 2.0D);
			x = x - u / (1.0D + x * u / 2.0D);
		}
		return x;
	}
