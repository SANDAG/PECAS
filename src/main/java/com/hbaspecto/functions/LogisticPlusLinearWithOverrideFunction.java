...
public class LogisticPlusLinearWithOverrideFunction
        implements SingleParameterFunction
{
    ...
    public LogisticPlusLinearWithOverrideFunction(LogisticPlusLinearFunction myFunction,
            double overridePoint, double overrideQuantity, double exponent)
    {
    ...
    public void writeOverride(Writer stream, int zone, String commodityName, double price)
            throws IOException
    {
        double overrideAmount = 0;
        if (price > overridePoint)
        {
            overrideAmount = overrideQuantity * Math.pow(price - overridePoint, exponent);
        }
        stream.write(zone + "," + commodityName + "," + price + "," + evaluate(price) + ","
                + overrideAmount + "\n");
