...
public class LogitTechnologyChoice extends LogitModel {
	...
	public void setUtilities(double[] buyingUtilitiesParam,
			double[] sellingUtilitiesParam, int zoneIndex) {
	 ...
	 *          the selling utilities (composite utility of exchange choice for
	 *          selling for each commodity)
	 * @return the composite utility over all of the options
	 * @throws ChoiceModelOverflowException
	 */
	public double overallUtility(double[] buyingUtilitiesParam,
			double[] sellingUtilitiesParam, int zoneParam)
			throws ChoiceModelOverflowException {
	...
	public double[] calcBuyingAmounts(double[] buyingUtilitiesParam,
			double[] sellingUtilitiesParam, int zoneIndex)
		...
		final double[] amounts = new double[buyingUtilitiesParam.length];
		try {
		...
		for (int option = 0; option < getAlternatives().size(); option++) {
			final TechnologyOption techOption = (TechnologyOption) alternativeAt(option);
			final double[] optionAmounts = techOption
					.getBuyingNegativeAmountsInOrder();
	...
	public double[] calcSellingAmounts(double[] buyingUtilitiesParam,
			double[] sellingUtilitiesParam, int zoneIndex)
			throws ChoiceModelOverflowException, NoAlternativeAvailable {
		setUtilities(buyingUtilitiesParam, sellingUtilitiesParam, zoneIndex);
		final double[] amounts = new double[sellingUtilitiesParam.length];
		...
		for (int option = 0; option < getAlternatives().size(); option++) {
			final TechnologyOption techOption = (TechnologyOption) alternativeAt(option);
			final double[] optionAmounts = techOption
					.getSellingPositiveAmountsInOrder();
			for (int c = 0; c < amounts.length; c++) {
				amounts[c] += probabilities[option] * optionAmounts[c];
			}
		}
	...
	public void addAlternative(Alternative a) {
		if (!(a instanceof TechnologyOption)) {
			final String msg = "Tried to add an alternative of type "
					+ a.getClass().getName()
					+ " to a LogitTechnologyChoice -- can only add TechnologyOptions";
			logger.error(msg);
	...
	public void addTechnologyOption(TechnologyOption newOption) {
		addAlternative(newOption);
	}

	public void checkAndLogRatios(String name, TableDataSet productionTable,
			TableDataSet consumptionTable, int row) {
		// check through all options.
		double highestOption = 0;
		for (int o = 0; o < getAlternatives().size(); o++) {
			final TechnologyOption option = (TechnologyOption) getAlternatives().get(
					o);
			highestOption = Math.max(option
					.calculateVarianceTimes6DividedByPiSquared(productionTable,
							consumptionTable, row), highestOption);
		}
		final double max = 1 / Math.sqrt(highestOption);
		if (getDispersionParameter() > max) {
			logger.warn("Technology dispersion parameter too high for " + name
					+ ", maximum is " + max);
		}
		logger.info("Dispersion parameter ratio for " + this + " is "
				+ getDispersionParameter() / max);
	 ...
	 * @return First elements in matrix are the partials of buying (buying amounts
	 *         are negative), next are partials of selling
	 */
	public double[] amountsDerivatives(double[] buyingCommodityUtilities,
		...
		final int size = buyingCommodityUtilities.length
				+ sellingCommodityUtilities.length;
		final double[][] diffFromAmountBaseline = getDifferenceAmountsFromBaseline(
				buyingCommodityUtilities, sellingCommodityUtilities, zoneIndex);
		final double[] diagonal = new double[size];
		for (int i = 0; i < size; i++) {
			if (getAllSames()[i]) {
				...
				for (int p = 0; p < choiceProbabilitiesTemporaryStorage.length; p++) {
					value += choiceProbabilitiesTemporaryStorage[p]
							* getTechnicalCoefficientsMatrix()[i][p]
	...
	public Matrix amountsDerivativesMatrix(double[] buyingCommodityUtilities,
		...
		final FlexCompColMatrix matrix = new FlexCompColMatrix(size, size);
		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {
				if (!getAllSames()[i] && !getAllSames()[j]) {
					double value = 0;
					for (int p = 0; p < choiceProbabilitiesTemporaryStorage.length; p++) {
						value += choiceProbabilitiesTemporaryStorage[p]
								* getTechnicalCoefficientsMatrix()[i][p]
	...
	private double[][] getDifferenceAmountsFromBaseline(
			double[] buyingCommodityUtilities, double[] sellingCommodityUtilities,
			int zoneIndex) {
		setUtilities(buyingCommodityUtilities, sellingCommodityUtilities, zoneIndex);
		final int size = buyingCommodityUtilities.length
				+ sellingCommodityUtilities.length;
		try {
			...
			final String error = "Overflow in choice derivative calculation " + e;
			// TODO should it error gracefully in case the step size is too
			// large?
			logger.error(error);
			...
			for (int i = 0; i < choiceProbabilitiesTemporaryStorage.length; i++) {
				choiceProbabilitiesTemporaryStorage[i] = 0;
			}
		}
		final double[] amountBaseline = new double[size];
		final double[][] diffFromAmountBaseline = new double[size][choiceProbabilitiesTemporaryStorage.length];
		for (int i = 0; i < size; i++) {
			for (int p = 0; p < choiceProbabilitiesTemporaryStorage.length; p++) {
			...
			for (int p = 0; p < choiceProbabilitiesTemporaryStorage.length; p++) {
				if (getAllSames()[i]) {
					diffFromAmountBaseline[i][p] = 0;
				}
				else {
					diffFromAmountBaseline[i][p] = getUtilityDerivativesWrtCommodityUtilities()[p][i]
							- amountBaseline[i];
				}
			}
		}
		return diffFromAmountBaseline;
	}

	private double[][] getAmountsDerivativesWrtProbabilities() {
		final double[][] matrix = new double[myCommodityOrder.size() * 2][getAlternatives()
				.size()];
		for (int o = 0; o < getAlternatives().size(); o++) {
			final TechnologyOption techOption = (TechnologyOption) getAlternatives()
					.get(o);
			final double[] buyingAmounts = techOption
					.getBuyingNegativeAmountsInOrder();
			final double[] sellingAmounts = techOption
					.getSellingPositiveAmountsInOrder();
			for (int i = 0; i < myCommodityOrder.size(); i++) {
				if (buyingAmounts[i] != 0) {
					matrix[i][o] = buyingAmounts[i];
				}
				if (sellingAmounts[i] != 0) {
					matrix[i + myCommodityOrder.size()][o] = sellingAmounts[i];
				}
			}
		}
		return matrix;
	...
	private double[][] getUtilityDerivativesWrtCommodityUtilities() {
		if (buyingUtilities == null || sellingUtilities == null) {
			logger
					.fatal("Need to set buying utilities and selling utilities at least once before initializing matrix");
		}
		if (utilityDerivativesWrtCommodityUtilities == null) {
			final double[][] derivatives = new double[getAlternatives().size()][buyingUtilities.length
					+ sellingUtilities.length];
			for (int o = 0; o < getAlternatives().size(); o++) {
				final TechnologyOption techOption = (TechnologyOption) getAlternatives()
						.get(o);
				final double[] scaledBuying = techOption
						.getBuyingNegativeScaledAmountsInOrder();
				final double[] scaledSelling = techOption
						.getSellingPositiveScaledAmountsInOrder();
				// for buying need to reverse the sign as the utility derivative
				// is positive
				for (int i = 0; i < buyingUtilities.length; i++) {
					if (scaledBuying[i] != 0) {
						derivatives[o][i] = -scaledBuying[i];
					}
				}
				for (int i = 0; i < sellingUtilities.length; i++) {
					if (scaledSelling[i] != 0) {
						derivatives[o][buyingUtilities.length + i] = scaledSelling[i];
					}
				}
			}
	...
	/**
	 * Calculate the derivative of the logsum term wrt changes in buying utilities
	 * or selling utilities
	 * 
	 * @param buyingCommodityUtilities
	 *          utility of buying different commodities
	 * @param sellingCommodityUtilities
	...
	public double[] compositeUtilityDerivatives(
		...
		try {
			...
			final String error = "Overflow calculating compositeUtilityDerivatives "
					+ e;
			if (logger.isDebugEnabled()) {
				logger.debug(e);
			}
			// TODO check to see whether returning zero is ok.
			final double[] utilityDerivatives = new double[buyingCommodityUtilities.length
					+ sellingCommodityUtilities.length];
		...
		for (int o = 0; o < getAlternatives().size(); o++) {
			...
			for (int i = 0; i < sellingCommodityUtilities.length; i++) {
				utilityDerivatives[i + buyingCommodityUtilities.length] += choiceProbabilities[o]
						* scaledAmounts[i];
			}
		}
		return utilityDerivatives;
	...
	public void writeOutOptionUtilities(int activityId, int zoneNumber,
			PrintWriter out) throws IOException, ChoiceModelOverflowException {
		double[] probabilities;
		try {
			probabilities = getChoiceProbabilities();
		}
		...
		int i = 0;
		for (final Object a : getAlternatives()) {
			final TechnologyOption t = (TechnologyOption) a;
	...
	public void setFloorspaceProportionsByLUZ(double[][] floorspaceInventoryByLUZ) {
		floorspaceProportionsByLUZ = floorspaceInventoryByLUZ;
	}
