...
public class LogitTechnologyChoice
        extends LogitModel
{
    ...
    public void setUtilities(double[] buyingUtilitiesParam, double[] sellingUtilitiesParam,
            int zoneIndex)
    {
     ...
     */
    public double overallUtility(double[] buyingUtilitiesParam, double[] sellingUtilitiesParam,
            int zoneParam) throws ChoiceModelOverflowException
    {
    ...
    public double[] calcBuyingAmounts(double[] buyingUtilitiesParam,
            double[] sellingUtilitiesParam, int zoneIndex) throws ChoiceModelOverflowException,
            NoAlternativeAvailable
    {
        ...
        for (int option = 0; option < getAlternatives().size(); option++)
        {
            ...
            final double[] optionAmounts = techOption.getBuyingNegativeAmountsInOrder();
            ...
            NoAlternativeAvailable
    {
        ...
        {
            final TechnologyOption techOption = (TechnologyOption) alternativeAt(option);
            final double[] optionAmounts = techOption.getSellingPositiveAmountsInOrder();
            for (int c = 0; c < amounts.length; c++)
            {
                amounts[c] += probabilities[option] * optionAmounts[c];
            }
    ...
    public void addAlternative(Alternative a)
    {
        ...
        {
            final String msg = "Tried to add an alternative of type " + a.getClass().getName()
            ...
            TableDataSet consumptionTable, int row)
    {
        ...
        {
            final TechnologyOption option = (TechnologyOption) getAlternatives().get(o);
            highestOption = Math.max(option.calculateVarianceTimes6DividedByPiSquared(
                    productionTable, consumptionTable, row), highestOption);
        }
        final double max = 1 / Math.sqrt(highestOption);
        if (getDispersionParameter() > max)
        {
            logger.warn("Technology dispersion parameter too high for " + name + ", maximum is "
                    + max);
        }
        logger.info("Dispersion parameter ratio for " + this + " is " + getDispersionParameter()
                / max);
            ...
            double[] sellingCommodityUtilities, int zoneIndex)
    {
        ...
        final int size = buyingCommodityUtilities.length + sellingCommodityUtilities.length;
            ...
            } else
            {
                ...
                for (int p = 0; p < choiceProbabilitiesTemporaryStorage.length; p++)
                {
                    value += choiceProbabilitiesTemporaryStorage[p]
                            * getTechnicalCoefficientsMatrix()[i][p] * diffFromAmountBaseline[i][p];
                }
    ...
    private double[][] getDifferenceAmountsFromBaseline(double[] buyingCommodityUtilities,
            double[] sellingCommodityUtilities, int zoneIndex)
    {
        setUtilities(buyingCommodityUtilities, sellingCommodityUtilities, zoneIndex);
        final int size = buyingCommodityUtilities.length + sellingCommodityUtilities.length;
    ...
    {
        final double[][] matrix = new double[myCommodityOrder.size() * 2][getAlternatives().size()];
        for (int o = 0; o < getAlternatives().size(); o++)
        {
            final TechnologyOption techOption = (TechnologyOption) getAlternatives().get(o);
            final double[] buyingAmounts = techOption.getBuyingNegativeAmountsInOrder();
            final double[] sellingAmounts = techOption.getSellingPositiveAmountsInOrder();
    ...
    private double[][] getUtilityDerivativesWrtCommodityUtilities()
    {
        ...
        if (utilityDerivativesWrtCommodityUtilities == null)
        {
            ...
            {
                final TechnologyOption techOption = (TechnologyOption) getAlternatives().get(o);
                final double[] scaledBuying = techOption.getBuyingNegativeScaledAmountsInOrder();
                final double[] scaledSelling = techOption.getSellingPositiveScaledAmountsInOrder();
    ...
    public double[] compositeUtilityDerivatives(double[] buyingCommodityUtilities,
            double[] sellingCommodityUtilities, int zoneIndex) throws NoAlternativeAvailable
    {
        ...
        {
            final String error = "Overflow calculating compositeUtilityDerivatives " + e;
    ...
    public void writeOutOptionUtilities(int activityId, int zoneNumber, PrintWriter out)
