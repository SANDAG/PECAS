...
public class TechnologyOption implements Alternative {
	...
	public TechnologyOption(LogitTechnologyChoice myChoiceModel, double constant,
			String name) {
		myName = name;
	...
	/**
	 * sorts internally so that when calcAmounts() or overallUtility() are called
	 * with commodity utilities passed in as an array of doubles, the
	 * CommodityQuantities object knows which utility vlaue applies to which
	 * commodity. This also affects subsequent calls to commodityAt() and must be
	 * called before commodityAt() is called.
	 * 
	 * @param commodityList
	...
	public void sortToMatch(java.util.List commodityList) {
		...
		for (int c = 0; c < commodityIndexNumbers.length; c++) {
			...
			if (found == false) {
				final String msg = "Can't find "
						+ commodityAmounts.get(c).commodity
	...
	public double getUtilityNoSizeEffect() throws ChoiceModelOverflowException {
		...
		for (int c = 0; c < commodityAmounts.size(); c++) {
			...
			if (ca.amount < 0) {
				utility += -ca.amount * ca.scale
						* buyingUtilities[commodityIndexNumbers[c]];
			...
			if (ca.amount > 0) {
				utility += ca.amount * ca.scale
						* sellingUtilities[commodityIndexNumbers[c]];
			}
		}
		return utility;
	}
	...
	public void printUtilityAndSizes(
			double dispersionParameterForSizeTermCalculation, PrintWriter out)
			throws ChoiceModelOverflowException {
		if (commodityIndexNumbers == null) {
			final String msg = "For TechnologyOptions after creating options you need to call sortToMatch first before trying to calculate their utility";
			logger.fatal(msg);
		...
		utility += 1 / dispersionParameterForSizeTermCalculation * Math.log(size);
		out.print(utility + "," + constant + "," + (baseUtility - constant) + ","
				+ 1 / dispersionParameterForSizeTermCalculation * Math.log(size) + ","
				+ size);
	}
	...
	public double calculateVarianceTimes6DividedByPiSquared(
			TableDataSet productionTable, TableDataSet consumptionTable, int tableRow) {
		double variance6pi = 0;
		for (int a = 0; a < commodityAmounts.size(); a++) {
			final CommodityAmount ca = commodityAmounts.get(a);
			double stdev = 0;
			if (ca.amount > 0) {
				stdev = ca.amount * ca.scale
						/ ca.commodity.getDefaultSellingDispersionParameter();
				int column = productionTable.getColumnPosition(ca.commodity.name);
				if (column == -1) {
					productionTable.appendColumn(
							new float[productionTable.getRowCount()], ca.commodity.name);
					column = productionTable.checkColumnPosition(ca.commodity.name);
				}
				productionTable.setValueAt(
						tableRow,
						column,
						Math.max((float) stdev,
								productionTable.getValueAt(tableRow, column)));
			}
			else if (ca.amount < 0) {
				stdev = -ca.amount * ca.scale
						/ ca.commodity.getDefaultBuyingDispersionParameter();
				int column = consumptionTable.getColumnPosition(ca.commodity.name);
				if (column == -1) {
					consumptionTable.appendColumn(
							new float[consumptionTable.getRowCount()], ca.commodity.name);
					column = consumptionTable.checkColumnPosition(ca.commodity.name);
				}
				consumptionTable.setValueAt(
						tableRow,
						column,
						Math.max((float) stdev,
								consumptionTable.getValueAt(tableRow, column)));
			}
