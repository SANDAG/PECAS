...
public class CommodityPriceSurplusDerivativeMatrix {
	...
	public CommodityPriceSurplusDerivativeMatrix(Commodity c) {
		...
		// Exchange[] exchanges = (Exchange[]) c.getAllExchanges().toArray(new
		// Exchange[1]);
		...
		for (int z = 0; z < allZones.length; z++) {
			...
			final CommodityZUtility bzu = c.retrieveCommodityZUtility(allZones[z],
					false);
			final CommodityZUtility szu = c.retrieveCommodityZUtility(allZones[z],
					true);
			if (c.exchangeType != 'n' && c.exchangeType != 'c') {
				temp = bzu.myFlows.getChoiceDerivatives(temp);
				final double scale = -bzu.getQuantity()
						* c.getBuyingUtilityPriceCoefficient();
			...
			if (c.exchangeType != 'n' && c.exchangeType != 'p') {
				...
				final double scale = szu.getQuantity()
						* c.getSellingUtilityPriceCoefficient();
				for (i = 0; i < temp.length; i++) {
					for (j = 0; j < temp[i].length; j++) {
						data[i][j] += temp[i][j] * scale;
					}
			...
			// and location
			// ENHANCEMENT simplify if there is no exchange choice (not
			// transportable, or exchanged where bought)
			double[] xProbabilitiesDouble = bzu.getExchangeProbabilities();
			// DenseVector xProbabilities = new
			// DenseVector(xProbabilitiesDouble);
			// create a column matrix;
			// DenseMatrix xProbabilitiesMatrix = new
			// DenseMatrix(xProbabilities.size(),1);
			// xProbabilitiesMatrix.setColumn(0,xProbabilities);
			double[] logSumDerivativesDouble = bzu.myFlows
					.getLogsumDerivativesWRTPrices();
			// DenseVector logSumDerivatives = new
			// DenseVector(logSumDerivativesDouble);
			...
			// exchangeQuantityChangeByPrice = a.multiply(xProbabilitiesMatrix,
			// logSumDerivativesMatrix);
			final double bzuDerivative = bzu.getDerivative();
			int col;
			int r;
			for (r = 0; r < xProbabilitiesDouble.length; r++) {
				for (col = logSumDerivativesDouble.length - 1; col >= 0; col--) {
					// data[r][col]+=exchangeQuantityChangeByPrice.elementAt(r,
					// col) * -bzu.getDerivative();
					data[r][col] += xProbabilitiesDouble[r]
							* logSumDerivativesDouble[col] * -bzuDerivative;
				}
			}

			// now for selling
			// ENHANCEMENT simplify if there is no exchange choice (non
			// transportable, or exchanged where sold)
			...
			// logSumDerivativesMatrix = new
			// DenseMatrix(1,logSumDerivatives.size());
			// logSumDerivativesMatrix.setRow(0,logSumDerivatives);

			// exchangeQuantityChangeByPrice = a.multiply(xProbabilitiesMatrix,
			// logSumDerivativesMatrix);
			final double szuDerivative = szu.getDerivative();

			for (r = 0; r < xProbabilitiesDouble.length; r++) {
				for (col = logSumDerivativesDouble.length - 1; col >= 0; col--) {
					data[r][col] += xProbabilitiesDouble[r]
							* logSumDerivativesDouble[col] * szuDerivative;
				}
			}
