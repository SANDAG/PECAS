...
class ActivityMatrixInitializer implements Runnable {
        ...
        public void run() {
            ...
            DenseMatrix dulbydprice = new DenseMatrix(fpl.sizeOfColumns(),AveragePriceSurplusDerivativeMatrix.numCommodities);
            int[] rows = new int[1];
            int[] columns = new int[AveragePriceSurplusDerivativeMatrix.numCommodities];
            ...
            DenseMatrix dLocationByDPrice = new DenseMatrix(AbstractZone.getAllZones().length,AveragePriceSurplusDerivativeMatrix.numCommodities);
                ...
                for (int r1 = 0;r1<dLocationByDPrice.sizeOfRows();r1++) {
                    for (int c1 = 0; c1<dLocationByDPrice.sizeOfColumns();c1++) {
                        if (Double.isNaN(dLocationByDPrice.elementAt(r1,c1))) {
                            AveragePriceSurplusDerivativeMatrix.logger.fatal("NaN in dLocationByDPrice, writing matrices to console");
            ...
            for (int location =0;location<pl.size();location++) {
                VectorI dThisLocationByPrices = new DenseVector(AveragePriceSurplusDerivativeMatrix.numCommodities);
