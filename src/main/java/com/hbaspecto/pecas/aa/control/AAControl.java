...
public class AAControl {
    ...
    public int runAAToFindPrices(){
        ...
        if ( Commodity.isCalculateSizeTerms()) {
            ...
            boolean forceConstraints = AggregateActivity.isForceConstraints();
            AggregateActivity.setForceConstraints(false); // no constraints in size term calculations.
            int iteration =0;
...
//            logger.info("*   Starting iteration "+ (nIterations+1)+".  Merit measure is "+newMeritMeasure);
    ...
    void runModelPerhapsWithConstraints() throws ModelDidntWorkException {
        ...
        boolean constrainedRun = ResourceUtil.getBooleanProperty(aaRb,"constrained",false);
        if(constrainedRun){
            logger.info("Running in constrained mode");
            //int constraintIterations = Integer.valueOf(ResourceUtil.getProperty(aaRb, "constraint.iterations"));
            double constraintSmoothing = ResourceUtil.getDoubleProperty(aaRb, "constraint.smoothing",1);
            //aaReaderWriter.maxConstantChange= ResourceUtil.getDoubleProperty(aaRb,"constraint.maxConstantChange",2.5);
            ...
            writeData();
            if (!AAModel.checkConstraints(ResourceUtil.getDoubleProperty(aaRb,"constraint.tolerance",0.001))) {
