...
public class AAControl {
	...
	public int runAAToFindPrices() {
		...
		final int logFrequency = ResourceUtil.getIntegerProperty(aaRb,
				"aa.logFrequency", 1);
		logger
				.info("* NOTE: Commodity Specific Merit Measures will be logged every "
						+ logFrequency + " iterations.");

		final boolean isParallel = ResourceUtil.getBooleanProperty(aaRb,
				"aa.jppfParallel", false);

		if (isParallel) {
			aa = new com.hbaspecto.pecas.aa.jppf.JppfAAModel(aaRb);
			logger
					.info("AA is running in multi-machine mode. To modify this configuration, refer to the aa.jppfParallel property in aa.properties file.");
		}
		else {
			aa = new AAModel(aaRb);
			logger
		...
		maxIterations = ResourceUtil.getIntegerProperty(aaRb, "aa.maxIterations",
				maxIterations);
		if (maxIterations == 0) {
		...
		final boolean calcDeltaUsingDerivatives = ResourceUtil.getBooleanProperty(
		...
		if (nanPresent) {
			logger
					.fatal("Initial prices cause overflow -- try again changing initial prices");
		...
		while (!stoppingNextIteration) {
			...
			if (isCurrentlyConverged) {
				...
				if (calcAvgPrice) {
					...
					logger
							.error("Don't use diagonal approximation anymore, please set aa.calculateAveragePrices to true or remove entry from properties file");
				...
				if (nIterations == maxIterations - 1) {
					...
					logger
							.warn("!!  Not Improving and at second last iteration -- backing up to last valid prices");
				}
				...
				if (nanPresent) {
					logger
							.warn("Overflow error, setting new merit measure to positive infinity");
					newMeritMeasure = Double.POSITIVE_INFINITY;
				}
			}
			isCurrentlyConverged = isConverged(nIterations);
			if (nIterations >= maxIterations && !isCurrentlyConverged) {
				stoppingNextIteration = true;
				logger
						.fatal("Terminating because maximum iterations reached -- did not converge to tolerance");
			}
	...
	private boolean isConverged(int iteration) {
		...
		final double largestSClear = Commodity
				.getLargestSClearForAllCommodities(ResourceUtil.getDoubleProperty(aaRb,
						"aa.ConFac"));
		if (largestSClear > ResourceUtil.getDoubleProperty(aaRb,
	...
	public static void main(String[] args) {
		...
		if (args.length == 0) {
			...
			logger
					.fatal("usage: com.pb.models.pecas.AAControl [baseYear timeInterval]");
			throw new RuntimeException(
