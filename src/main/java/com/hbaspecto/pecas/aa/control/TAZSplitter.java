...
public class TAZSplitter {
	 ...
	 * stored in an array Dimension 0 is production activities Dimension 1 is beta
	 * zone Dimension 2 is commodity name Dimension 3 is "M" or "U" for make or
	 * use
	 */
	...
	TAZSplitter(AlphaToBeta a2b, Hashtable floorspaceInventory, int maxTAZ,
			String outputPath, PECASZone[] zones,
			StringIndexedNDimensionalMatrix zonalMakeUseCoefficients) {
	...
	void writeFloorspaceZoneTables(
			ArrayList<String> CommodityNamesForDetailedOutputs) {
		...
		try {
			locationsFile = new BufferedWriter(new FileWriter(outputPath
			...
			AAStatusLogger
					.logText("Splitting production activities into floorspace zones");
			while (productionActivityIterator.hasNext()) {
				final ProductionActivity p = (ProductionActivity) productionActivityIterator
						.next();
				...
				try {
					...
					for (int betaZoneIndexNumber = 0; betaZoneIndexNumber < p
							.getMyDistribution().length; betaZoneIndexNumber++) {
						...
						for (int c = 0; c < cf.size(); c++) {
							...
							if (com == null) {
								...
								buyingZUtilities[c] = com.calcZUtility(
						...
						for (int c = 0; c < pf.size(); c++) {
							...
							if (com == null) {
								...
								sellingZUtilities[c] = com.calcZUtility(
										zones[betaZoneIndexNumber], true);
							}
						} // CUSellc,z and CUBuyc,z have now been calculated for
							// the commodites made or used by the activity
						...
						try {
							...
							totalFloorspaceConsumption = calculateTotalFloorspaceConsumption(
									cf, betaZoneIndexNumber, buyingZUtilities, amounts);
						}
						...
						if (totalFloorspaceConsumption == 0) {
							...
							splitActivityTotalBasedOnSpaceConsumption(cf,
									activityLocationsSplit, betaZoneIndexNumber, pecasZoneTotal,
					...
					final int[] alphaZonesArray = floorspaceZoneCrossref
							.getAlphaExternals0Based();
					...
					splitMakeAndUseAmount(commodityNames, p, activityLocationsSplit,
							CommodityNamesForDetailedOutputs, detailedMakeFile,
							detailedUseFile);
	...
	private void writeAsciiAlphaZoneTotalMakeUse(String[] commodityNames) {
		try {
			...
			logger
					.fatal("Can't write out FloorspaceZoneTotalMakeUse use ascii csv file "
							+ e);
		}
	}
	...
	private void writeBinaryAlphaZoneTotalMakeUse() {
		...
		if (filename != null) {
			try {
				...
				logger
						.fatal("Can't write out FloorspaceZoneTotalMakeUse use binary file "
								+ e);
			}
		}
	}
	...
	private void splitMakeAndUseAmount(String[] commodityNames,
			...
			ArrayList<String> commodityNamesForDetailedOutput,
			BufferedWriter detailedMakeFile, BufferedWriter detailedUseFile)
			throws IOException {
		final int[] zonalMakeUseIndices = new int[4];
		final String[] aZoneTotalMUIndices = new String[2];
		zonalMakeUseIndices[0] = zonalMakeUseCoefficients
				.getIntLocationForDimension(0, productionActivity.name);

		final int makeIndex = zonalMakeUseCoefficients.getIntLocationForDimension(
				3, "M");
		final int useIndex = zonalMakeUseCoefficients.getIntLocationForDimension(3,
				"U");
		for (int azone = 0; azone < activityLocationsSplit.length; azone++) {
			...
			if (betaZone != -1) {
				...
				// those
				zonalMakeUseIndices[1] = zonalMakeUseCoefficients
						.getIntLocationForDimension(1, String.valueOf(betaZone));
				for (int commodity = 0; commodity < commodityNames.length; commodity++) {
					...
					if (commodityNamesForDetailedOutput != null) {
						if (commodityNamesForDetailedOutput
								.contains(commodityNames[commodity])) {
							detailedOutputCommodity = true;
						}
					}
					zonalMakeUseIndices[2] = zonalMakeUseCoefficients
							.getIntLocationForDimension(2, commodityNames[commodity]);
					...
					try {
						zonalMakeLocation = alphaZonalMake
								.getIntLocation(aZoneTotalMUIndices);
					}
					...
					double zonalMakeValue = activityLocationsSplit[azone]
							* makeCoefficient;
					...
					try {
						zonalUseLocation = alphaZonalUse
								.getIntLocation(aZoneTotalMUIndices);
					}
					catch (final RuntimeException e) {
						// location doesnt exist yet
	...
	private double calculateTotalFloorspaceConsumption(ConsumptionFunction cf,
		...
		try {
			...
			logger
					.fatal("Error adding activity quantity to ActivityLocations2 table");
			e.printStackTrace();
		}
		double totalFloorspaceConsumption = 0;
	...
	private String[] setupZonalMakeAndUseObjects() {
		...
		alphaZonalMake = new StringIndexedNDimensionalMatrix("zonalMake", 2,
				makeUseArraySize, columnNames);
		alphaZonalUse = new StringIndexedNDimensionalMatrix("zonalUse", 2,
				makeUseArraySize, columnNames);

		// set up indices for zonalMake and zonalUse
		Commodity[] commodities = new Commodity[AbstractCommodity
				...
				if (commodity.isFloorspaceCommodity()) {
					...
					if (floorspaceTotal == 0) {
						...
						for (final int alpha2 : alphaZones) {
							final int betaZoneNumber = floorspaceZoneCrossref
									.getBetaZone(alpha2);
