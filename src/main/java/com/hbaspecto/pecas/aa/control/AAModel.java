        ...
        while (comIt.hasNext()) {
            ...
            for (int xNum=0; xNum < exchanges.size(); xNum++) {
                ...
                if (c.isHasFixedPrices()) {
                	...
                	// unfair to the zones that ARE chaning their prices to try to solve any global inbalance problem with the commodity.
	            ...
	            for (int beta=0;beta<amounts.length;beta++) {
	                if (amounts[beta].isConstrained()) {
	                    if (amounts[beta].constraintQuantity!= amounts[beta].quantity) {
	                        String msg = "Problem in constraint process -- constraint quantity is unequal to quantity for "+amounts[beta];
	                        logger.fatal(msg);
	                        throw new RuntimeException("msg");
	                    }
	                    double currentConstant = amounts[beta].getLocationSpecificUtilityInclTaxes();
	                    ...
	                    if (amounts[beta].constraintQuantity==0) {
	                        ...
	                        // constraint !=0
	                        if (probs[beta]==0) {
	                            if (runAroundCount==1) ascChangeLogger.error("In zone "+amounts[beta].getMyTaz().getZoneUserNumber()+" constraint for "+a.name+" is non-zero ("+amounts[beta].constraintQuantity+") but current amount is zero.  Increasing Zonal ASC by arbitrary amount.");
	                            ...
	                            double probRatio = (amounts[beta].constraintQuantity/totalAmount/probs[beta]);
	                    ...
	                    ascChangeLogger.debug("Changing zone "+amounts[beta].getMyTaz().getZoneUserNumber()+" constant on "+a.name+" by "+constantChange+" to attempt to change "+amounts[beta].quantity+" to "+amounts[beta].constraintQuantity+", current constant is "+currentConstant);
            ...
            for (int beta=0;beta<amounts.length;beta++) {
                if (amounts[beta].isConstrained()) {
                    if (!checkRelativeError(constraintTolerance,
                        ...
                        String msg = "constraint not matched, cons="+amounts[beta].constraintQuantity+" , modelled="+amounts[beta].quantity+", in "+amounts[beta];
                        logger.error(msg);
                        ok = false;
                    }
                }
