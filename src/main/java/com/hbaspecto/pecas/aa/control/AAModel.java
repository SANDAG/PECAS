...
public class AAModel {
	...
	/*
	 * This constructor is used by AAControl and by AADAF - The aa.properties file
	 * will not be in the classpath when running on the cluster (or even in the
	 * mono-version when running for mulitple years using AO) and therefore we
	 * need to pass in the ResourceBundle to aaModel from within the AAServerTask.
	 * Because there may be more than 1 aa.properties file over the course of a 30
	 * year simulation it is not practical to include 30 different location in the
	 * classpath. AO will locate the most recent aa.properties file based on the
	 * timeInterval and will write it's absolute path location into a file called
	 * 'RunParameters.txt' where it will be read in by AAServerTask in it's
	 * 'onStart( ) method and will subsequently be passed to aaModel during the
	 * 'new aaModel(rb)' call.
	 */
	public AAModel(ResourceBundle aaRb) {
		setResourceBundles(aaRb);
		final String initialStepSizeString = ResourceUtil.getProperty(aaRb,
				"aa.initialStepSize");
		if (initialStepSizeString == null) {
			logger
					.info("*   No aa.initialStepSize set in properties file -- using default");
		}
		else {
		...
		minimumStepSize = ResourceUtil.getDoubleProperty(aaRb,
				"aa.minimumStepSize", 1.0E-4);
		logger.info("*   Minimum step size set to " + minimumStepSize);

		conFac = ResourceUtil.getDoubleProperty(aaRb, "aa.ConFac");

		final String maximumStepSizeString = ResourceUtil.getProperty(aaRb,
				"aa.maximumStepSize");
		if (maximumStepSizeString == null) {
			logger
					.info("*   No aa.maximumStepSize set in properties file -- using default");
		}
		else {
		...
		final String convergedString = ResourceUtil.getProperty(aaRb,
				"aa.converged");
		if (convergedString == null) {
			logger
					.info("*   No aa.converged set in properties file -- using default");
		}
		else {
		...
		final String localPriceStepSizeAdjustmentString = ResourceUtil.getProperty(
				aaRb, "aa.localPriceStepSizeAdjustment");
		if (localPriceStepSizeAdjustmentString == null) {
			logger
					.info("*   No aa.localPriceStepSizeAdjustment set in properties file -- using default of 1.0");
		}
		else {
		...
		final String commoditySpecificAdjustmentString = ResourceUtil.getProperty(
				aaRb, "aa.commoditySpecificAdjustment");
		if (commoditySpecificAdjustmentString == null) {
			logger
					.info("*   No aa.commoditySpecificAdjustment set in properties file -- using default");
		}
		else {
	...
	public boolean calculateExchangeSizeTermsForSpecifiedNonFloorspace() {
		...
		for (final ProductionActivity p : ProductionActivity
		...
		if (nanPresent) {
			...
			throw new RuntimeException(
					"Default prices caused overflow in size term calculation");
		}
	 ...
	 * Commodity. These prices are effectively the input to this routine, although
	 * they do not appear in the parameter list because a precondition is that the
	...
	public boolean calculateCompositeBuyAndSellUtilities() {
		...
		logger
				.info("Composite buy and sell utilities have been calculated for all commodities. Time in seconds: "
						+ (System.currentTimeMillis() - startTime) / 1000.0);
		return nanPresent;
	}

	/**
	 * This method calculates the TC, TP and derivative of TC and derivative of TP
	 * for each commodity in each zone. Along the way, it calculates several
	...
	public boolean calculateLocationConsumptionAndProduction() {
		if (logger.isDebugEnabled()) {
			logger
					.debug("Beginning Activity Iteration: calling 'migrationAndAllocationWithOverflowTracking' for each activity");
		}
		...
		while (it.hasNext()) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Finished activity " + count + " in "
						+ (System.currentTimeMillis() - activityStartTime) / 1000.0
						+ " seconds");
			}
			count++;
		}
	...
	public boolean recalculateLocationConsumptionAndProduction() {
		...
		if (logger.isDebugEnabled()) {
			logger
					.debug("Beginning Activity Iteration: calling 'reMigrationAndAllocationWithOverflowTracking' for each activity");
		}
		CommodityZUtility.resetCommodityBoughtAndSoldQuantities();
		final long startTime = System.currentTimeMillis();
		boolean nanPresent = false;
	...
	/**
	 * This method calculates the Buying and Selling quantities of each commodity
	 * produced or consumed in each zone that is allocated to a particular
	 ...
	 *          indicates whether we are currently setting size terms, ignored in
	 *          this Symmetric Multiprocessing version but in multiple-machine
	...
	public double calculateMeritMeasureWithoutLogging() throws OverflowException {
		...
		final Iterator commodities = AbstractCommodity.getAllCommodities()
				.iterator();
	...
	public double calculateMeritMeasureWithLogging() throws OverflowException {
		...
		while (commodities.hasNext()) {
			...
			final double rmsError = Math.sqrt(commodityMeritMeasure
					/ c.getAllExchanges().size());
			final double oldRmsError = Math.sqrt(c.oldMeritMeasure
					/ c.getAllExchanges().size());
			if (commodityMeritMeasure > c.oldMeritMeasure) {
				...
				logger.info("\t Commodity RMS Error " + rmsError
						+ " NOT IMPROVING was " + oldRmsError + " (adj now "
						+ c.scalingAdjustmentFactor + ")");
			}
			else {
				c.scalingAdjustmentFactor *= 1 + commoditySpecificScalingAdjustment;
				logger.info("\t Commodity RMS Error " + rmsError + " (was "
						+ oldRmsError + ") (adj now " + c.scalingAdjustmentFactor + ")");
			}
			logger.info("\t Weighted Commodity Merit Measure is "
					+ commodityMeritMeasure * c.compositeMeritMeasureWeighting
					* c.compositeMeritMeasureWeighting);
			final double tClear = Math
					.sqrt(commodityMeritMeasure * c.compositeMeritMeasureWeighting
							* c.compositeMeritMeasureWeighting)
	...
	public void decreaseStepSizeEvenIfBelowMinimumAndAdjustPrices() {
		...
		stepSize = stepSize * decreaseStepSizeMultiplier;
		if (stepSize < minimumStepSize) {
			logger.warn("Setting step size to " + stepSize
					+ " which is *below* minimum of " + minimumStepSize);
		}
		newPricesC = StepPartwayBackBetweenTwoOtherPrices(oldPricesC, newPricesC,
				decreaseStepSizeMultiplier);
		setExchangePrices(newPricesC);
	...
	public void calculateNewPricesUsingBlockDerivatives(
			boolean calcDeltaUsingDerivatives) {
		...
		if (stepSize >= 1) {
			for (int i = 0; i < averagePriceChange.size(); i++) {
				averagePriceChange.setElementAt(i, averagePriceChange.elementAt(i)
						* stepSize);
			}
		}
		...
		while (comIt.hasNext()) {
			...
			if (calcDeltaUsingDerivatives) {
				try {
					...
					// diagonal
					if (stepSize * localPriceStepSizeAdjustment
				...
				for (int xNum = 0; xNum < exchanges.size(); xNum++) {
					...
					deltaPricesDouble[xNum] = increase * stepSize
							* localPriceStepSizeAdjustment * c.scalingAdjustmentFactor;
					// TODO remove this debug code
					// Debug code of 2007.05.10
					if (x.monitor) {
						logger.info("In " + x + " surplus is " + sAndD[0]
								+ " derivative is " + sAndD[1]
								+ " unscaled local price change " + increase + " scaled "
	...
	private void writeOutMatrix(AveragePriceSurplusDerivativeMatrix avgMatrix,
			Exception e) {
		logger.fatal("Having problems solving for change in average prices", e);
		logger
				.fatal("Writing out average price change derivative matrix to file AvgMatrix.txt");
		FileOutputStream badMatrixStream = null;
		try {
			badMatrixStream = new FileOutputStream(ResourceUtil.getProperty(aaRb,
					"output.data") + "AvgMatrix.txt");
		}
		catch (final FileNotFoundException e1) {
			logger.fatal("Can't seem to open file "
					+ ResourceUtil.getProperty(aaRb, "output.data") + "AvgMatrix.txt");
			throw new RuntimeException(e);
		}
	...
	public static void writeOutMatrix(Matrix aMatrix, String matrixName) {
		logger.info("Having problems with matrix " + matrixName
				+ " writing it out to a file");
		FileOutputStream badMatrixStream = null;
		try {
			badMatrixStream = new FileOutputStream(lastAaRb.getString("output.data")
					+ matrixName + ".txt");
		}
		catch (final FileNotFoundException e1) {
			logger.error("Can't seem to open file "
					+ lastAaRb.getString("output.data") + matrixName + ".txt");
		}
		final SMFWriter matrixWriter = new SMFWriter(badMatrixStream);
			...
			while (exchanges.hasNext()) {
				...
				if (ex.monitor || Double.isNaN(surplusAndDerivative[0])
					...
					logger.info("Exchange:" + ex + " surplus:" + surplusAndDerivative[0]
							+ " planning price change from " + ex.getPrice() + " to "
							+ newPrice);
				}
				if (c.isDoSearch() && ex.isDoSearch()) {
	 ...
	 */
	private static HashMap StepPartwayBackBetweenTwoOtherPrices(
			HashMap firstPrices, HashMap secondPrices, double howFarBack) {
	 ...
	 * aa.properties file has been found in the tn subdirectories and has been set
	 * by calling the setApplicationResourceBundle method (a ModelComponent
	 * method) from AO.
	 * 
	...
	public void startModel(int baseYear, int timeInterval) {
		...
		AAControl aa;
		try {
			aa = new AAControl(Class.forName(pProcessorClass), aaRb, baseYear,
					timeInterval);
		}
		catch (final ClassNotFoundException e) {
			throw new RuntimeException(pProcessorClass
	...
	public void updateActivityZonalConstantsBasedOnConstraints(double adjustment) {
		...
		final Collection activities = ProductionActivity
				.getAllProductionActivities();
		...
		while (it.hasNext()) {
			...
			do {// have to do this iteratively because the logsum denominator
				// changes
				maxAbsChange = 0;
				runAroundCount++;
				try {
					...
					final String msg = "Activity " + a
							+ " seems to have no location alternatives";
					logger.fatal(msg);
				...
				for (int beta = 0; beta < amounts.length; beta++) {
					if (amounts[beta].isConstrained()) {
						...
						ascChangeLogger.debug("Changing zone "
								+ amounts[beta].getMyTaz().getZoneUserNumber()
								+ " constant on " + a.name + " by " + constantChange
								+ " to attempt to change " + amounts[beta].quantity + " to "
			...
			for (int beta = 0; beta < amounts.length; beta++) {
				if (amounts[beta].isConstrained()) {
					if (!checkRelativeError(constraintTolerance,
							amounts[beta].constraintQuantity, amounts[beta].quantity)) {
