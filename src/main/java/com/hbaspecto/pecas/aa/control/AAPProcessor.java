...
public abstract class AAPProcessor {
	...
	protected PECASZone[] zones; // will be initialized in setUpZones method
	...
	static class HistogramSpec {
		...
		/**
		 * <code>boundaries</code> contains Float objects describing the histogram
		 * band boundaries
		 */
	...
	public void setUpAA() {
		...
		if (ResourceUtil.getBooleanProperty(aaRb,
			...
			logger
					.info("aa.automaticTechnologySizeTerms is true, not setting FloorspaceBuyingSizeTerms");
			setFloorspaceProportionsForTechnologySizeTerms(floorspaceInventoryByLUZ);
			logger
					.info("Since you are using technology size terms, ActivitySizeTermsI should only have entries for activities that do not use space");
			recalcActivitySizeTerms(floorspaceInventoryByLUZ);
		}
		else {
			logger
					.warn("aa.automaticTechnologySizeTerms is false, using FloorspaceBuyingSizeTerms instead, this is deprecated functionality, please upgrade your model");
			recalcActivitySizeTerms(floorspaceInventoryByLUZ);
			logger
					.info("Since you are not using technology size terms, be sure ActivitySizeTermsI has entries for all activities that use space");
	...
	protected String[] setUpCommodities() {
		...
		if (manualSizeTermsC <= 0) {
			if (ResourceUtil
					.getBooleanProperty(aaRb, "calculateExchangeSizes", false)) {
			...
			logger
					.info("ManualSizeTerms column in CommoditiesI, calculating size terms for non floorspace commodities where ManualSizeTerms=true");
			Commodity.setCalculateSizeTerms(true);
			if (!ResourceUtil
					.getBooleanProperty(aaRb, "calculateExchangeSizes", true)) {
				logger
						.warn("calculateExchangeSizes=false in properties file, this conflicts with the existance of the ManualSizeTerms column in CommoditiesI, ignoring entry in properties file");
			}
		}
		...
		for (int row = 1; row <= ctab.getRowCount(); row++) {
			...
			final float defaultBuyingDispersionParameter = ctab.getValueAt(row,
					bdpColumn);
			final float defaultSellingDispersionParameter = ctab.getValueAt(row,
					sdpColumn);
			...
			final Commodity c = Commodity.createOrRetrieveCommodity(commodityName,
			...
			while (found) {
				final int coeffColumnNumber = ctab
						.getColumnPosition("InterchangeCoefficient" + skimNumber);
				final int nameColumnNumber = ctab.getColumnPosition("InterchangeName"
						+ skimNumber);
				if (skimNumber == 1
	...
	protected void readFloorspaceZones() {
		...
		if (ResourceUtil.getProperty(aaRb, "aa.useFloorspaceZones")
			...
			logger
					.info("Not using floorspace zones (TAZs) -- TAZ are the same as LUZ");
	...
	protected void setUpExchangesAndZUtilities() {
		...
		if (exchanges == null) {
			logger
					.info("Did not find ExchangeImportExportI in aa.current.data, looking in aa.base.data");
			exchanges = loadTableDataSet("ExchangeImportExportI", "aa.base.data",
					true);
		}
		else {
			logger
					.info("Found year-specific ExchangeImportExportI in current year (aa.current.data)");
		}
		...
		if (monitorColumn == -1) {
			logger
					.warn("No MonitorExchange column in ExchangeImportExport table -- not monitoring any exchanges");
		}
		...
		while (comit.hasNext()) {
				...
				if (c.exchangeType != 's' || specifiedExchange) {
					...
					if (!found) {// backup default data.
						if (++numExchangeNotFoundErrors < 20 && !c.isFloorspaceCommodity()) {
							logger
									.info("Can't locate size term for Commodity "
											+ c
											+ " zone "
			...
			for (int z = 0; z < zones.length; z++) {
				...
				if (c.exchangeType == 'c' || c.exchangeType == 's'
						|| c.exchangeType == 'a') {
					final CommodityZUtility czu = c.retrieveCommodityZUtility(zones[z],
							true);
	...
	protected void setExchangePrices() {
		...
		if (initialPrices == null) {
			logger
					.info("No special price data for current year, check for previous year prices");
			initialPrices = loadTableDataSet("ExchangeResults", "aa.previous.data",
					false);
			if (initialPrices == null) {
		...
		if (commodityNameColumn * priceColumn * zoneNumberColumn <= 0) {
			logger
					.fatal("Missing column in Exchange Results -- check for Commodity, Price and ZoneNumber columns.  Continuing without initial prices");
		}
	...
	protected void setUpTransportConditions(String[] skimNames) {
		...
		final String skimFormat = ResourceUtil
				.getProperty(aaRb, "Model.skimFormat");
	...
	public TableDataSet readSkimFile(String path1, String path2,
		...
		if (s == null) {
			try {
				// read from file
				s = reader.readFile(new File(path2 + skimFileName), columnFilter,
						skimColumns);
			}
	...
	private ExcelFileReader getExcelFileReader() {
		...
		excelFileReader.setWorkbookFile(new File(ResourceUtil.checkAndGetProperty(
				aaRb, "aa.datasource")));
		return excelFileReader;
	}
	...
	private JDBCConnection getJDBCConnection() {
		...
		if (datasourceUrl != null) {
			...
			final String datasourceName = ResourceUtil.checkAndGetProperty(aaRb,
					"aa.datasource");
	...
	protected void recalcActivitySizeTerms(double[][] floorspaceInventoryByLUZ) {
		...
		final TableDataSet activitySizeTermsCalculation = loadTableDataSet(
				"ActivitySizeTermsI", "aa.base.data", false);
		if (activitySizeTermsCalculation == null) {
			logger
					.fatal("No ActivitySizeTermsI table, not recalculating activity size terms from floorspace quantities");
	...
	protected void recalcFloorspaceBuyingSizeTerms() {
		...
		if (floorspaceSizeTermsCalculation == null) {
			logger
					.warn("No FloorspaceBuyingSizeTermsI table, not recalculating floorspace buying size terms from floorspace quantities");
			return;
		}
		...
		for (int row = 1; row <= floorspaceSizeTermsCalculation.getRowCount(); row++) {
			...
			final ZoneQuantityStorage group = (ZoneQuantityStorage) floorspaceGroups
					.get(groupName);
			final String typeName = floorspaceSizeTermsCalculation.getStringValueAt(
					row, "FloorspaceType");
			...
			for (int betaZone = 0; betaZone < zones.length; betaZone++) {
				...
				if (!(groupName.equalsIgnoreCase("none") || groupName.equals(""))) {
					...
					if (groupQuantity == 0) {
						if (typeQuantity != 0) {
							logger
									.fatal("Error in size term calculation -- type quantity is zero but the associated group quantity is non-zero");
							throw new RuntimeException(
	...
	protected void recalcFloorspaceImport() {
		...
		if (deltaString != null) {
			logger
					.fatal("aa.floorspaceDelta is specified in properties file; this is deprecated, please specify FloorspaceSupplyI input table instead");
			throw new RuntimeException(
		...
		while (it.hasNext()) {
			...
			for (int bz = 0; bz < zones.length; bz++) {
				...
				final Iterator anotherIterator = fqs.getInventoryMap().entrySet()
						.iterator();
				...
				x.setExportFunction(new LogisticPlusLinearFunction(0, zeroPrice, 0, 0,
						exportSlope));
	...
	public void writeOutputs() {
		...
		if (ResourceUtil.getBooleanProperty(aaRb, "aa.useFloorspaceZones", true)) {
			if (ResourceUtil.getBooleanProperty(aaRb,
				...
				splitter = new TAZSplitter(tableA2B, floorspaceInventory,
						maxAlphaZone(), getOutputPath(), zones, zonalMakeUseCoefficients);
		...
		if (ResourceUtil.getBooleanProperty(aaRb, "aa.writeFlowMatrices", true)) {
			logger
					.info("Writing buying/selling matrices, PctIntrazonalxCommodityxBzone.csv and Histograms.csv");
			AAStatusLogger
	...
	public void writeTripMatrices(StringIndexedNDimensionalMatrix aZoneMake,
		...
		if (ResourceUtil.getBooleanProperty(aaRb, "aa.makeTripMatrices", false)) {
			...
			final double useRoundedAt = ResourceUtil
					.getDoubleProperty(aaRb, "aa.roundExpectedValueForTripsIfHigherThan",
			...
			final TableDataSet tripCalculationSpecification = loadTableDataSet(
					"TripCalculationsI", "aa.base.data");
			final boolean usePoissonDistribution = ResourceUtil.getBooleanProperty(
					aaRb, "aa.poissonTrips", true);
			final TripCalculator myTripCalculator = new TripCalculator(useNormalAt,
					useRoundedAt);
			...
			if (ResourceUtil.getBooleanProperty(aaRb, "aa.TAZTripList", false)) {
				if (aZoneMake == null || aZoneUse == null) {
					...
					final SparseMatrix[] tazTrips = myTripCalculator.sampleTazTrips(
							tripArrays, floorspaceZoneCrossref, tripCalculationSpecification,
					...
					if (ResourceUtil.getBooleanProperty(aaRb,
						...
						for (int i = 0; i < tazTrips.length; i++) {
							...
							final Emme2311MatrixWriter writer = new Emme2311MatrixWriter(
									new File(getOutputPath() + "TAZxTAZ"
											+ myTripCalculator.tripTypes[i] + ".csv"));
					...
					try {
						final BufferedWriter tazTripFile = new BufferedWriter(
								new FileWriter(getOutputPath() + "tazTrips.csv"));
	...
	public void writeZonalMakeUseCoefficients() {
		...
		if (binary) {
			utilities = new StringIndexedNDimensionalMatrix("Utility", 4, shape,
					columnNames);
			utilities.setAddKeysOnTheFly(true);
			quantities = new StringIndexedNDimensionalMatrix("Amount", 4, shape,
					columnNames);
		...
		try {
			...
			final BufferedWriter aggregateMakeUseFile = new BufferedWriter(
					new FileWriter(getOutputPath() + "MakeUse.csv"));
					...
					for (int c = 0; c < pf.size(); c++) {
						...
						if (com != null) {
							if (ascii) {
								...
								zMakeUseFile
										.write(activityAmount * productionAmounts[c] + "\n");
							}
							indices[2] = com.getName();
							zonalMakeUseCoefficients.setValue((float) productionAmounts[c],
									indices);
							if (binary) {
			...
			if (filename != null) {
				try {
					final java.io.FileOutputStream fos = new java.io.FileOutputStream(
							filename);
					final java.io.ObjectOutputStream out = new java.io.ObjectOutputStream(
							fos);
	...
	private void writeCommodityAndActivityFiles() {
		try {
			final BufferedWriter activitiesFile = new BufferedWriter(new FileWriter(
					getOutputPath() + "ActivityNumbers.csv"));
	...
	public void writeLocationTable(String tableName) {
		...
		try {
			locationsFile = new BufferedWriter(new FileWriter(getOutputPath()
					+ tableName + ".csv"));
			...
			while (it.hasNext()) {
				...
				for (int z = 0; z < p.getMyDistribution().length; z++) {
					try {
						...
						logger
								.fatal("Error adding activity quantity to ActivityLocations table");
						e.printStackTrace();
					}
						...
						if (ex.getImportFunction() instanceof LogisticPlusLinearWithOverrideFunction) {
							...
							if (overrideFile != null) {
								((LogisticPlusLinearWithOverrideFunction) ex
										.getImportFunction()).writeOverride(overrideFile,
										ex.exchangeLocationUserID, ex.myCommodity.getName(),
										ex.getPrice());
							}
				...
				if (exchangeResultsTotals != null) {
					try {
						exchangeResultsTotals.write(c.name + ","
								+ (internalBought + exports) + "," + internalBought + ","
								+ exports + "," + (internalSold + imports) + "," + internalSold
								+ "," + imports + "," + Math.sqrt(msSurplus) + "," + price
								+ "\n");
					}
					catch (final IOException e) {
			...
			while (exchangeIterator.hasNext()) {
				...
				try {
					...
					exchangeResults.write(ex.boughtTotal() + importExport[1] + ","
							+ ex.boughtTotal() + "," + importExport[1] + ","
							+ (ex.soldTotal() + importExport[0]) + "," + ex.soldTotal() + ","
							+ importExport[0] + "," + ex.exchangeSurplus() + ","
							+ ex.getPrice() + "\n");
				}
				catch (final IOException e) {
		...
		try {
			...
			while (it.hasNext()) {
				...
				try {
					...
					logger
							.fatal("Error adding activity quantity to activity location table");
					e.printStackTrace();
				}
				catch (final OverflowException e) {
	...
	public String[] readInHistogramSpecifications() {
		...
		if (histogramSpecTable == null) {
			logger
					.error("No histogram specifications -- please create a HistogramsI.csv file");
			return new String[0];
		}
		for (int row = 1; row <= histogramSpecTable.getRowCount(); row++) {
			...
			for (int i = 0; i < histogramSpecTable.getColumnCount() - 2; i++) {
				...
				if (column > 0) {
					...
					if (boundary > lastHistogramBoundary) {// force increasing
						// boudaries
	...
	public void writeAllFlowZipMatrices() {
		...
		try {
			final BufferedWriter histogramFile = new BufferedWriter(new FileWriter(
					getOutputPath() + "Histograms.csv"));
			histogramFile
					.write("Commodity,BuyingSelling,BandNumber,LowerBound,Quantity,AverageLength\n");
			final PrintWriter pctFile = new PrintWriter(
					new BufferedWriter(new FileWriter(getOutputPath()
							+ "PctIntrazonalxCommodityxBzone.csv")));
			pctFile
					.println("Bzone,Commodity,BuyIntra,BuyFrom,BuyTo,BuyPctIntraFrom,BuyPctIntraTo,SellIntra,SellFrom,SellTo,SellPctIntraFrom,SellPctIntraTo");
	...
	public void writeFlowZipMatrices(String name, Writer histogramFile,
		...
		if (b != null) {
			final ZipMatrixWriter zmw = new ZipMatrixWriter(new File(getOutputPath()
					+ "buying_" + com.name + zipExtension));
			zmw.writeMatrix(b);
		}
		...
		if (s != null) {
			final ZipMatrixWriter zmw = new ZipMatrixWriter(new File(getOutputPath()
					+ "selling_" + com.name + zipExtension));
			zmw.writeMatrix(s);
		}
		if (logger.isDebugEnabled()) {
			logger
					.debug("Buying and Selling Commodity Flow Matrices have been written for "
							+ name);
		}
	...
	protected void writePctIntrazonalFile(PrintWriter writer, String name,
		...
		try {
			/*
			 * for daf version, we have to write out a file for each commodity so we
			 * create a new file each time this routine is called, write to the file
			 * and then close it. In the monolithic version, we just write lines to a
			 * single file as we iterate over the commodities and the file will be
			 * closed by the calling method.
			 */
			if (writer == null) {
				writer = new PrintWriter(new BufferedWriter(new FileWriter(
						getOutputPath() + "PctIntrazonalxBetazone_" + name + ".csv")));
				writer
						.println("Bzone,Commodity,BuyIntra,BuyFrom,BuyTo,BuyPctIntraFrom,BuyPctIntraTo,SellIntra,SellFrom,SellTo,SellPctIntraFrom,SellPctIntraTo");
				closePctFile = true;
			}
			...
			/*
			 * close the file if we are running the DAF version of aa otherwise the
			 * file will be closed after we have iterated thru all the commodities.
			 */
	...
	/**
	 * @param histogramFile
	 *          the file to write the histogram to. Can be "null" in which case a
	 *          file named histograms_commodityName.csv will be created.
	 * @param commodityName
	 * @param buyingMatrix
	 * @param sellingMatrix
	 */
	protected void writeFlowHistograms(Writer histogramFile,
		...
		if (!ResourceUtil.getBooleanProperty(aaRb, "aa.externalZonesInHistogram",
				false)) {
				...
				synchronized (theExchanges) {
					...
					while (it.hasNext()) {
						...
						if (selling == 'b') {
							...
							writer.write(tazUserID + ",");
							writer
									.write(String.valueOf(-x.getFlowQuantity(tazIndex, selling)));
							/*
							 * if (myFlows.timeAndDistanceUtilities && myFlows.peakAutoSkims
							 * != null) { writer.write("," +
							 * myFlows.peakAutoSkims.getDistance(exchangeID, tazUserID));
							 * writer.write("," + myFlows.peakAutoSkims.getTime(exchangeID,
							 * tazUserID)); }
							 */
							writer.write("\n");
						}
							 ...
							 * if (myFlows.timeAndDistanceUtilities && myFlows.peakAutoSkims
							 * != null) { writer.write("," +
							 * myFlows.peakAutoSkims.getDistance(tazUserID, exchangeID));
							 * writer.write("," + myFlows.peakAutoSkims.getTime(tazUserID,
							 * exchangeID)); }
							 */
							writer.write("\n");
	...
	public void writeFlowTextFiles() {
		try {
			final BufferedWriter consumptionFlows = new BufferedWriter(
					new FileWriter(getOutputPath() + "FlowsFromConsumption(buying).csv"));
	 ...
	 * inputs we specify them in aa.datasource. If we are using CSV inputs we look
	 * in aa.base.data.
	 * 
	...
	/**
	 * Gets a TableDataSetCollection which will read from inputSource and write to
	 * outputDirectory if aa.useSQLInputs is set to true in the properties file
	 * then inputSource must be a property which defines the name of a defined
	 * jdbc datasource defined in the OS or a database. If aa.useSQLInputs is set
	 * to false (the default value) then inputSource is a property which defines
	 * the name of a directory containing inputs csv files.
	 * 
	 * @param inputSource
	 *          the name of a property in the aa.properties file which defines the
	 *          inputs source (directory or jdbc database)
	 * @param outputSource
	 *          the name of a property in the aa.properties file which defines the
	 *          output directory
	 ...
	 */
	protected TableDataSetCollection getTableDataSetCollection(
			String inputSource, String outputSource) {
		...
		if (useSQLInputs) {
			...
			final String jdbcDriver = ResourceUtil.checkAndGetProperty(aaRb,
					"aa.jdbcDriver");
			final JDBCConnection jdbcConnection = new JDBCConnection(datasourceName,
					jdbcDriver, "", "");
			final JDBCTableReader jdbcTableReader = new JDBCTableReader(
					jdbcConnection);
			final boolean excelInputs = ResourceUtil.getBooleanProperty(aaRb,
		...
		if (alphaZoneColumn != -1) {
			for (int row = 1; row <= activityConstraintsTable.getRowCount(); row++) {
				...
				final float quantity = activityConstraintsTable.getValueAt(row,
						quantityColumn);
				final String activityName = activityConstraintsTable.getStringValueAt(
						row, activityColumn);
				final ProductionActivity a = ProductionActivity
						.retrieveProductionActivity(activityName);
				if (a == null) {
				...
				if (constraintSet == null) {
					...
					alphaZoneActivityConstraintsInventory
							.put(activityName, constraintSet);
				}
			...
			logger
					.info("No TAZ or AZone Column in ActivityConstraintsI, now checking for LUZ or BZone");
			...
			for (int row = 1; row <= activityConstraintsTable.getRowCount(); row++) {
				final int betaZone = (int) activityConstraintsTable.getValueAt(row,
						betaZoneColumn);
				final float quantity = activityConstraintsTable.getValueAt(row,
						quantityColumn);
				final String activityName = activityConstraintsTable.getStringValueAt(
