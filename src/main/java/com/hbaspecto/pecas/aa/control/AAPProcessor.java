...
public abstract class AAPProcessor {
	...
	public void setUpAA(IResource resourceUtil) {

		setUpZones(resourceUtil); // read in PECASZonesI.csv and creates a
									// 'Zone' table data
		// set
		logger.info("Setting up commmodities");
		final String[] skimNames = setUpCommodities(resourceUtil); // read in
		// CommoditiesI.csv and
		// initalize an array of
		// commodities
		readFloorspaceZones(resourceUtil);
		setUpTransportConditions(skimNames, resourceUtil); // read in
															// betapktime.zip
															// and
		// betapkdist.zip
		setUpProductionActivities(resourceUtil); // read in ActivitiesI.csv and
													// initalize an
		// array of aggregate activities
		setUpExchangesAndZUtilities(resourceUtil); // read in
													// ExchangeImportExportI.csv,
		// create an 'Exchanges'
		// table data set and link the exchanges to commodities.
		setExchangePrices(resourceUtil);
		setUpMakeAndUse(resourceUtil); // read in MakeUseI.csv and create a
										// 'MakeUse' table
		// data set.
		// Or in case of AASetupWithTechnologySubstitution
		// class, it reads TechnologyOptionsI.csv
		checkTechnologyNestingCoefficients(resourceUtil);
		final double[][] floorspaceInventoryByLUZ = readFloorspace(resourceUtil);
		if (resourceUtil.getBooleanProperty(aaRb,
			...
			recalcActivitySizeTerms(floorspaceInventoryByLUZ, resourceUtil);
		} else {
			...
			recalcFloorspaceBuyingSizeTerms(resourceUtil);
		}
		recalcFloorspaceImport(resourceUtil);

	}

	protected abstract double[][] readFloorspace(IResource resourceUtil);

	protected void setUpZones(IResource resourceUtil) {
		logger.info("Setting up Zones");
		final TableDataSet ztab = loadTableDataSet("PECASZonesI",
				"aa.reference.data", resourceUtil);
	...
	protected abstract void setUpProductionActivities(IResource resourceUtil);
	 ...
	 */
	protected String[] setUpCommodities(IResource resourceUtil) {
		final ArrayList skimNames = new ArrayList();
		logger.info("Setting up Commodities");
		final TableDataSet ctab = loadTableDataSet("CommoditiesI",
				"aa.base.data", resourceUtil);
		final int nameColumn = ctab.checkColumnPosition("Commodity");
		final int bdpColumn = ctab
				.checkColumnPosition("BuyingDispersionParameter");
		final int sdpColumn = ctab
				.checkColumnPosition("SellingDispersionParameter");
		final int bscColumn = ctab.checkColumnPosition("BuyingSizeCoefficient");
		final int bpcColumn = ctab
				.checkColumnPosition("BuyingPriceCoefficient");
		final int btcColumn = ctab
				.checkColumnPosition("BuyingTransportCoefficient");
		final int sscColumn = ctab
				.checkColumnPosition("SellingSizeCoefficient");
		final int spcColumn = ctab
				.checkColumnPosition("SellingPriceCoefficient");
		final int stcColumn = ctab
				.checkColumnPosition("SellingTransportCoefficient");
		final int exchangeTypeColumn = ctab.checkColumnPosition("ExchangeType");
		final int floorspaceTypeC = ctab
				.checkColumnPosition("FloorspaceCommodity");
		...
		if (manualSizeTermsC <= 0) {
			if (resourceUtil.getBooleanProperty(aaRb, "calculateExchangeSizes",
					false)) {
		...
		final int[] overRideExternalZones = PECASZone
				.getOverrideExternalZones();
		for (int row = 1; row <= ctab.getRowCount(); row++) {
			final String commodityName = ctab.getStringValueAt(row, nameColumn);
			final float defaultBuyingDispersionParameter = ctab.getValueAt(row,
					bdpColumn);
			final float defaultSellingDispersionParameter = ctab.getValueAt(
					row, sdpColumn);
			final float buyingSizeCoefficient = ctab.getValueAt(row, bscColumn);
			final float buyingPriceCoefficient = ctab
					.getValueAt(row, bpcColumn);
			final float buyingTransportCoefficient = ctab.getValueAt(row,
					btcColumn);
			final float sellingSizeCoefficient = ctab
					.getValueAt(row, sscColumn);
			final float sellingPriceCoefficient = ctab.getValueAt(row,
					spcColumn);
			final float sellingTransportCoefficient = ctab.getValueAt(row,
					stcColumn);
			final boolean isFloorspace = ctab.getBooleanValueAt(row,
					floorspaceTypeC);
			final Commodity c = Commodity.createOrRetrieveCommodity(
					commodityName,
					ctab.getStringValueAt(row, exchangeTypeColumn).charAt(0));
			...
			c.setBuyingUtilityCoefficients(buyingSizeCoefficient,
					buyingPriceCoefficient, buyingTransportCoefficient);
			c.setSellingUtilityCoefficients(sellingSizeCoefficient,
					sellingPriceCoefficient, sellingTransportCoefficient);
			...
			if (overRideExternalZones == null) {
				...
				lfoss = new LinearSkimFunctionEEOverride(overRideExternalZones,
						-Double.MAX_VALUE);
			}
			int skimNumber = 1;
			boolean found = true;
			while (found) {
				final int coeffColumnNumber = ctab
						.getColumnPosition("InterchangeCoefficient"
								+ skimNumber);
				final int nameColumnNumber = ctab
						.getColumnPosition("InterchangeName" + skimNumber);
				if (skimNumber == 1
						&& (coeffColumnNumber == -1 || nameColumnNumber == -1)) {
				...
				if (coeffColumnNumber == -1 || nameColumnNumber == -1) {
					...
					final String skimName = ctab.getStringValueAt(row,
							nameColumnNumber);
					if (skimName != null) {
						if (skimName.length() != 0
								&& skimName.trim().length() != 0
								&& !skimName.equalsIgnoreCase("none")) {
							lfoss.addSkim(skimName,
									ctab.getValueAt(row, coeffColumnNumber));
			...
			c.compositeMeritMeasureWeighting = ctab.getValueAt(row,
					"GOFWeighting");
			c.setExpectedPrice(ctab.getValueAt(row, expectedPriceC));
			if (manualSizeTermsC > 0) {
				c.setManualSizeTerms(ctab.getBooleanValueAt(row,
						manualSizeTermsC));
			}
			if (searchC > 0) {
	...
	protected void readFloorspaceZones(IResource resourceUtil) {
		logger.info("Reading Floorspace Zones");
		floorspaceZoneCrossref = new HashtableAlphaToBeta();
		if (resourceUtil.getProperty(aaRb, "aa.useFloorspaceZones")
				.equalsIgnoreCase("true")) {
			final TableDataSet alphaZoneTable = loadTableDataSet(
					"FloorspaceZonesI", "aa.reference.data", resourceUtil);
			...
			for (int zRow = 1; zRow <= alphaZoneTable.getRowCount(); zRow++) {
				final Integer floorspaceZone = new Integer(
						(int) alphaZoneTable.getValueAt(zRow, tazColumn));
				final int pecasZoneInt = (int) alphaZoneTable.getValueAt(zRow,
						luzColumn);
				final AbstractZone pecasZone = AbstractZone
						.findZoneByUserNumber(pecasZoneInt);
				if (pecasZone != null) {
					...
					floorspaceZoneCrossref
							.put(floorspaceZone, pecasZoneInteger);
				} else {
					logger.warn("Bad  LUZ number " + pecasZoneInt
							+ " in FloorspaceZonesI ... ignoring TAZ "
							+ floorspaceZone.intValue());
				}
			}
		} else {
			logger.info("Not using floorspace zones (TAZs) -- TAZ are the same as LUZ");
			for (int z = 0; z < zones.length; z++) {
				final Integer zoneNumber = new Integer(
	...
	protected void setExchangePrices(IResource resourceUtil) {
		...
		TableDataSet initialPrices = loadTableDataSet("ExchangeResultsI",
				"aa.current.data", false, resourceUtil);
		if (initialPrices == null) {
			logger.info("No special price data for current year, check for previous year prices");
			initialPrices = loadTableDataSet("ExchangeResults",
					"aa.previous.data", false, resourceUtil);
			if (initialPrices == null) {
		...
		final int commodityNameColumn = initialPrices
				.checkColumnPosition("Commodity");
		final int priceColumn = initialPrices.checkColumnPosition("Price");
		final int zoneNumberColumn = initialPrices
				.checkColumnPosition("ZoneNumber");
		final int fixedPriceColumn = initialPrices
				.getColumnPosition("FixedPrice");
		...
		for (int row = 1; row <= initialPrices.getRowCount(); row++) {
			final String cname = initialPrices.getStringValueAt(row,
					commodityNameColumn);
			...
			if (com == null) {
				...
				final int zoneUserNumber = (int) initialPrices.getValueAt(row,
						zoneNumberColumn);
				final AbstractZone t = AbstractZone
						.findZoneByUserNumber(zoneUserNumber);
				if (t == null) {
					...
					if (x == null) {
						...
						if (fixedPriceColumn >= 0) {
							if (initialPrices.getBooleanValueAt(row,
									fixedPriceColumn)) {
								x.setDoSearch(false);
							}
						}
	...
	protected void setUpTransportConditions(String[] skimNames,
		...
		final String skimFormat = resourceUtil.getProperty(aaRb,
				"Model.skimFormat");
		...
		if (skimFormat.equalsIgnoreCase("TableDataSet")) {
			...
			final String skimFileName = resourceUtil.getProperty(aaRb,
					"skim.filename");
			final TableDataSet s = readSkimFile(path1, path2, skimColumns,
					skimFileName);
			someSkims = new SomeSkims();
			someSkims.addTableDataSetSkims(s, skimNames,
					AbstractZone.maxZoneNumber);
	...
	public TableDataSet readSkimFile(String path1, String path2,
		...
		try {
			// read from file
			s = reader.readFile(new File(path1 + skimFileName), columnFilter,
					skimColumns);
		...
		if (s == null) {
			try {
				// read from URL
				s = reader.readFile(path1 + skimFileName, columnFilter,
						skimColumns);
		...
		if (s == null) {
			try {
				// read from file
				s = reader.readFile(new File(path2 + skimFileName),
						columnFilter, skimColumns);
		...
		if (s == null) {
			try {
				// read from URL
				s = reader.readFile(path2 + skimFileName, columnFilter,
						skimColumns);
		...
		if (s == null) {
			...
			throw new RuntimeException(
					"Error loading in skims " + skimFileName, e);
		}
		return s;
	}

	protected abstract void setUpMakeAndUse(IResource resourceUtil);

	private void checkTechnologyNestingCoefficients(IResource resourceUtil) {
		...
		final String[] activityNames = new String[ProductionActivity
				.getAllProductionActivities().size()];
		final Iterator it = ProductionActivity.getAllProductionActivities()
				.iterator();
		int a = 0;
		while (it.hasNext()) {
		...
		final Iterator activityIterator = ProductionActivity
		...
		while (activityIterator.hasNext()) {
			...
			conString = "";
			prodString = "";
			final AggregateActivity aa = (AggregateActivity) activityIterator
					.next();
			...
			if (aa.getConsumptionFunction() instanceof LogitTechnologyChoiceConsumptionFunction) {
				...
				techChoice.checkAndLogRatios(aa.name,
						maxErrorTermSizeProduction,
		...
		getTableDataSetCollection(resourceUtil).addTableDataSet(
				maxErrorTermSizeConsumption);
		getTableDataSetCollection(resourceUtil).addTableDataSet(
				maxErrorTermSizeProduction);
		getTableDataSetCollection(resourceUtil).flushAndForget(
				maxErrorTermSizeConsumption);
		getTableDataSetCollection(resourceUtil).flushAndForget(
				maxErrorTermSizeProduction);
	}
	...
	private ExcelFileReader getExcelFileReader(IResource resourceUtil) {
		if (excelFileReader != null) {
			return excelFileReader;
		}
		excelFileReader = new ExcelFileReader();
		excelFileReader.setWorkbookFile(new File(resourceUtil
				.checkAndGetProperty(aaRb, "aa.datasource")));
		return excelFileReader;
	}

	protected JDBCTableReader getJDBCTableReader(IResource resourceUtil) {
		final JDBCTableReader jdbcTableReader = new JDBCTableReader(
				getJDBCConnection(resourceUtil));
		final boolean excelInputs = resourceUtil.getBooleanProperty(aaRb,
	...
	private JDBCConnection getJDBCConnection(IResource resourceUtil) {
		JDBCConnection jdbcConnection = null;
		final String datasourceUrl = resourceUtil.getProperty(aaRb,
				"aa.jdbcUrl");
		if (datasourceUrl != null) {
			try {
				jdbcConnection = (JDBCConnection) DriverManager
						.getConnection(datasourceUrl);
			} catch (final SQLException e) {
				logger.fatal("Cannot connect to JDBC connection "
						+ datasourceUrl);
				throw new RuntimeException("Cannot connect to JDBC connection "
						+ datasourceUrl, e);
			}
		} else {
			final String datasourceName = resourceUtil.checkAndGetProperty(
					aaRb, "aa.datasource");
			final String jdbcDriver = resourceUtil.checkAndGetProperty(aaRb,
					"aa.jdbcDriver");
	...
	private void setFloorspaceProportionsForTechnologySizeTerms(
		...
		for (final ProductionActivity a : ProductionActivity
	...
	protected void recalcActivitySizeTerms(double[][] floorspaceInventoryByLUZ,
		...
		final TableDataSet activitySizeTermsCalculation = loadTableDataSet(
				"ActivitySizeTermsI", "aa.base.data", false, resourceUtil);
		...
		for (int row = 1; row <= activitySizeTermsCalculation.getRowCount(); row++) {
			final String activityName = activitySizeTermsCalculation
					.getStringValueAt(row, "Activity");
			...
			if (a == null) {
				logger.error("Bad production activity in zone size term calculation: "
						+ activityName);
				throw new Error(
						"Bad production activity in zone size term calculation: "
								+ activityName);
			}
			...
			final double weight = activitySizeTermsCalculation.getValueAt(row,
					"Weight");
			final String commodityName = activitySizeTermsCalculation
					.getStringValueAt(row, "Floorspace");
			final Commodity c = Commodity.retrieveCommodity(commodityName);
			if (c == null) {
				logger.error("Bad commodity name in zone size term calculation: "
						+ commodityName);
				throw new Error(
						"Bad commodity name in zone size term calculation: "
								+ commodityName);
			}
     ...
     * 
     */
	protected void recalcFloorspaceBuyingSizeTerms(IResource resourceUtil) {
		logger.info("Reading Floorspace Buying Size Terms");
		TableDataSet floorspaceSizeTermsCalculation = null;
		try {
			floorspaceSizeTermsCalculation = loadTableDataSet(
					"FloorspaceBuyingSizeTermsI", "aa.base.data", false,
					resourceUtil);
		} catch (final RuntimeException e) {
			logger.fatal("Exception loading floorspace buying size terms " + e);
			throw new RuntimeException(
					"Exception loading floorspace buying size terms", e);
		}
		...
		final int scaleColumn = floorspaceSizeTermsCalculation
				.getColumnPosition("Scale");
		if (scaleColumn == -1) {
		...
		for (int row = 1; row <= floorspaceSizeTermsCalculation.getRowCount(); row++) {
			final String groupName = floorspaceSizeTermsCalculation
					.getStringValueAt(row, "FloorspaceGroup");
			if (!floorspaceGroups.containsKey(groupName)) {
				logger.info("Setting up floorspace group " + groupName);
				final ZoneQuantityStorage inv = new ZoneQuantityStorage(
						groupName);
		...
		for (int row = 1; row <= floorspaceSizeTermsCalculation.getRowCount(); row++) {
			final String groupName = floorspaceSizeTermsCalculation
					.getStringValueAt(row, "FloorspaceGroup");
			final ZoneQuantityStorage group = (ZoneQuantityStorage) floorspaceGroups
					.get(groupName);
			final String typeName = floorspaceSizeTermsCalculation
					.getStringValueAt(row, "FloorspaceType");
			float typeScale = (float) 1.0;
			if (scaleColumn != -1) {
				typeScale = floorspaceSizeTermsCalculation.getValueAt(row,
						scaleColumn);
			}
			...
			if (type == null) {
				...
				throw new RuntimeException("invalid floorspace type "
						+ typeName + " in FloorspaceBuyingSizeTermsI");
			}
			final Iterator typeIt = type.getInventoryMap().entrySet()
					.iterator();
			while (typeIt.hasNext()) {
				...
				group.increaseQuantityForZone(alphaZone.intValue(),
						((Double) typeEntry.getValue()).doubleValue()
								* typeScale);
			}
		...
		for (int row = 1; row <= floorspaceSizeTermsCalculation.getRowCount(); row++) {
			...
			for (int betaZone = 0; betaZone < zones.length; betaZone++) {
				...
				while (en.hasNext()) {
					...
					if (b != null) {
						if (b.intValue() == betaZoneUserNumber) {
							groupQuantity += group.getQuantityForZone(a
									.intValue());
							typeQuantity += type.getQuantityForZone(a
									.intValue()) * typeScale;
							if (Double.isNaN(groupQuantity)
									|| Double.isNaN(typeQuantity)) {
								throw new RuntimeException(
				...
				if (!(groupName.equalsIgnoreCase("none") || groupName
						.equals(""))) {
     ...
     * 
     */
	protected void recalcFloorspaceImport(IResource resourceUtil) {
		logger.info("Calculating floorspace supply functions");
		// boolean oldWay = false;
		final String deltaString = resourceUtil.getProperty(aaRb,
				"aa.floorspaceDelta");
		double delta = 1;
		double eta = 1;
		double p0 = 0;
		...
		final Iterator<ZoneQuantityStorage> it = floorspaceInventory.values()
				.iterator();
		while (it.hasNext()) {
			final ZoneQuantityStorage fqs = it.next();
			logger.info("Building floorspace import functions for "
					+ fqs.typeName);
			final String[][] floorspaceNames = new String[1][1];
			floorspaceNames[0][0] = fqs.typeName;
			final TableDataSetCollection collection = getTableDataSetCollection(resourceUtil);
			final TableDataSetIndexedValue floorspaceParam = new TableDataSetIndexedValue(
					"FloorspaceSupplyI", new String[] { "Commodity" },
					new String[0], floorspaceNames, new int[1][0],
					"SupplyFunctionMidpointFactor");
			...
			try {
				...
				if (oversupplyFunction) {
					logger.info("Setting up oversupply function for "
							+ fqs.typeName);
				} else {
					logger.info("No oversupply functions for " + fqs.typeName);
				}
			} catch (final RuntimeException e) {
				logger.info("No oversupply function for floorspace type "
						+ fqs.typeName + ", value missing", e);
				oversupplyFunction = false;
			}
			for (int bz = 0; bz < zones.length; bz++) {
				...
				final Iterator anotherIterator = fqs.getInventoryMap()
						.entrySet().iterator();
				while (anotherIterator.hasNext()) {
					...
					if (betaZoneInteger != null) {
						if (betaZoneInteger.intValue() == betaZoneNumber) {
							quantityExpected += ((Double) entry.getValue())
									.doubleValue();
						}
					}
				}
				...
				final LogisticPlusLinearFunction baseFunction = new LogisticPlusLinearFunction(
						quantityExpected * midpoint, p0, eta, quantityExpected
								* delta, quantityExpected * slope);
				if (oversupplyFunction) {
					x.setImportFunction(new LogisticPlusLinearWithOverrideFunction(
							baseFunction, oversupplyPrice, oversupplyAmount,
							oversupplyExponent));
				} else {
					x.setImportFunction(baseFunction);
				}
				x.setExportFunction(new LogisticPlusLinearFunction(0,
						zeroPrice, 0, 0, exportSlope));
	...
	public void writeOutputs(IResource resourceUtil) {
		...
		if (resourceUtil.getBooleanProperty(aaRb, "aa.writeFlowMatrices", true)) {
			...
			writeAllFlowZipMatrices(resourceUtil); // writes out a
													// 'buying_commodityName'
			...
			writeAllHistograms(resourceUtil);
		}
	...
	protected abstract void writeTechnologyChoice(IResource resourceUtil);

	public void writeTripMatrices(StringIndexedNDimensionalMatrix aZoneMake,
			StringIndexedNDimensionalMatrix aZoneUse, IResource resourceUtil) {
		if (resourceUtil.getBooleanProperty(aaRb, "aa.makeTripMatrices", false)) {
			...
			final double useRoundedAt = resourceUtil.getDoubleProperty(aaRb,
					"aa.roundExpectedValueForTripsIfHigherThan",
					Double.POSITIVE_INFINITY);
			final double useNormalAt = resourceUtil.getDoubleProperty(aaRb,
					"aa.useNormalPoissonApproximationIfHigherThan", 10.0);

			final TableDataSet tripCalculationSpecification = loadTableDataSet(
					"TripCalculationsI", "aa.base.data", resourceUtil);
			final boolean usePoissonDistribution = resourceUtil
					.getBooleanProperty(aaRb, "aa.poissonTrips", true);
			final TripCalculator myTripCalculator = new TripCalculator(
					useNormalAt, useRoundedAt);
			Matrix[] tripArrays = myTripCalculator
					.calculateLuzTrips(tripCalculationSpecification);
			...
			if (resourceUtil.getBooleanProperty(aaRb, "aa.TAZTripList", false)) {
				if (aZoneMake == null || aZoneUse == null) {
					...
					final SparseMatrix[] tazTrips = myTripCalculator
							.sampleTazTrips(tripArrays, floorspaceZoneCrossref,
									tripCalculationSpecification, aZoneMake,
									aZoneUse, maxAlphaZone);
					if (resourceUtil.getBooleanProperty(aaRb,
							"aa.writeEmme2FormatTazTrips", true)) {
						for (int i = 0; i < tazTrips.length; i++) {
							final int alphaCount = floorspaceZoneCrossref
									.getAlphaExternals0Based().length;
							final Matrix m = new Matrix(alphaCount, alphaCount);
							m.setExternalNumbers(floorspaceZoneCrossref
									.getAlphaExternals1Based());
							for (final MatrixEntry e : tazTrips[i]) {
								m.setValueAt(e.row(), e.column(),
										(float) e.get());
							}
							final Emme2311MatrixWriter writer = new Emme2311MatrixWriter(
									new File(getOutputPath(resourceUtil)
											+ "TAZxTAZ"
					...
					try {
						final BufferedWriter tazTripFile = new BufferedWriter(
								new FileWriter(getOutputPath(resourceUtil)
										+ "tazTrips.csv"));
			...
			final MatrixWriter writer = new CSVMatrixWriter(new File(
					getOutputPath(resourceUtil) + "LUZTrips.csv"));
			writer.writeMatrices(myTripCalculator.getTripArrayNames(),
					tripArrays);
	 ...
	 */
	public void writeZonalMakeUseCoefficients(IResource resourceUtil,
			PECASZone[] allZones, Collection<ProductionActivity> collection,
			Collection<AbstractCommodity> list) {
		final boolean ascii = resourceUtil.getBooleanProperty(aaRb,
				"aa.writeAsciiZonalMakeUse", true);
		final boolean binary = resourceUtil.getBooleanProperty(aaRb,
				"aa.writeBinaryZonalMakeUse", false);
		final boolean stringsInZonalMakeUse = resourceUtil.getBooleanProperty(
				aaRb, "aa.stringsInZonalMakeUse", false);
		if (!stringsInZonalMakeUse) {
			writeCommodityAndActivityFiles(resourceUtil);
		}
		...
		final String[] columnNames = { "Activity", "ZoneNumber", "Commodity",
				"MorU" };
		shape[3] = 2; // "M" or "U"
		...
		zonalMakeUseCoefficients = new StringIndexedNDimensionalMatrix(
				"Coefficient", 4, shape, columnNames);
		zonalMakeUseCoefficients.setAddKeysOnTheFly(true);
		if (binary) {
			utilities = new StringIndexedNDimensionalMatrix("Utility", 4,
					shape, columnNames);
			utilities.setAddKeysOnTheFly(true);
			quantities = new StringIndexedNDimensionalMatrix("Amount", 4,
					shape, columnNames);
		...
		try {
			if (ascii) {
				zMakeUseFile = GetZonalMakeUseBufferedWriter(getOutputPath(resourceUtil));
				zMakeUseFile
						.write("Activity,ZoneNumber,Commodity,MorU,Coefficient,Utility,Amount\n");
			}
			final BufferedWriter aggregateMakeUseFile = GetAggregateMakeUseBufferedWriter(getOutputPath(resourceUtil));
			aggregateMakeUseFile
					.write("Activity,Commodity,MorU,Coefficient,StdDev,Amount\n");
			final Iterator it = ProductionActivity.getAllProductionActivities()
					.iterator();
			while (it.hasNext()) {
				...
				final double[] madeAmounts = new double[AbstractCommodity
				...
				while (commodityIterator.hasNext()) {
					...
					final double madeSD = Math.sqrt(madeQ[c.commodityNumber]
							/ nonZeroZones);
					final double usedSD = Math.sqrt(usedQ[c.commodityNumber]
							/ nonZeroZones);
					if (madeSD > 0 || madeA[c.commodityNumber] != 0) {
						aggregateMakeUseFile.write(p.name + "," + c.name
								+ ",M," + madeA[c.commodityNumber] + ","
								+ madeSD + "," + madeAmounts[c.commodityNumber]
								+ "\n");
					}
					if (usedSD > 0 || usedA[c.commodityNumber] != 0) {
						aggregateMakeUseFile.write(p.name + "," + c.name
								+ ",U," + usedA[c.commodityNumber] + ","
								+ usedSD + "," + usedAmounts[c.commodityNumber]
								+ "\n");
					}
				}
		...
		if (binary) {
			final String filename = getOutputPath(resourceUtil)
					+ "ZonalMakeUse.bin";
			if (filename != null) {
				try {
					final java.io.ObjectOutputStream out = GetZonalMakeUseObjectOutputStream(filename);
					...
					logger.fatal("Can't write out zonal make use binary file "
							+ e);
				}
			}
		}
	...
	private void writeCommodityAndActivityFiles(IResource resourceUtil) {
		try {
			final BufferedWriter activitiesFile = GetActivitiesBufferedWriter(getOutputPath(resourceUtil));
			activitiesFile.write("ActivityNumber,Activity\n");
			Iterator it = ProductionActivity.getAllProductionActivities()
					.iterator();
			while (it.hasNext()) {
				final ProductionActivity p = (ProductionActivity) it.next();
			...
			final BufferedWriter commoditiesFile = GetCommodityBufferedWriter(getOutputPath(resourceUtil));
			...
			logger.warn("Can't write ActivityNumbers.csv and CommodityNumbers.csv, "
					+ e);
		}
	}

	public void writeLocationTable(String tableName, IResource resourceUtil) {
		BufferedWriter locationsFile;
		try {
			locationsFile = new BufferedWriter(new FileWriter(
					getOutputPath(resourceUtil) + tableName + ".csv"));
			...
			while (it.hasNext()) {
				...
				for (int z = 0; z < p.getMyDistribution().length; z++) {
					try {
						...
						locationsFile.write(p.getMyDistribution()[z].getMyTaz()
								.getZoneUserNumber() + ",");
						locationsFile.write(p.getMyDistribution()[z]
								.getQuantity() + ",");
						p.getMyDistribution()[z]
								.writeLocationUtilityTerms(locationsFile);
					} catch (final IOException e) {
	...
	public void writeZUtilitiesTable(IResource resourceUtil) {
		BufferedWriter os = null;
		try {
			os = new BufferedWriter(new FileWriter(getOutputPath(resourceUtil)
					+ "CommodityZUtilities.csv"));
			final boolean writeComponents = resourceUtil.getBooleanProperty(
					aaRb, "aa.writeUtilityComponents");
			if (writeComponents) {
			...
			final Iterator it = AbstractCommodity.getAllCommodities()
					.iterator();
			while (it.hasNext()) {
				final Commodity c = (Commodity) it.next();
				for (int b = 0; b < 2; b++) {
					...
					while (it2.hasNext()) {
						...
						try {
							...
							if (writeComponents) {
								os.write(czu.getUtility(czu
										.getLastHigherLevelDispersionParameter())
										+ ",");
								final double[] components = czu
										.getUtilityComponents(czu
												.getLastHigherLevelDispersionParameter());
								for (int i = 0; i < 7; i++) {
	...
	public void writeExchangeResults(IResource resourceUtil) {
		...
		while (exchangeResults == null && i < 200) {
			try {
				i++;
				if (i == 0) {
					exchangeResults = new BufferedWriter(
							new FileWriter(getOutputPath(resourceUtil)
									+ "ExchangeResults.csv"));
				} else {
					exchangeResults = new BufferedWriter(new FileWriter(
							getOutputPath(resourceUtil) + "ExchangeResults-"
									+ i + ".csv"));
				}
				exchangeResults
		...
		try {
			exchangeResultsTotals = new BufferedWriter(new FileWriter(
					getOutputPath(resourceUtil) + "ExchangeResultsTotals.csv"));
		...
		if (exchangeResults != null) {
			...
			while (it.hasNext()) {
				...
				double price = 0;
				double msSurplus = 0;
				final Iterator exchangeIterator = c.getAllExchanges()
						.iterator();
				...
				while (exchangeIterator.hasNext()) {
					...
					try {
						exchangeResults.write(ex.myCommodity.name + ","
								+ ex.exchangeLocationUserID + ",");
						final double surplus = ex.exchangeSurplus();
						final double[] importExport = ex.importsAndExports(ex
								.getPrice());
						exchangeResults.write(ex.boughtTotal()
								+ importExport[1] + "," + ex.boughtTotal()
								+ "," + importExport[1] + ","
								+ (ex.soldTotal() + importExport[0]) + ","
								+ ex.soldTotal() + "," + importExport[0] + ","
								+ surplus + "," + ex.getPrice());
						if (writeSizeTerms) {
						...
						if (writeDerivatives) {
							exchangeResults
									.write("," + ex.exchangeDerivative());
						}
						if (ex.getImportFunction() instanceof LogisticPlusLinearWithOverrideFunction) {
							final BufferedWriter overrideFile = getOverrideFile(resourceUtil);
							if (overrideFile != null) {
								((LogisticPlusLinearWithOverrideFunction) ex
										.getImportFunction())
										.writeOverride(overrideFile,
												ex.exchangeLocationUserID,
												ex.myCommodity.getName(),
												ex.getPrice());
							}
							((LogisticPlusLinearWithOverrideFunction) ex
									.getImportFunction()).logOverrides(
									ex.getPrice(), ex.toString());
						}
						exchangeResults.write("\n");
						...
						logger.error("Error adding exchange " + this
								+ " to table");
						e.printStackTrace();
					}
				...
				if (exchangeResultsTotals != null) {
					try {
						exchangeResultsTotals.write(c.name + ","
								+ (internalBought + exports) + ","
								+ internalBought + "," + exports + ","
								+ (internalSold + imports) + "," + internalSold
								+ "," + imports + "," + Math.sqrt(msSurplus)
								+ "," + price + "\n");
					} catch (final IOException e) {
	...
	private BufferedWriter getOverrideFile(IResource resourceUtil) {
		if (overRideFile == null) {
			try {
				overRideFile = new BufferedWriter(
						new FileWriter(getOutputPath(resourceUtil)
								+ "FloorspaceOverrides.csv"));
				overRideFile.write(LogisticPlusLinearWithOverrideFunction
						.getHeader() + "\n");
			} catch (final IOException e) {
	...
	public void writeExchangeResults(String commodityName,
		...
		final Commodity c = Commodity.retrieveCommodity(commodityName);
		try {
			final BufferedWriter exchangeResults = new BufferedWriter(
					new FileWriter(getOutputPath(resourceUtil) + commodityName
							+ "_ExchangeResults.csv"));
			exchangeResults
					.write("Commodity,ZoneNumber,Demand,InternalBought,Exports,Supply,InternalSold,Imports,Surplus,Price\n");
			...
			while (exchangeIterator.hasNext()) {
				...
				try {
					...
					final double[] importExport = ex.importsAndExports(ex
							.getPrice());
					exchangeResults
							.write(ex.boughtTotal() + importExport[1] + ","
			...
			logger.fatal("Can't create exchange results output file for commodity "
					+ commodityName);
			e.printStackTrace();
		}
	}

	public void writeActivitySummary(IResource resourceUtil) {
		BufferedWriter activityFile;
		try {
			activityFile = new BufferedWriter(new FileWriter(
					getOutputPath(resourceUtil) + "ActivitySummary.csv"));
			// ENHANCEMENT shall we write out the dollars spent and earned as
			// well as the utility associated with size/variation/transport
			// components?
	...
	public String[] readInHistogramSpecifications(IResource resourceUtil) {
		final ArrayList newSkimNames = new ArrayList();
		final TableDataSet histogramSpecTable = loadTableDataSet("HistogramsI",
				"aa.base.data", false, resourceUtil);
		...
		for (int row = 1; row <= histogramSpecTable.getRowCount(); row++) {
			...
			hspec.commodityName = histogramSpecTable.getStringValueAt(row,
					"Commodity");
			hspec.categorizationSkim = histogramSpecTable.getStringValueAt(row,
					"Skim");
			...
			for (int i = 0; i < histogramSpecTable.getColumnCount() - 2; i++) {
				final int column = histogramSpecTable
						.getColumnPosition("C" + i);
				if (column > 0) {
					final float boundary = histogramSpecTable.getValueAt(row,
							column);
	...
	public void writeAllFlowZipMatrices(IResource resourceUtil) {
		// check for property in aa.properties - should start with a '.' in the
		// file.
		if (zipExtension == null) {
			zipExtension = resourceUtil.getProperty(aaRb, "zip.extension",
					".zipMatrix");
		}
		try {
			final BufferedWriter histogramFile = new BufferedWriter(
					new FileWriter(getOutputPath(resourceUtil)
							+ "Histograms.csv"));
			histogramFile
					.write("Commodity,BuyingSelling,BandNumber,LowerBound,Quantity,AverageLength\n");
			final PrintWriter pctFile = new PrintWriter(new BufferedWriter(
					new FileWriter(getOutputPath(resourceUtil)
							+ "PctIntrazonalxCommodityxBzone.csv")));
			...
			final Iterator com = AbstractCommodity.getAllCommodities()
					.iterator();
			while (com.hasNext()) {
				writeFlowZipMatrices(((Commodity) com.next()).getName(),
						histogramFile, pctFile, resourceUtil);
			}
	...
	public void writeFlowZipMatrices(String name, Writer histogramFile,
			PrintWriter pctFile, IResource resourceUtil) {
		final Commodity com = Commodity.retrieveCommodity(name);

		final Matrix b = com.getBuyingFlowMatrix();
		if (b != null) {
			final ZipMatrixWriter zmw = new ZipMatrixWriter(new File(
					getOutputPath(resourceUtil) + "buying_" + com.name
							+ zipExtension));
			zmw.writeMatrix(b);
		}
		...
		if (s != null) {
			final ZipMatrixWriter zmw = new ZipMatrixWriter(new File(
					getOutputPath(resourceUtil) + "selling_" + com.name
							+ zipExtension));
			zmw.writeMatrix(s);
		}
		if (logger.isDebugEnabled()) {
			logger.debug("Buying and Selling Commodity Flow Matrices have been written for "
					+ name);
		}

		// write intrazonal numbers to calculate percentages
		if (b != null && s != null) {
			writePctIntrazonalFile(pctFile, name, b, s, resourceUtil);
		}

		if (b != null && s != null) {
			writeFlowHistograms(histogramFile, name, b, s, resourceUtil);
		}
	}

	protected void writePctIntrazonalFile(PrintWriter writer, String name,
			Matrix b, Matrix s, IResource resourceUtil) {
		...
		try {
			...
			if (writer == null) {
				writer = new PrintWriter(new BufferedWriter(new FileWriter(
						getOutputPath(resourceUtil) + "PctIntrazonalxBetazone_"
								+ name + ".csv")));
	...
	public void writeFlowHistograms(String name, Writer stream,
		...
		if (someHistogramsForThisCommodity) {
			...
			final Matrix s = com.getSellingFlowMatrix();
			if (b != null & s != null) {
				writeFlowHistograms(stream, name, b, s, resourceUtil);
			}
		}
	}
	 ...
	 * @param sellingMatrix
	 */
	protected void writeFlowHistograms(Writer histogramFile,
			String commodityName, Matrix buyingMatrix, Matrix sellingMatrix,
			IResource resourceUtil) {
		...
		while (it.hasNext()) {
			...
			if (hspec.commodityName.equals(commodityName)) {
				...
				for (int bound = 0; bound < hspec.boundaries.size(); bound++) {
					boundaries[bound] = ((Float) hspec.boundaries.get(bound))
							.doubleValue();
				}
				final MatrixHistogram mhBuying = new MatrixHistogram(boundaries);
				final MatrixHistogram mhSelling = new MatrixHistogram(
						boundaries);
				Matrix skim = null;
				try {
					...
					logger.fatal("Can't find skim name "
							+ hspec.categorizationSkim
				...
				try {
					if (histogramFile == null) {
						histogramFile = new BufferedWriter(new FileWriter(
								getOutputPath(resourceUtil) + "Histograms_"
										+ commodityName + ".csv"));
						histogramFile
								.write("Commodity,BuyingSelling,BandNumber,LowerBound,Quantity,AverageLength\n");
						closeHistogramFile = true;
					}
					mhBuying.writeHistogram(commodityName, "buying",
							histogramFile);
					mhSelling.writeHistogram(commodityName, "selling",
							histogramFile);
				} catch (final IOException e) {
					logger.fatal("IO exception " + e
							+ " in writing out histogram file for " + this);
					logger.fatal(e);
					System.exit(1);
				}
			}
	...
	public void writeAllHistograms(IResource resourceUtil) {
		try {
	...
	public void writeFlowsToFile(BufferedWriter writer,
		...
		final int tazUserID = myFlows.theCommodityZUtility.myTaz
				.getZoneUserNumber();
		char selling = 's';
		...
		try {
			if (com.exchangeType == 'p' && selling == 's'
				...
				writer.write(tazUserID + ",");
				if (selling == 'b') {
					writer.write(String.valueOf(-x.getFlowQuantity(tazIndex,
							selling)));
				} else {
					writer.write(String.valueOf(x.getFlowQuantity(tazIndex,
							selling)));
				...
				synchronized (theExchanges) {
					...
					while (it.hasNext()) {
						...
						if (selling == 'b') {
							...
							writer.write(tazUserID + ",");
							writer.write(String.valueOf(-x.getFlowQuantity(
									tazIndex, selling)));
							/*
	...
	public void writeFlowTextFiles(IResource resourceUtil) {
		try {
			final BufferedWriter consumptionFlows = new BufferedWriter(
					new FileWriter(getOutputPath(resourceUtil)
							+ "FlowsFromConsumption(buying).csv"));
			final BufferedWriter productionFlows = new BufferedWriter(
					new FileWriter(getOutputPath(resourceUtil)
							+ "FlowsFromProduction(selling).csv"));
			consumptionFlows
					.write("commodity,origin,destination,quantity,distance,time\n");
			productionFlows
					.write("commodity,origin,destination,quantity,distance,time\n");
			// AbstractZone[] zones = AbstractZone.getAllZones();
			// for (int z = 0; z < zones.length; z++) {
			final Iterator cit = AbstractCommodity.getAllCommodities()
					.iterator();
			while (cit.hasNext()) {
	...
	public String getOutputPath(IResource resourceUtil) {
		if (outputPath == null) {
	 ...
	 */
	protected TableDataSetCollection getTableDataSetCollection(
			String inputSource, String outputSource, IResource resourceUtil) {
		TableDataSetCollection collection = collections.get(inputSource);
		if (collection != null) {
			return collection;
		}
		final boolean useSQLInputs = resourceUtil.getBooleanProperty(aaRb,
				"aa.useSQLInputs", false);
		final String outputDirectory = resourceUtil.checkAndGetProperty(aaRb,
				outputSource);
		...
		if (useSQLInputs) {
			...
			final String inputPath = resourceUtil.checkAndGetProperty(aaRb,
					inputSource);
		...
		writer.setMyDecimalFormat(new GeneralDecimalFormat("0.#########E0",
				10000000, .001));
	...
	public final TableDataSet loadTableDataSet(String tableName, String source,
			IResource resourceUtil) {
		return loadTableDataSet(tableName, source, true, resourceUtil);
	};
	...
	protected TableDataSet loadTableDataSet(String tableName, String source,
		...
		try {
			if (useSQLInputs) {
				table = getJDBCTableReader(resourceUtil).readTable(tableName);
				if (table == null) {
					final String msg = "Can't find table " + tableName
							+ " in JDBC table";
					if (check) {
						logger.fatal(msg);
						throw new RuntimeException(msg);
	...
	public void setupActivityConstraints(IResource resourceUtil) {
		readActivityConstraints(resourceUtil);
		checkActivityConstraints();
	}

	protected void readActivityConstraints(IResource resourceUtil) {
		logger.info("Reading ActivityConstraintsI File");
		if (maxAlphaZone == 0) {
			readFloorspaceZones(resourceUtil);
		}
		final TableDataSet activityConstraintsTable = loadTableDataSet(
				"ActivityConstraintsI", "aa.current.data", resourceUtil);
		...
		final int quantityColumn = activityConstraintsTable
				.checkColumnPosition("Quantity");
		final int activityColumn = activityConstraintsTable
				.checkColumnPosition("Activity");
		if (alphaZoneColumn != -1) {
			for (int row = 1; row <= activityConstraintsTable.getRowCount(); row++) {
				final int alphaZone = (int) activityConstraintsTable
						.getValueAt(row, alphaZoneColumn);
				final float quantity = activityConstraintsTable.getValueAt(row,
						quantityColumn);
				final String activityName = activityConstraintsTable
			...
			final Iterator alphaZoneConstraints = alphaZoneActivityConstraintsInventory
					.entrySet().iterator();
			while (alphaZoneConstraints.hasNext()) {
				final Map.Entry constraintEntry = (Entry) alphaZoneConstraints
						.next();
				...
				if (activity == null) {
					...
					throw new RuntimeException("Invalid constraint activity "
							+ activityName);
				}
				final ZoneQuantityStorage alphaConstraint = (ZoneQuantityStorage) constraintEntry
						.getValue();
				final Iterator alphaConstraintIterator = alphaConstraint
						.getInventoryMap().entrySet().iterator();
				while (alphaConstraintIterator.hasNext()) {
					final Map.Entry e = (Map.Entry) alphaConstraintIterator
							.next();
					final Integer alphaZone = (Integer) e.getKey();
					final int betaZone = floorspaceZoneCrossref
							.getBetaZone(alphaZone.intValue());
					// search for betazone using brute force
					for (int z = 0; z < activity.myDistribution.length; z++) {
						if (activity.myDistribution[z].myTaz
								.getZoneUserNumber() == betaZone) {
							activity.myDistribution[z].setConstrained(true);
							activity.myDistribution[z].constraintQuantity += ((Double) e
			...
			for (int row = 1; row <= activityConstraintsTable.getRowCount(); row++) {
				final int betaZone = (int) activityConstraintsTable.getValueAt(
						row, betaZoneColumn);
				final float quantity = activityConstraintsTable.getValueAt(row,
						quantityColumn);
				final String activityName = activityConstraintsTable
	...
	protected ObjectOutputStream GetZonalMakeUseObjectOutputStream(
		...
		final java.io.FileOutputStream fos = new java.io.FileOutputStream(
				filename);
		return new java.io.ObjectOutputStream(fos);
	}

	protected BufferedWriter GetAggregateMakeUseBufferedWriter(String outputPath)
			throws IOException {
		return new BufferedWriter(new FileWriter(outputPath + "MakeUse.csv"));
	}
	...
	protected double[] CalculateBuyingZUtilities(ConsumptionFunction cf,
		...
		for (int c = 0; c < cf.size(); c++) {
			...
			if (com == null) {
				...
				buyingZUtilities[c] = com.calcZUtility(zone, false);
			}
		} // CUSellc,z and CUBuyc,z have now been calculated for the
			// commodites made or used by the activity

		return buyingZUtilities;
	}
