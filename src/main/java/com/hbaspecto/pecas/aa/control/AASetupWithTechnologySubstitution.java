...
public class AASetupWithTechnologySubstitution extends AAPProcessor {
	...
	protected void setUpMakeAndUse() {
		...
		if (ResourceUtil
				.getBooleanProperty(aaRb, "aa.automaticTechnologySizeTerms")) {
			optionWeightColumnName = "OptionWeight";
			final int columnId = technologyOptions
					.getColumnPosition(optionWeightColumnName);
			if (columnId < 0) {
		...
		for (int techRow = 1; techRow <= technologyOptions.getRowCount(); techRow++) {
			...
			final String name = technologyOptions.getStringValueAt(techRow,
					"OptionName");
			final TechnologyOption option = new TechnologyOption(choice, 1
					/ choice.getDispersionParameter()
					* Math.log(technologyOptions.getValueAt(techRow,
							optionWeightColumnName)), name);
			...
			for (int column = 1; column <= technologyOptions.getColumnCount(); column++) {
				...
				if (!(columnName.equals("Activity") || columnName.equals("OptionName") || columnName
						...
						if (strings.length - 1 == numberShouldBeInField) {
							try {
								final int number = Integer
										.valueOf(strings[numberShouldBeInField]);
							}
					...
					final float amount = technologyOptions.getValueAt(techRow, column)
							* sign;
	...
	protected TableDataSet loadTechnologyOptionsTable() {
		final TableDataSet technologyOptions = loadTableDataSet(
				"TechnologyOptionsI", "aa.base.data");
	...
	protected void readActivitiesAndActivityZonalValues() {
		...
		zonalData = loadTableDataSet("ActivitiesZonalValuesW", "aa.current.data",
				false);

		if (zonalData == null) {
			zonalData = loadTableDataSet("ActivitiesZonalValuesI", "aa.base.data",
					false);
		}
		if (ptab == null) {
		...
		if (zonalData == null) {
			logger
					.info("no ActivitiesZonalValuesI or ActivitiesZonalValuesW in aa.base.data or aa.current.data, trying to get zonal data from previous run (ActivityLocations in aa.previous.data)");
			zonalData = loadTableDataSet("ActivityLocations", "aa.previous.data",
					false);
		}
		final Hashtable activityZonalHashtable = new Hashtable();
		if (zonalData == null) {
			logger
					.error("No ActivitiesZonalValuesI, ActivitiesZonelValuesW or ActivityLocations for activity zonal constants input");
		}
		else {
			...
			for (int zRow = 1; zRow <= zonalData.getRowCount(); zRow++) {
				final String activityZone = zonalData
						.getStringValueAt(zRow, "Activity")
						+ "@"
		...
		for (int pRow = 1; pRow <= ptab.getRowCount(); pRow++) {
			...
			final AggregateActivity aa = new AggregateActivity(activityName, zones,
					true);
			aa.setLocationDispersionParameter(ptab.getValueAt(pRow,
					"LocationDispersionParameter"));
			aa.setSizeTermCoefficient(ptab.getValueAt(pRow, "SizeTermCoefficient"));
			final LogitTechnologyChoice techChoice = new LogitTechnologyChoice(
					aa.name, false);
			...
			final double lambda = ptab.getValueAt(pRow,
					"ProductionSubstitutionNesting");
			final int otherLambdaColumn = ptab
					.getColumnPosition("ConsumptionSubstitutionNesting");
			...
			aa.setConsumptionFunction(new LogitTechnologyChoiceConsumptionFunction(
					techChoice));
			aa.setProductionFunction(new LogitTechnologyChoiceProductionFunction(
					techChoice));
			for (int z = 0; z < zones.length; z++) {
				final String zoneDataKey = activityName + "@"
						+ zones[z].getZoneUserNumber();
				final Integer zoneDataIndex = (Integer) activityZonalHashtable
						.get(zoneDataKey);
				if (zoneDataIndex != null) {
					...
					if (zonalData.containsColumn("Quantity")) {
						...
						quant = zonalData.getValueAt(zoneDataIndex.intValue(),
								"InitialQuantity");
					}
	...
	protected void readActivityTotals() {
		...
		final TableDataSet activityTotalsTable = loadTableDataSet(
				"ActivityTotalsI", "aa.current.data");
		for (int row = 1; row <= activityTotalsTable.getRowCount(); row++) {
			...
			if (a == null) {
				final String msg = "Missing or misspelled activity name in ActivityTotals "
						+ name;
				logger.fatal(msg);
				throw new RuntimeException(msg);
			}
	...
	protected double[][] readFloorspace() {
		...
		TableDataSet floorspaceTable = loadTableDataSet("FloorspaceW",
				"aa.floorspace.data", false);
		if (floorspaceTable == null) {
			floorspaceTable = loadTableDataSet("FloorspaceI", "aa.floorspace.data",
					false);
		}
		if (floorspaceTable == null) {
			floorspaceTable = loadTableDataSet("Floorspace", "aa.floorspace.data",
					false);
		}
		if (floorspaceTable == null) {
			logger
					.error("Can't find FloorspaceI in the directory specified by the aa.floorspace.data property, looking in aa.base.data");
	...
	protected double[][] processFloorspaceTable(TableDataSet floorspaceTable)
		...
		final int floorspaceTypeColumn = floorspaceTable
				.checkColumnPosition("Commodity");
		for (int row = 1; row <= floorspaceTable.getRowCount(); row++) {
			final int alphaZone = (int) floorspaceTable.getValueAt(row,
					alphaZoneColumn);
			final float quantity = floorspaceTable.getValueAt(row, quantityColumn);
			final String commodityName = floorspaceTable.getStringValueAt(row,
					floorspaceTypeColumn);
			final Commodity c = Commodity.retrieveCommodity(commodityName);
			if (c == null) {
				throw new Error("Bad commodity name " + commodityName
						+ " in FloorspaceI.csv");
			}
		...
		if (inputPath != null) {
			...
			try {
				...
				logger.warn("Failed to read file " + fileName
						+ ", trying as a URL Instead", fileFailed);
				try {
			...
			if (table != null) {
				...
				logger.info("Can't read " + fileName + " from " + source + " ("
						+ inputPath + "), trying to read from JDBC instead");
			}

		}
		else {
			logger.warn("Source " + source
					+ " is not defined in property file, trying to get table "
					+ tableName + " from JDBC source instead");
		}
		if (table != null) {
			return table;
		}
