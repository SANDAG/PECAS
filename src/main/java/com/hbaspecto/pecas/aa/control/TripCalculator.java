...
public class TripCalculator {
	...
	public Matrix[] calculateLuzTrips(TableDataSet spec) {
		...
		for (int r = 1; r <= spec.getRowCount(); r++) {
			...
			final Commodity c = Commodity.retrieveCommodity(spec.getStringValueAt(r,
					"CommodityName"));
			if (c == null) {
				logger.error("Invalid commodity name "
						+ spec.getStringValueAt(r, "CommodityName")
			...
			else if (spec.getStringValueAt(r, "CommodityDirection").equalsIgnoreCase(
					"s")) {
				flows = c.getSellingFlowMatrix();
			}
			else {
				logger.error("Invalid CommodityDirection "
						+ spec.getStringValueAt(r, "CommodityDirection")
						+ "in TripCalculation.csv");
			}
			flows = flows.multiply(spec.getValueAt(r, "Coefficient"));
			final Matrix trips = luzMatrices[findStringIndex(spec.getStringValueAt(r,
					"TripType"))];
	...
	public com.hbaspecto.matrix.SparseMatrix[] sampleTazTrips(Matrix[] luzTrips,
		...
		if (tripTypes == null) {
			final String msg = TripCalculator.class.getName()
					+ ".sampleTazTrips called before " + TripCalculator.class.getName()
					+ ".calculateLuzTrips";
			logger.fatal(msg);
		...
		for (int r = 1; r <= spec.getRowCount(); r++) {
			...
			makeLocationSpecifier[0] = alphaZonalMake.getIntLocationForDimension(0,
					c.getName());
			useLocationSpecifier[0] = alphaZonalUse.getIntLocationForDimension(0,
					c.getName());
			final int tripIndex = findStringIndex(spec
					.getStringValueAt(r, "TripType"));
			...
			for (int tazIdx = 0; tazIdx < alphaZones.length; tazIdx++) {
				...
				makeLocationSpecifier[1] = alphaZonalMake.getIntLocationForDimension(1,
						alphaString);
				final double alphaMake = alphaZonalMake.getValue(makeLocationSpecifier);
				useLocationSpecifier[1] = alphaZonalUse.getIntLocationForDimension(1,
						alphaString);
		...
		for (int tripType = 0; tripType < tripTypes.length; tripType++) {
			...
			for (final Iterator origIt = luzMatrix.getExternalNumberIterator(); origIt
					.hasNext();) {
	...
	private void poissonSample(Matrix[] matrices) {
		...
		for (final Matrix m : matrices) {
			...
			logger.info("Poisson sampling of trip matrix "
					+ tripTypes[matrixNumber++] + ", origTrips:" + origTotal
					+ ", newTrips:" + newTotal);

		}
	}

	private static int poisson(double lambda, double useNormalAt,
			double useRoundedAt) {
		if (lambda < 0) {
		...
		if (lambda > useNormalAt) {
			...
			final double invCumNormal = InverseCumulativeNormal.getInvCDF(random,
					false);
		...
		logger.info("poisson sampling count is 10000 times expected value of "
				+ lambda + " , random sample is " + random + " cumProbability was "
				+ cumProb + ", returning the next value " + i);
		return i;
	}
