...
public class JppfNodeSetup {
		...
		synchronized (AAPProcessor.isSetup) {
			if (AAPProcessor.isSetup) {
				logger.info("Thread " + Thread.currentThread().getId()
						+ " has already been setup, no need to setup again");
				return;
			}
			...
			try {
				...
				// test
				System.out
						.println("Can't read aa.properties or global.properties ResourceBundle on the node");
				logger
						.fatal("Can't read aa.properties or global.properties ResourceBundle on the node");
				throw new RuntimeException(
						"Can't read aa.properties or global.properties ResourceBundle on the node");
			}
			final long startTime = System.currentTimeMillis();
			final String pProcessorClass = aaProps.getProperty("pprocessor.class");
			logger.info(JppfNodeSetup.class + " will be using the " + pProcessorClass
			...
			final ResourceBundle aar = new PropertiesResourceBundle(aaProps);
	...
	private static void fixRelativePathsOnNode(Properties aaProps2) {
		// TODO Change aa.base.data, aa.reference.data, aa.skim.data and all the
		// other paths
		// to reflect the setup of the node. For instance if aa.reference.data
		// is
		...
		// /Volumes/modelshare/Oregon/OregonTestData/reference on a mac/unix
		// node

		// We haven't had to implement this because we've used the same drive
		// mapping
		// on all nodes, so all nodes can use the same path. But this means we
		// can't
