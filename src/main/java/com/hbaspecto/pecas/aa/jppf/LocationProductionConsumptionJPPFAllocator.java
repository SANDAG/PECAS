...
class LocationProductionConsumptionJPPFAllocator extends JPPFTask {
	...
	static final Logger logger = Logger
			.getLogger(LocationProductionConsumptionJPPFAllocator.class);
	...
	/**
	 * Location to hold the activity we are dealing with, transient so that it is
	 * not sent as part of the message, rather it is retrieved from the name of
	 * the Activty instead.
	 */
	...
	private void getSellingAndBuyingAmountsAndDerivatives() {
		...
		while (comit.hasNext()) {
			...
			for (int z = 0; z < zones.length; z++) {
				buyingAmounts[comNum][zones[z].zoneIndex] = c
						.retrieveCommodityZUtility(zones[z], false).getQuantity();
				sellingAmounts[comNum][zones[z].zoneIndex] = c
						.retrieveCommodityZUtility(zones[z], true).getQuantity();
				buyingDerivatives[comNum][zones[z].zoneIndex] = c
						.retrieveCommodityZUtility(zones[z], false).getDerivative();
				sellingDerivatives[comNum][zones[z].zoneIndex] = c
						.retrieveCommodityZUtility(zones[z], true).getDerivative();
			}
	...
	private void getLocationAllocation() {
		...
		for (int z = 0; z < zones.length; z++) {
			locationAllocation[z] = activity.myDistribution[zones[z].zoneIndex]
					.getQuantity();
		}
	}
	...
	void putActivityAllocationAmountsIntoMemory() throws OverflowException {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("We have the results from activity allocation for "
					+ activityName + " now storing results back in the client machine");
		}
		...
		for (int z = 0; z < locations.length; z++) {
			locations[z]
					.setQuantity(locationAllocation[locations[z].getMyTaz().zoneIndex]);
		}

		final Iterator<AbstractCommodity> it = AbstractCommodity
		...
		while (it.hasNext()) {
			...
			final Iterator<CommodityZUtility> selling = c
	...
	LocationProductionConsumptionJPPFAllocator(AggregateActivity a)
		...
		final AbstractZone[] zones = AbstractZone.getAllZones();
		commodityBuyingUtilities = new double[AbstractCommodity.getAllCommodities()
				.size()][zones.length];
		commoditySellingUtilities = new double[AbstractCommodity
		...
		while (commodityIt.hasNext()) {
			...
			for (int z = 0; z < zones.length; z++) {
				final CommodityZUtility bzu = c.retrieveCommodityZUtility(zones[z],
						false);
				commodityBuyingUtilities[comNum][zones[z].zoneIndex] = bzu
						.getUtility(1.0);
				final CommodityZUtility szu = c.retrieveCommodityZUtility(zones[z],
						true);
				commoditySellingUtilities[comNum][zones[z].zoneIndex] = szu
						.getUtility(1.0);
			}
		}
