...
public class ConditionCalculator extends JPPFTask {
	...
	ConditionCalculator(Commodity cParam) {
		...
		prices = myCommodity.getPriceInAllExchanges();
	...
	public void run() {
		try {
			...
			// could be running on the node, better get the commodity from the
			// name
	...
	Commodity getMyCommodity() {
		if (myCommodity == null) {
			myCommodity = Commodity.retrieveCommodity(commodityName);
		}
		return myCommodity;
	}
