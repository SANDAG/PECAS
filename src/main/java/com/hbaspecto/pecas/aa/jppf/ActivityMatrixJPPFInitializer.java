...
class ActivityMatrixJPPFInitializer
        extends JPPFTask
{
    ...
    public void run()
    {
        try
        {
            ...
            System.out.println("Calculating production/consumption rates and derivatives for "
                    + activityName);
            ...
            {
                pl = new DenseVector(
                        getActivity().logitModelOfZonePossibilities.getChoiceProbabilities());
                fpl = new DenseMatrix(
                        getActivity().logitModelOfZonePossibilities.choiceProbabilityDerivatives());
            ...
            final DenseMatrix dulbydprice = new DenseMatrix(fpl.sizeOfColumns(), numCommodities);
            ...
            DenseMatrix dLocationByDPrice = new DenseMatrix(AbstractZone.getAllZones().length,
                    numCommodities);
            ...
            for (int location = 0; location < pl.size(); location++)
            {
                ...
                final AggregateDistribution l = (AggregateDistribution) getActivity().logitModelOfZonePossibilities
                        .alternativeAt(location);
                l.addTwoComponentsOfDerivativesToAveragePriceMatrix(getActivity().getTotalAmount(),
                        dStorage, dThisLocationByPrices);
            }
