...
public class LinearSkimFunctionEEOverride
        extends LinearFunctionOfSomeSkims
        implements TravelUtilityCalculatorInterface
{
    ...
    {
        final double[] wouldBe = super.getUtilityComponents(origin, destination, travelConditions);
        if (origin < isOverRidden.length && destination < isOverRidden.length)
        {
            if (isOverRidden[origin] && isOverRidden[destination])
            {
