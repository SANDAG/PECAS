...
public class PeakAutoSkims extends TransportKnowledge {
	 ...
	 */
	public PeakAutoSkims(TableDataSet s, int originField, int destinationField,
			int distanceField, int timeField) {
	...
	public double getUtility(int fromZoneUserNumber, int toZoneUserNumber,
		...
		dt.time = pkTime.getValueAt(fromZoneUserNumber, toZoneUserNumber);
		dt.distance = pkDist.getValueAt(fromZoneUserNumber, toZoneUserNumber);
		if (dt.distance < 0 || dt.time < 0 || Double.isNaN(dt.distance)
				|| Double.isNaN(dt.time)) {
			throw new Error("Bad distance " + dt.distance + " or bad time " + dt.time
					+ " in skim from " + fromZoneUserNumber + " to " + toZoneUserNumber);
		}
		final double rutility = tp.getUtility(fromZoneUserNumber, toZoneUserNumber,
				dt);
		// debug Feb 26 2003
		if (printSkims > 0 || Double.isNaN(rutility)) {
			logger.info("skim from " + fromZoneUserNumber + " to " + toZoneUserNumber
					+ " is " + rutility + " for " + tp);
	...
	public double getUtility(int fromZoneUserNumber, int toZoneUserNumber,
			double valueOfTime, double costOfDistance) {
		final float distance = pkDist.getValueAt(fromZoneUserNumber,
				toZoneUserNumber);
		final float time = pkTime.getValueAt(fromZoneUserNumber, toZoneUserNumber);
		// debug March 9 2004
		if (distance < 0 || time < 0 || Double.isNaN(distance)
				|| Double.isNaN(time)) {
			throw new Error("Bad distance " + distance + " or bad time " + time
					+ " in skim from " + fromZoneUserNumber + " to " + toZoneUserNumber);
		}
	...
	public double getDistance(int fromZoneUserNumber, int toZoneUserNumber) {
		...
		final double distance = pkDist.getValueAt(fromZoneUserNumber,
				toZoneUserNumber);
		if (Double.isNaN(distance) || distance < 0) {
			...
			logger.warn(distance + " distance in skims, from zone "
