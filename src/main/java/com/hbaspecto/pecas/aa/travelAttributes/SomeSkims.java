 ...
 * 
 */
public class SomeSkims
        extends TransportKnowledge
        implements TravelAttributesInterface
{
    ...
    public void addCSVSquareMatrix(String matrixName)
    {
        ...
        {
            final CSVSquareTableMatrixReader csvReader1 = new CSVSquareTableMatrixReader(my1stPath);
            final CSVSquareTableMatrixReader csvReader2 = new CSVSquareTableMatrixReader(my2ndPath);
            Matrix m = null;
            try
            {
            ...
            } catch (final Exception e)
            {
                ...
                } catch (final Exception e1)
                {
                    logger.fatal("Could not find " + matrixName + ".csv in " + my1stPath
                            + " or in " + my2ndPath, e1);
                    throw new RuntimeException("Could not find " + matrixName + ".csv in "
                            + my1stPath + " or in " + my2ndPath, e1);
                }
            }
            matrixList.add(m);
        ...
        {
            logger.debug("finished reading zipmatrix of skims " + matrixName + " into memory");
        }
    ...
    public void addZipMatrix(String matrixName)
    {
        ...
        } else
        {
            ...
            if (!skim.exists())
            {
                ...
                if (!skim.exists())
                {
                    logger.fatal("Could not find " + matrixName
    ...
    public void addTableDataSetSkims(TableDataSet s, String[] fieldsToAdd, int maxZoneNumber)
    {
        ...
        for (int mNum = 0; mNum < fieldsToAdd.length; mNum++)
        {
            ...
            {
                fieldIds[mNum] = -1;
                logger.warn("SomeSkims already contains matrix named " + fieldsToAdd[mNum]
                        + ", not reading it in again");
            } else
            {
                ...
                if (fieldIds[mNum] <= 0)
                {
                    logger.fatal("No field named " + fieldsToAdd[mNum] + " in skim TableDataSet "
                            + s);
        ...
        for (final Matrix m : matrices)
        {
            for (final int origin : internalZoneNumbers)
            {
                ...
                for (final int destination : externalZoneNumbers)
                {
                    if (Float.isNaN(m.getValueAt(origin, destination)))
                    {
                        logger.fatal("I-E Skim " + m + " from " + origin + " to " + destination
                                + " has not been set");
                        ok = false;
                    }
                    ...
                    {
                        logger.fatal("E-I Skim " + m + " from " + origin + " to " + destination
                                + " has not been set");
                        ok = false;
                    }
