 ...
 * 
 */
public class SomeSkims extends TransportKnowledge implements
		TravelAttributesInterface {
	protected static Logger logger = Logger.getLogger("com.pb.models.pecas");

	private final ArrayList matrixList = new ArrayList();
	public Matrix[] matrices = new Matrix[0];
	private final ArrayList matrixNameList = new ArrayList();
	...
	public Matrix getMatrix(String name) {
		final int place = matrixNameList.indexOf(name);
		if (place >= 0) {
			return matrices[place];
		}
		return null;
	}
	...
	public void addCSVSquareMatrix(String matrixName) {
		if (matrixNameList.contains(matrixName)) {
			logger.info("SomeSkims already contains matrix named " + matrixName
					+ ", not reading it in again");
		}
		else {
			final CSVSquareTableMatrixReader csvReader1 = new CSVSquareTableMatrixReader(
					my1stPath);
			final CSVSquareTableMatrixReader csvReader2 = new CSVSquareTableMatrixReader(
					my2ndPath);
			Matrix m = null;
			try {
				...
				try {
					...
					logger.fatal("Could not find " + matrixName + ".csv in " + my1stPath
							+ " or in " + my2ndPath, e1);
					throw new RuntimeException("Could not find " + matrixName
							+ ".csv in " + my1stPath + " or in " + my2ndPath, e1);
				}
			}
			matrixList.add(m);
		...
		System.out.println("finished reading zipmatrix of skims " + matrixName
				+ " into memory");
		if (logger.isDebugEnabled()) {
			logger.debug("finished reading zipmatrix of skims " + matrixName
					+ " into memory");
		}
	...
	public void addZipMatrix(String matrixName) {
		if (matrixNameList.contains(matrixName)) {
			...
			if (!skim.exists()) {
				skim = new File(my1stPath + matrixName + ".zipMatrix");
			}
			if (!skim.exists()) {
				skim = new File(my1stPath + matrixName + ".zmx");
			}
			if (!skim.exists()) {
				skim = new File(my2ndPath + matrixName + ".zip");
				if (!skim.exists()) {
					skim = new File(my2ndPath + matrixName + ".zipMatrix");
				}
				if (!skim.exists()) {
					skim = new File(my2ndPath + matrixName + ".zmx");
				}
				if (!skim.exists()) {
					logger
							.fatal("Could not find "
									+ matrixName
									+ ".zip, .zipMatrix or .zmx in either the pt or the ts directory");
					throw new RuntimeException("Could not find " + matrixName
							+ ".zip, .zipMatrix or .zmx in either the pt or the ts directory");
				}
			}
	...
	public void addMatrixCSVSkims(TableDataSet s, String name) {
		final int rows = s.getRowCount();
		final int columns = s.getColumnCount() - 1;
		if (rows != columns) {
			logger
					.fatal("Trying to add CSV Matrix Skims and number of columns does not equal number of rows");
			throw new RuntimeException(
					"Trying to add CSV Matrix Skims and number of columns does not equal number of rows");
		}
		final float[][] tempArray = new float[rows][columns];
		final int[] userToSequentialLookup = new int[rows + 1];
		// check order of rows and columns
		for (int check = 1; check < s.getRowCount(); check++) {
			if (!s.getColumnLabel(check + 1).trim()
					.equals(String.valueOf((int) s.getValueAt(check, 1)))) {
				logger
						.fatal("CSVMatrixSkims have columns out of order (needs to be the same as rows)");
				throw new RuntimeException(
						"CSVMatrixSkims have columns out of order (needs to be the same as rows)");
			}
		}
		...
		final Matrix m = new Matrix(name, "", tempArray);
	 ...
	 * 
	 * @param s
	 *          the table dataset of skims. There must be a column called "origin"
	 *          and another column called "destination"
	 * @param fieldsToAdd
	 *          the names of the fields from which to create matrices from, all
	 *          other fields will be ignored.
	 */
	public void addTableDataSetSkims(TableDataSet s, String[] fieldsToAdd,
			int maxZoneNumber) {
		int originField = s.getColumnPosition("origin");
		if (originField == -1) {
			originField = s.checkColumnPosition("i");
		}
		int destinationField = s.getColumnPosition("destination");
		if (destinationField == -1) {
			destinationField = s.checkColumnPosition("j");
		}
		final int[] userToSequentialLookup = new int[maxZoneNumber];
		final int[] sequentialToUserLookup = new int[maxZoneNumber];
		...
		final int[] fieldIds = new int[fieldsToAdd.length];
		for (int mNum = 0; mNum < fieldsToAdd.length; mNum++) {
			if (matrixNameList.contains(fieldsToAdd[mNum])) {
				fieldIds[mNum] = -1;
				logger.warn("SomeSkims already contains matrix named "
						+ fieldsToAdd[mNum] + ", not reading it in again");
			}
			else {
				fieldIds[mNum] = s.getColumnPosition(fieldsToAdd[mNum]);
				if (fieldIds[mNum] <= 0) {
					logger.fatal("No field named " + fieldsToAdd[mNum]
							+ " in skim TableDataSet " + s);
					throw new RuntimeException("No field named " + fieldsToAdd[mNum]
							+ " in skim TableDataSet " + s);
				}
			}
		}
		final float[][][] tempArrays = new float[fieldsToAdd.length][zonesFound][zonesFound];

		// initialize to NaN so that missing values throw errors later
		for (final float[][] array : tempArrays) {
		...
		for (int row = 1; row <= s.getRowCount(); row++) {
			...
			for (int entry = 0; entry < fieldIds.length; entry++) {
				if (fieldIds[entry] > 0) {
					tempArrays[entry][userToSequentialLookup[origin]][userToSequentialLookup[destination]] = s
							.getValueAt(row, fieldIds[entry]);
				}
			}
		}
		...
		for (int matrixToBeAdded = 0; matrixToBeAdded < fieldsToAdd.length; matrixToBeAdded++) {
			if (fieldIds[matrixToBeAdded] > 0) {
				...
				final Matrix m = new Matrix(fieldsToAdd[matrixToBeAdded], "",
						tempArrays[matrixToBeAdded]);
	...
	public double[] getUtilityComponents(int fromZoneUserNumber,
			int toZoneUserNumber, TravelUtilityCalculatorInterface tp,
			boolean useRouteChoice) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.pb.models.pecas.TransportKnowledge#getUtility(com.pb.models.pecas
	...
	public double getUtility(AbstractZone from, AbstractZone to,
			TravelUtilityCalculatorInterface tp, boolean useRouteChoice) {
		return getUtility(from.getZoneUserNumber(), to.getZoneUserNumber(), tp,
				useRouteChoice);
	}
	...
	public double[] getUtilityComponents(AbstractZone from, AbstractZone to,
			TravelUtilityCalculatorInterface tp, boolean useRouteChoice) {
		return getUtilityComponents(from.getZoneUserNumber(),
				to.getZoneUserNumber(), tp, useRouteChoice);
	}

	/**
	 * @param my1stPath
	 *          The my1stPath to set.
	 */
	...
	/**
	 * @param my2ndPath
	 *          The my2ndPath to set.
	 */
	public void setMy2ndPath(String my2ndPath) {
		this.my2ndPath = my2ndPath;
	}

	public void checkCompleteness(ArrayList<Integer> internalZoneNumbers,
			ArrayList<Integer> externalZoneNumbers) {
		boolean ok = true;
		for (final Matrix m : matrices) {
			for (final int origin : internalZoneNumbers) {
				for (final int destination : internalZoneNumbers) {
					if (Float.isNaN(m.getValueAt(origin, destination))) {
						logger.fatal("Skim " + m + " from " + origin + " to " + destination
								+ " has not been set");
						ok = false;
					}
				}
				for (final int destination : externalZoneNumbers) {
					if (Float.isNaN(m.getValueAt(origin, destination))) {
						logger.fatal("I-E Skim " + m + " from " + origin + " to "
								+ destination + " has not been set");
						ok = false;
					}
					// also check destination to origin for the external zones,
					// here in the inner loop
					if (Float.isNaN(m.getValueAt(destination, origin))) {
						logger.fatal("E-I Skim " + m + " from " + origin + " to "
								+ destination + " has not been set");
						ok = false;
					}
