 ...
 * 
 */
public class LinearFunctionOfSomeSkims implements
		TravelUtilityCalculatorInterface {

	private static Logger logger = Logger
			.getLogger(LinearFunctionOfSomeSkims.class);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.pb.models.pecas.TravelUtilityCalculatorInterface#getUtility(com.pb
	 * .models.pecas.TravelAttributesInterface)
	 */
	@Override
	public double getUtility(int origin, int destination,
		...
		if (travelConditions == lastSkims) {
			// access by index number if the index numbers are already correct
			// because we've been here before.
			double utility = 0;
			for (int i = 0; i < coefficients.length; i++) {
				try {
					utility += coefficients[i]
							* lastSkims.matrices[matrixIndices[i]].getValueAt(origin,
									destination);
				}
				catch (final ArrayIndexOutOfBoundsException e) {
					logger.fatal("Something wrong with "
							+ namesList.get(matrixIndices[i]));
					throw new RuntimeException("Something wrong with "
							+ namesList.get(matrixIndices[i]), e);
				}
			}
			return utility;
		}
		if (travelConditions instanceof SomeSkims) {
			// On the other hand, if the index numbers are not yet correct, then
			// need to look up the matrices by name
			...
			// now that we've set up all the index numbers to match the matrix
			// names, we can call recursively to access by index number
			return getUtility(origin, destination, lastSkims);
		}
		else {
			throw new RuntimeException(
					"Can't use LinearFunctionOfSomeSkims with travel attributes of type "
							+ travelConditions.getClass());
		}
	}
	...
	public double[] getUtilityComponents(int origin, int destination,
			TravelAttributesInterface travelConditions) {
		if (travelConditions == lastSkims) {
			final double[] components = new double[coefficients.length];
			for (int i = 0; i < coefficients.length; i++) {
				components[i] = coefficients[i]
						* lastSkims.matrices[matrixIndices[i]].getValueAt(origin,
								destination);
			}
			return components;
