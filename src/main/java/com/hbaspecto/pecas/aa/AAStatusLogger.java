...
public class AAStatusLogger {
	 ...
	 * title is formed by taking the first part of the module name (the text up to
	 * the first dot (".")), capitalizing it, and appending " Status" to it.
	 * 
	 * @param module
	 *          The module logging this message. Should be in the form
	 *          <code>module.submodule.subsubmodule</code>(etc.). An example would
	 *          be <code>pt.ld</code>
	 * 
	 * @param text
	...
	/**
	 * Log a histogram status message. A histogram message can be looked at as an
	 * "amount complete" gauge; it essentially reports how much has been
	 ...
	 */
	public static void logHistogram(String title, double goal,
			double currentPoint, String xAxisLabel, String yAxisLabel) {
		if (enabled) {
			StatusLogger.logHistogram(moduleName, title, goal, currentPoint,
	 ...
	 * (x,y) representing the current status of the model/module. A collection of
	 * these points could be read by the processor and graphed to show both the
	 * current status as well as its progression. As the logger semicolon delimits
	 * its message (for whatever processor may use it), semicolons should not be
	 * used in any of the arguments in the message.
	 * 
	 * @param module
	 *          The module logging this message. Should be in the form
	 *          <code>module.submodule.subsubmodule</code>(etc.). An example would
	 ...
	 *          The label to use for the histogram's y-axis.
	 */
	public static void logGraph(String title, double xPoint, double yPoint,
			String xAxisLabel, String yAxisLabel) {
		if (enabled) {
			StatusLogger.logGraph(moduleName, title, xPoint, yPoint, xAxisLabel,
					yAxisLabel);
		}
	}
}
