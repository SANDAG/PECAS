...
public class ActivityInLocationWithLogitTechnologyChoice extends
	...
	public ActivityInLocationWithLogitTechnologyChoice(ProductionActivity p,
			AbstractZone t) {
		super(p, t);
	}

	@Override
	public double calcLocationUtilityNoSizeEffect(ConsumptionFunction cf,
	...
	@Override
	public double calcLocationUtilityDebug(ConsumptionFunction cf,
			ProductionFunction pf, boolean debug,
		...
		if (debug) {
			...
			logger.info(this
					...
					+ tchoice.overallUtility(buyingCommodityUtilities,
		...
		if (debug) {
			logger.info("utility = " + utility + " + "
					+ getLocationSpecificUtilityInclTaxes() + " + " + 1
					/ higherLevelDispersionParameter
					* myProductionActivity.getSizeTermCoefficient()
					* Math.log(getAllocationSizeTerm()));
		}
		return utility + getLocationSpecificUtilityInclTaxes() + 1
				/ higherLevelDispersionParameter
				* myProductionActivity.getSizeTermCoefficient()
				* Math.log(getAllocationSizeTerm());
	}
	...
	@Override
	public void setCommoditiesBoughtAndSold(ConsumptionFunction cf,
			ProductionFunction pf) throws OverflowException {
		...
		try {
			buyingQuantities = techChoice.calcBuyingAmounts(buyingCommodityUtilities,
					sellingCommodityUtilities, myTaz.zoneIndex);
			sellingQuantities = techChoice.calcSellingAmounts(
		...
		final double[] amountsDerivatives = techChoice.amountsDerivatives(
				buyingCommodityUtilities, sellingCommodityUtilities, myTaz.zoneIndex);
		...
		for (int c = 0; c < numCommodities; c++) {
			...
			if (Double.isNaN(buyingQuantities[c])
					|| Double.isInfinite(buyingQuantities[c])) {
				final String error = "Error in consumption :" + myProductionActivity
						+ " in " + myTaz + " consumes " + buyingQuantities[c] + " of "
						+ commodity;
				logger.error(error);
			...
			if (debug) {
				logger.info("In zone " + myTaz.getZoneUserNumber() + " "
						+ getQuantity() + " of " + myProductionActivity + " consumes "
						+ buyingQuantities[c] * getQuantity() + " of " + commodity);
			}
			try {
				// first set the amount bought
				if (getQuantity() != 0) {
					buyingZUtilities[c].changeQuantityBy(-getQuantity()
							* buyingQuantities[c]);
					sellingZUtilities[c].changeQuantityBy(getQuantity()
							* sellingQuantities[c]);
				}
				...
				// zero.
				if (derivative == 0) {
					buyingZUtilities[c].changeDerivativeBy(-getQuantity()
					...
					buyingZUtilities[c].changeDerivativeBy(derivative
							* compositeUtilityDerivatives[c] * buyingQuantities[c]
							- getQuantity() * amountsDerivatives[c]);
					sellingZUtilities[c].changeDerivativeBy(derivative
							* compositeUtilityDerivatives[c + numCommodities]
				...
				logger.fatal("Error: " + myProductionActivity + " in " + myTaz
						+ " consumes " + buyingQuantities[c] + " of " + commodity
						+ " leading to " + e);
				e.printStackTrace();
				throw new Error("Error: " + myProductionActivity + " in " + myTaz
						+ " consumes " + buyingQuantities[c] + " of " + commodity
						+ " leading to " + e);
			}
	...
	@Override
	public void writeLocationUtilityTerms(Writer w)
		...
		try {
			...
			techChoice.setUtilities(buyingCommodityUtilities,
					sellingCommodityUtilities, myTaz.zoneIndex);
			...
			w.write(technologyChoiceLogsum + getLocationSpecificUtilityInclTaxes()
					+ sizeTerm + ",");
	...
	@Override
	protected void allocateLocationChoiceAveragePriceDerivatives(
		...
		double[] sellingQuantities = null;
		try {
			buyingNegativeQuantities = techChoice.calcBuyingAmounts(
					buyingCommodityUtilities, sellingCommodityUtilities, myTaz.zoneIndex);
			sellingQuantities = techChoice.calcSellingAmounts(
					buyingCommodityUtilities, sellingCommodityUtilities, myTaz.zoneIndex);
		}
		...
		for (int cNum = 0; cNum < techChoice.getMyCommodityOrder().size(); cNum++) {
			...
			final double buyingQuantity = totalActivityQuantity
					* buyingNegativeQuantities[cNum];
			final double sellingQuantity = totalActivityQuantity
					* sellingQuantities[cNum];
			for (int c1 = 0; c1 < locationChoiceDerivatives.size(); c1++) {
				...
				// down.
				if (Double.isNaN(buyingQuantity
					...
					logger
							.fatal(this
									...
									+ cNum + "," + c1);
					logger.fatal("buyingQuantity:" + buyingQuantity
				...
				if (Double.isNaN(sellingQuantity
						* locationChoiceDerivatives.elementAt(c1))) {
					logger
							.fatal(this
	...
	protected void allocateProductionChoiceAveragePriceDerivatives(
		...
		for (int c = 0; c < numCommodities; c++) {
			final Commodity commodity = techChoice.getMyCommodityOrder().get(c);
			sellingPriceCoefficients[c] = commodity
		...
		for (int row = 0; row < averagePriceSurplusMatrix.length; row++) {
			for (int col = 0; col < averagePriceSurplusMatrix[row].length; col++) {
				...
				averagePriceSurplusMatrix[row][col] += technologyDerivatives.get(row,
						col) * (quantity * buyingPriceCoefficients[col]);
				// change in amount bought wrt selling price
				averagePriceSurplusMatrix[row][col] += technologyDerivatives.get(row,
						col + numCommodities) * quantity * sellingPriceCoefficients[col];
	...
	public double[] calculateLocationUtilityWRTAveragePrices() {
		...
		try {
			compositeUtilityDerivatives = techChoice.compositeUtilityDerivatives(
					techChoice.buyingUtilities, techChoice.sellingUtilities,
					myTaz.zoneIndex);
		}
		catch (final NoAlternativeAvailable e1) {
			final String msg = "No alternative available when calculating composite utility derivatives for "
		...
		for (int cNum = 0; cNum < techChoice.getMyCommodityOrder().size(); cNum++) {
			...
			final double buyingPriceCoefficient = c
					.getBuyingUtilityPriceCoefficient();
			final double sellingPriceCoefficient = c
					.getSellingUtilityPriceCoefficient();
			...
			compositeUtilityDerivativesWrtChangesInRegionPrice[cNum] = -compositeUtilityDerivatives[cNum]
					* buyingPriceCoefficient
					+ compositeUtilityDerivatives[cNum
							+ techChoice.getMyCommodityOrder().size()]
					* sellingPriceCoefficient;
			if (Double
					.isNaN(compositeUtilityDerivativesWrtChangesInRegionPrice[cNum])) {
				logger.warn("NaN occuring in locationUtilityWRTAveragePrices");
			}
		}
	...
	public void writeOutInformationOnEachOption(int zoneNumber, PrintWriter out)
		...
		techChoice.setUtilities(buyingCommodityUtilities,
				sellingCommodityUtilities, myTaz.zoneIndex);
		techChoice.writeOutOptionUtilities(myProductionActivity.getNumber(),
				zoneNumber, out);
	}
