...
/**
 * Attributes (and methods) that relate to a particular ProductionActivity in a
 * particular zone. Includes the quantity, net tax, and any constraints. Also a
 * zone-specific Utility. This implements the "Alternative" interface because
 * the total amount of activity is allocated amongst these with a logit model.
 * 
 * @author John Abraham
 */
public abstract class AggregateDistribution extends AmountInZone implements
		AggregateAlternative {
	protected static transient Logger logger = Logger
			.getLogger("com.pb.models.pecas");
	double derivative;
	 ...
	 */
	@Override
	public abstract void writeLocationUtilityTerms(Writer w)
			throws ChoiceModelOverflowException;
	...
	public double calcLocationUtility(ConsumptionFunction cf,
			ProductionFunction pf, double higherLevelDispersionParameter)
			throws OverflowException {
		final boolean debug = false;
		return calcLocationUtilityDebug(cf, pf, debug,
				higherLevelDispersionParameter);
	}

	public abstract double calcLocationUtilityNoSizeEffect(
			ConsumptionFunction cf, ProductionFunction pf) throws OverflowException;

	public abstract double calcLocationUtilityDebug(ConsumptionFunction cf,
	...
	public double getUtilityNoSizeEffect() throws ChoiceModelOverflowException {
		try {
			return calcLocationUtilityNoSizeEffect(
					myProductionActivity.getConsumptionFunction(),
					myProductionActivity.getProductionFunction());
		}
		catch (final OverflowException e) {
			throw new ChoiceModelOverflowException(e.toString());
	...
	public void setAggregateQuantity(double amount, double derivative)
		...
		if (Double.isNaN(amount) || Double.isInfinite(amount)) {
			logger.fatal("amount in zone is NaN/Infinite " + this
					+ " previous quantity:" + getQuantity()
					+ " -- try less agressive step size...");
			logger
					.fatal("following (between ***) is the utility calculation for this location");
			logger.fatal("**********************************************");
			try {
				calcLocationUtilityDebug(lastConsumptionFunction,
						lastProductionFunction, true,
						((AggregateActivity) myProductionActivity)
			...
			logger.fatal("**********************************************");
			throw new Error("amount in zone is NaN/Infinite " + this
					+ " previous quantity:" + getQuantity()
					+ " -- try less agressive step size...");
		}
		setQuantity(amount);
	...
	public abstract void setCommoditiesBoughtAndSold(ConsumptionFunction cf,
	 ...
	 */
	public void addTwoComponentsOfDerivativesToAveragePriceMatrix(
			double activityAmount, double[][] averagePriceSurplusMatrix,
			VectorI thisLocationByPrices) {
