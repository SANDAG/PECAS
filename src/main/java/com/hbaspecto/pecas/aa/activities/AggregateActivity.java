   ...
   // private double locationDispersionParameter;
    ...
    public AggregateActivity(String name, AbstractZone[] allZones, boolean technologyChoice) {
        ...
        for (int z = 0; z < allZones.length; z++) {
            if (technologyChoice) myDistribution[z] = new ActivityInLocationWithLogitTechnologyChoice(this, allZones[z]);
            else {
            	String msg = "Must use TechnologyChoice now in PECAS AA";
            	logger.fatal(msg);
	...
	public void checkConstraintConsistency() {
    	...
    	if (remainingAmount <=0) {
    		if (remainingAmount ==0) {
	    		logger.warn("Quantity of "+this+" is zero, making sure all constraints are also zero");
    	...
    	// check the constraint consistency
    	{
	    	...
	    	if (allZonesConstrained || remainingAmount<=.05*originalTotal) {
	    		...
	    		logger.info("Amount of "+this+" to be allocated amonst unconstrained zones is "+remainingAmount+ " ("+remainingAmount/originalTotal*100+"%)");
	    	}
    	}
