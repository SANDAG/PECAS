...
public class AggregateActivity extends ProductionActivity {
	...
	public AggregateActivity(String name, AbstractZone[] allZones,
			boolean technologyChoice) {
		super(name, allZones);
		...
		for (int z = 0; z < allZones.length; z++) {
			if (technologyChoice) {
				myDistribution[z] = new ActivityInLocationWithLogitTechnologyChoice(
						this, allZones[z]);
			}
			else {
			...
			logitModelOfZonePossibilities
					.addAlternative((AggregateDistribution) myDistribution[z]);
		}
	 ...
	 * to zones. PA calls this for each production activity. To simulate economic
	 * equilibrium, aaModel may have to call this function once, then repeatedly
	 * call reMigrateAndReAllocate. Note, though, that not all activities are in
	 * spatial equilibrium. For ODOT, for instance, household transitions are not
	 * in equilibrium. This routine models a discrete time step. It's important to
	 * be able to adjust the time step if necessary, and in fact different things
	 * may need different time steps. So these routines should be careful to use
	 * this parameter. A longer time step implies larger changes.
	 * 
	 * @param timeStep
	 *          the amount of time that has passed since this function was last
	 *          called. If zero, then the ProductionActivity is to redo the
	 *          previous allocation
	 */
	public void migrationAndAllocation(double timeStep)
			throws ChoiceModelOverflowException {
		logitModelOfZonePossibilities
				.setDispersionParameter(getLocationDispersionParameter());
		...
		try {
			...
			logger
					.fatal("This error shouldn't occur -- AggregateActivities can always redo their allocation process");
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.pb.models.pecas.ProductionActivity#migrationAndAllocation(double,
	 * double, double)
	 */
	@Override
	public void migrationAndAllocation(double timeStep, double inMigration,
		...
		try {
			...
			throw new OverflowException(
					"Overflow exception when allocating activity " + name + " "
							+ e.toString(), e);
		}
	 ...
	 * AggregateActivities, there is a need to do the allocation, then adjust the
	 * prices and redo the allocation repeatedly to find the economic equilibrium.
	 * Thus aaModel needs to call migrationAndAllocation once, to set the in
	 * migration, out migration and time period and to reset the previous time
	 * point. Then aaModel can adjust the prices that result and have the
	 * ProductionActivity reallocate itself and redo the migration in response to
	 * different prices by calling this method.
	 * 
	 * @throws CantRedoError
	 *           not all types of ProductionActivity can redo their allocation.
	 *           Obviously, then, they can't be modelled as being in spatial
	...
	public void reMigrationAndReAllocation() throws CantRedoError {
		try {
			...
			logger.error("Overflow exception when allocating activity " + name);
			throw new RuntimeException("Overflow exception when allocating activity "
					+ name, e);
		}
	}

	public void reMigrationAndReAllocationWithOverflowTracking()
		...
		try {
			...
			throw new OverflowException("Overflow allocating " + name + ","
					+ e.toString(), e);
		}
		...
		for (int i = 0; i < probs.length; i++) {
			...
			if (a instanceof AggregateDistribution) {
				...
				logger
						.fatal("AmountInZone object for activity "
								+ this
		...
		if (originalTotal == 0 && remainingAmount < 0) {
			final String msg = "For " + this
					+ " total amount is zero but constraints are non-zero";
			logger.fatal(msg);
			throw new RuntimeException(msg);
		}
		if (originalTotal != 0) {
			...
			if (allZonesConstrained
					&& Math.abs(remainingAmount / originalTotal) > 0.001) {
				logger
						.error("All zones constrained for "
								+ this
			...
			if (a instanceof AggregateDistribution) {
				if (!isForceConstraints() || !((AggregateDistribution) a).constrained) {
					if (totalProbs <= 0) {
						...
						logger
								.error("This can happen if the unconstrained zones have no suitable space, in which case the solution is to enter a constraint of zero in any zone with no suitable space");
					...
					((AggregateDistribution) a).setAggregateQuantity(
							remainingAmount * prob,
							prob * (1 - prob)
									* logitModelOfZonePossibilities.getDispersionParameter()
									* remainingAmount);
				}
			}
			else {
		...
		// check the constraint consistency
		{
			...
			if (allZonesConstrained || remainingAmount <= .05 * originalTotal) {
				...
				if (Math.abs(ratio) > .0001) {
					...
					logger.error("originalTotal =" + originalTotal
							+ ", constraints sum to " + constraintTotal);
				}
				else {
				...
				logger.info("Amount of " + this
						+ " to be allocated amonst unconstrained zones is "
	...
	public void setLocationDispersionParameter(double locationDispersionParameter) {
		logitModelOfZonePossibilities
				.setDispersionParameter(locationDispersionParameter);
