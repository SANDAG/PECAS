...
/**
 * This class represents a type of economic activity that occurs in the modelled area. This is an abstract class -- all ProductionActivity is
 * represented by one of the subclasses AggregateActivity or DisaggregateActivity The main role of this class is to track the amount of activity in
 * each zone and to allocate the changes in the amount of activity in each zone over each time step. As part of this, the class also needs to work
 * with an AssignmentPeriod to load a network with trips.
 * 
...
public abstract class ProductionActivity
{
    ...
    /**
     * A ProductionActivity needs to set up its array of AggregateDistributions to keep track of how much activity is in each zone. To ensure that
     * this is done properly we make the constructor take as an argument the array of all zones.
     * 
     * @param allZones
    ...
    /**
     * setDistribution sets the amount of activity in a zone. The ProbabilityDensityFunction v can be used to specify how this activity might not be
     * homogeneous.
     * 
     * @param zone
     *            the zone in which the activity is being set param v a statistical function for accounting for the variation within this category of
     ...
     * @param outMigration
     *            the amount of activity leaving the region. Net migration is inMigration-outMigration.
     */
     ...
     * redo the allocation repeatedly to find the economic equilibrium. Thus AAModel needs to call mygrationAndAllocation once, to set the in
     ...
     *                not all types of ProductionActivity can redo their allocation. Obviously, then, they can't be modelled as being in spatial
