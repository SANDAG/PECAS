...
public abstract class ProductionActivity {
	...
	public void increaseSizeTerm(int zoneUserNumber, double sizeTermIncrement) {
		final int zoneIndex = AbstractZone.findZoneByUserNumber(zoneUserNumber)
				.getZoneIndex();
	...
	/**
	 * A ProductionActivity needs to set up its array of AggregateDistributions to
	 * keep track of how much activity is in each zone. To ensure that this is
	 * done properly we make the constructor take as an argument the array of all
	 * zones.
	 * 
	 * @param allZones
	 ...
	 * ProbabilityDensityFunction v can be used to specify how this activity might
	 * not be homogeneous.
	 * 
	 * @param zone
	 *          the zone in which the activity is being set param v a statistical
	 *          function for accounting for the variation within this category of
	 *          activity, only used for Disaggregate activities
	 * @param quantity
	 ...
	 */
	public void setDistribution(AbstractZone zone, double quantity,
			double sizeTerm, double locationUtility) {
		for (int z = 0; z < myDistribution.length; z++) {
			if (myDistribution[z].getMyTaz().equals(zone)) {
				myDistribution[z].setQuantity(quantity);
	 ...
	 */
	abstract public void migrationAndAllocation(double timeStep,
			double inMigration, double outMigration) throws OverflowException;
	 ...
	 * Thus AAModel needs to call mygrationAndAllocation once, to set the in
