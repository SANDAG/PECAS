...
/**
 * This is the amount of commodity moving from a location to an exchange
 * location. Thus the quantity should be a positive number for sellingZUtility
 * (producers of the commodity) and a negative number for buyingZUtility
...
public class CommodityFlowArray implements AggregateAlternative /* CompositeAlternativeInterface */{
	...
	/**
	 * Attribute describing the type of flow. theCommodityZUtility has attributes
	 * describing the commodity being transferred, whether the commodity is being
	 * bought or sold in the exchange zone, and what zone it is being brought in
	 * from (for selling) or being shipped out to (for buying).
	 */
	...
	/**
	 * The market where the flow is going to or coming from. Sellers
	 * (sellingZUtility) ship a commodity to a market and sell it there; buyers go
	 * to the market, buy commodities, and ship them back. theExchange represents
	 * a market at a spatial location.
	 */
	// public Exchange[] theExchanges;

	CommodityFlowArray(CommodityZUtility where,
	...
	private double buyingUtilityConsideringPriceSizeAndTransport(double rUtility,
		...
		final double bob = c.getBuyingUtilitySizeCoefficient() * 1
				/ dispersionParameter * Math.log(theExchange.getBuyingSizeTerm())
				+ c.getBuyingUtilityPriceCoefficient() * theExchange.getPrice()
				+ c.getBuyingUtilityTransportCoefficient() * rUtility;
		if (Double.isNaN(bob)) {
			logger.warn("buying utility is NaN for " + theCommodityZUtility + " to "
					+ theExchange);
		...
		if (Double.isInfinite(bob) && theExchange.getBuyingSizeTerm() != 0) {
			if (bob > 0) {
				logger.warn("buying utility is infinite for " + theCommodityZUtility
						+ " to " + theExchange);
				logger.warn("rutility:" + rUtility + "  price:"
						+ theExchange.getPrice() + "  Size:"
						+ theExchange.getBuyingSizeTerm());
			}
			else if (logger.isDebugEnabled()) {
				logger.debug("buying utility is infinite for " + theCommodityZUtility
						+ " to " + theExchange);
				logger.debug("rutility:" + rUtility + "  price:"
						+ theExchange.getPrice() + "  Size:"
						+ theExchange.getBuyingSizeTerm());
			}
		}
	...
	private double buyingUtilityConsideringPriceAndTransport(double rUtility,
			Exchange theExchange) {
		final Commodity c = theCommodityZUtility.myCommodity;
		final double bob = c.getBuyingUtilityPriceCoefficient()
				* theExchange.getPrice() + c.getBuyingUtilityTransportCoefficient()
				* rUtility;
		return bob;
	}

	private double[] buyingUtilityComponents(double[] transportComponents,
			Exchange theExchange) {
		final double[] components = new double[transportComponents.length + 2];
		final Commodity c = theCommodityZUtility.myCommodity;
		components[0] = c.getBuyingUtilityPriceCoefficient()
				* theExchange.getPrice();
		components[1] = c.getBuyingUtilitySizeCoefficient() * 1
				/ dispersionParameter * Math.log(theExchange.getBuyingSizeTerm());
		for (int i = 2; i < components.length; i++) {
			components[i] = transportComponents[i - 2]
					* c.getBuyingUtilityTransportCoefficient();
		}
		return components;
	}

	private double sellingUtilityConsideringPriceAndTransport(double rUtility,
			Exchange theExchange) {
		...
		// difference.
		final double bob = c.getSellingUtilityPriceCoefficient()
				* theExchange.getPrice() + c.getSellingUtilityTransportCoefficient()
				* rUtility;
		return bob;
	}

	private double sellingUtilityConsideringPriceSizeAndTransport(
			double rUtility, Exchange theExchange) {
		// calculates SUc,z,k or BUc,z,k which is the utility of selling to or
		// buying from exchange
		// location 'k', a commodity 'c' that was produced or consumed in zone
		// 'z'.
		...
		// the value is set to 1.0 however so maybe it doesn't make a
		// difference.
		final double bob = c.getSellingUtilitySizeCoefficient() * 1
				/ dispersionParameter * Math.log(theExchange.getSellingSizeTerm())
				+ c.getSellingUtilityPriceCoefficient() * theExchange.getPrice()
				+ c.getSellingUtilityTransportCoefficient() * rUtility;
		if (Double.isNaN(bob)) {
			logger.warn("selling utility is NaN for " + theCommodityZUtility
	...
	private double[] sellingUtilityComponents(double[] transportComponents,
			Exchange theExchange) {
		final double[] components = new double[transportComponents.length + 2];
		final Commodity c = theCommodityZUtility.myCommodity;
		components[0] = c.getSellingUtilityPriceCoefficient()
				* theExchange.getPrice();
		components[1] = c.getSellingUtilitySizeCoefficient() * 1
				/ dispersionParameter * Math.log(theExchange.getSellingSizeTerm());
		for (int i = 2; i < components.length; i++) {
			components[i] = transportComponents[i - 2]
					* c.getSellingUtilityTransportCoefficient();
		}
		return components;
	}
	...
	public double calcUtilityForExchange(Exchange theExchange) {
		if (theCommodityZUtility instanceof SellingZUtility) {
			...
			// modes
			double rUtility;
			rUtility = TransportKnowledge.globalTransportKnowledge
					.getUtility(theCommodityZUtility.getTaz(), AbstractZone
							.findZoneByUserNumber(theExchange.exchangeLocationUserID),
			...
			return sellingUtilityConsideringPriceSizeAndTransport(rUtility,
					theExchange);
		}
		else {
			...
			double rUtility;
			rUtility = TransportKnowledge.globalTransportKnowledge
					.getUtility(AbstractZone
							.findZoneByUserNumber(theExchange.exchangeLocationUserID),
							theCommodityZUtility.getTaz(), theCommodityZUtility
									.getMyTravelPreferences(), theCommodityZUtility
									.getUseRouteChoice());
			...
			return buyingUtilityConsideringPriceSizeAndTransport(rUtility,
					theExchange);
		}
	}
	...
	public double[] calcUtilityComponentsForExchange(Exchange theExchange) {
		if (theCommodityZUtility instanceof SellingZUtility) {
			...
			final double[] utilityComponentsTemp = TransportKnowledge.globalTransportKnowledge
					.getUtilityComponents(theCommodityZUtility.getTaz(), AbstractZone
							.findZoneByUserNumber(theExchange.exchangeLocationUserID),
							theCommodityZUtility.getMyTravelPreferences(),
							theCommodityZUtility.getUseRouteChoice());
			...
			// zone 'z'. It is the weighted sum of time/cost across all modes
			final double[] utilityComponentsTemp = TransportKnowledge.globalTransportKnowledge
					.getUtilityComponents(AbstractZone
							.findZoneByUserNumber(theExchange.exchangeLocationUserID),
							theCommodityZUtility.getTaz(), theCommodityZUtility
									.getMyTravelPreferences(), theCommodityZUtility
									.getUseRouteChoice());
	...
	public double calcUtilityForTravelPreferences(
			TravelUtilityCalculatorInterface tp, Exchange theExchange) {
		if (theCommodityZUtility instanceof SellingZUtility) {
			final double rUtility = TransportKnowledge.globalTransportKnowledge
					.getUtility(theCommodityZUtility.getTaz(), AbstractZone
							.findZoneByUserNumber(theExchange.exchangeLocationUserID), tp,
							false);
			return sellingUtilityConsideringPriceSizeAndTransport(rUtility,
					theExchange);
		}
	...
	public double[] getUtilityComponents(double higherLevelDispersionParameter) {
		...
		if (com.exchangeType == 'p'
				&& theCommodityZUtility instanceof SellingZUtility) {
	...
	public double getUtilityNoSizeEffect() throws ChoiceModelOverflowException {
		...
		if (com.exchangeType == 'p'
				&& theCommodityZUtility instanceof SellingZUtility) {
			return calcUtilityForExchange(com.getExchange(theCommodityZUtility.myTaz
					.getZoneIndex()));
		}
		if (com.exchangeType == 'c'
				&& theCommodityZUtility instanceof BuyingZUtility) {
		...
		// error checking:
		if (Double.isNaN(bob)) { // write out the individual exchange utilities
		...
		if (bob == Double.POSITIVE_INFINITY) {
			logger.error("composite utility is positive infinity for "
		...
		if (bob == Double.NEGATIVE_INFINITY) {
			logger.warn("composite utility is negative infinity for "
					+ theCommodityZUtility);
		}
			...
			for (i = 0; i < weights.length; i++) {
				for (j = weights.length - 1; j >= 0; j--) {
					if (i == j) {
						...
						returns[i][j] = -dispersionParameter * (weights[i] / sum)
								* weights[j] / sum;
					}
				}
			}
	...
	public double[] getChoiceProbabilities() {
		...
		if (com.exchangeType == 'p'
				...
				|| com.exchangeType == 'n') {
			weights = new double[theExchanges.size()];
			final Exchange thisExchangeHere = com.getExchange(theCommodityZUtility
					.getTaz().getZoneIndex());
		...
		if (com.exchangeType == 'p' && selling == 's' || com.exchangeType == 'c'
			...
			try {
				if (Double.isNaN(amount) || Double.isInfinite(amount)) {
					logger.warn("quantity for intrazonal flow " + taz + " selling:"
							+ selling + " to " + x);
					logger.warn("   amount:" + amount);
					throw new ChoiceModelOverflowException("Infinite or NaN flow");
				}
				if (selling == 's') {
					x.setFlowQuantityAndDerivative(
							taz,
							selling,
							amount,
							theCommodityZUtility.myCommodity
									.getSellingUtilityPriceCoefficient() * derivative);
				}
				else {
					x.setFlowQuantityAndDerivative(
							taz,
							...
							amount,
							theCommodityZUtility.myCommodity
									.getBuyingUtilityPriceCoefficient() * derivative);
				}
			}
			...
			for (int i = 0; i < aggregateQuantityWeights.length; i++) {
				...
				if (Double.isNaN(quantity) || Double.isInfinite(quantity)) {
					logger.warn("quantity for flow " + taz + " selling:" + selling
							+ " to " + anEx);
					logger.warn("   amount:" + amount + " * weight:"
							+ aggregateQuantityWeights[i] + " = " + quantity);
				}
				try {
					if (theCommodityZUtility instanceof SellingZUtility) {
	...
	public double[] getLogsumDerivativesWRTPrices() {
		...
		double multiplier = 0;
		if (theCommodityZUtility instanceof SellingZUtility) {
			multiplier = theCommodityZUtility.myCommodity
					.getSellingUtilityPriceCoefficient();
		}
		if (theCommodityZUtility instanceof BuyingZUtility) {
			multiplier = theCommodityZUtility.myCommodity
