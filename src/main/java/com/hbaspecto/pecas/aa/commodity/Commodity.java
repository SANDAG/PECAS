...
public class Commodity extends AbstractCommodity {
	...
	private Commodity(String name, char exchangeTypePar) {
		...
		if (exchangeType != 'c' && exchangeType != 'p' && exchangeType != 'a'
			...
			throw new Error("Commodity " + name + " has invalid exchange type"
					+ exchangeTypePar + ": only c,p,a,n or s are allowed");
		}
	}
	...
	public void unfixPricesAndConditions() {
		...
		for (int s = 0; s < 2; s++) {
			...
			for (int z = 0; z < zones.length; z++) {
				...
				final CommodityZUtility czu = retrieveCommodityZUtility(zones[z],
						selling);
	...
	public CommodityZUtility retrieveCommodityZUtility(int zoneNumber,
			boolean selling) {
		final PECASZone t = (PECASZone) AbstractZone
				.findZoneByUserNumber(zoneNumber);
	...
	public Matrix getBuyingFlowMatrix() {
		...
		for (int exchange = 0; exchange < nZones; exchange++) {// exchange zones
			...
			if (ex != null) {
				zoneNumbers[exchange + 1] = AbstractZone.getZone(exchange)
						.getZoneUserNumber();
	...
	public double getLargestSClear(double confac) {
		...
		while (exIt.hasNext()) {
			...
			final double singleExchangeTotal = (x.boughtTotal() + x.soldTotal()) / 2
					+ confac * avgExchangeTotal / compositeMeritMeasureWeighting;
			...
			for (final Exchange e : c.allExchanges) {
				...
				if (e.getBuyingSizeTerm() != oldExchangeSizes[0]
					...
					logger.debug("Buying exchange size change from "
							+ oldExchangeSizes[0] + " to " + e.getBuyingSizeTerm() + " for "
							+ e);
				}
				if (e.getSellingSizeTerm() != oldExchangeSizes[1]
						&& logger.isDebugEnabled()) {
					logger.debug("Selling exchange size change from "
							+ oldExchangeSizes[1] + " to " + e.getSellingSizeTerm() + " for "
							+ e);
				}
			}
		}
