...
public class PECASZone extends AbstractZone implements UnitOfLand {
    ...
    public static PECASZone createTaz(int zoneIndex, int zoneUserNumber, String zoneName, boolean external) {
        AbstractZone zones[] = getAllZones();
        if (zoneIndex >= zones.length || zoneIndex < 0)
            throw new Error("Need to index zones consecutively within the allocated array size");
        if (zones[zoneIndex] != null) throw new Error("Attempt to create zone with index" + zoneIndex + " more than once");
  ...
  /*  public double calcZUtility(Commodity c, boolean selling) throws ChoiceModelOverflowException {
        // message #1.2.3.1.3.1 to theZutilityOfACommodityInAZone:com.pb.models.pecas.CommodityZutility
...
/*    public double calcZUtilityForPreferences(Commodity c, boolean selling, TravelUtilityCalculatorInterface tp,
        boolean withRouteChoice) {
            // message #2.5.1 to zUtilityOfLabourOrConsumption:com.pb.models.pecas.CommodityZutility
...
/*    public static void createTestTazAndExchange(ZoningSchemeInterface zs) {
        ...
        for (int c = 0; c < numTestComs; c++) {
            for (int z = 0; z < numTestZones; z++) {
                SellingZUtility szu = new SellingZUtility(coms[c], (PECASZone)allZonesCopy[z]); // constructor has to set up links in both directions
                BuyingZUtility bzu = new BuyingZUtility(coms[c], (PECASZone)allZonesCopy[z]);
                szu.setDispersionParameter(0.2);
                bzu.setDispersionParameter(0.2);
                Exchange xc = new Exchange(coms[c], (PECASZone)allZonesCopy[z]);
            ...
            for (int z = 0; z < numTestZones; z++) {
                PECASZone t = (PECASZone)allZonesCopy[z];
                if (c < numTestComs / 2) {
