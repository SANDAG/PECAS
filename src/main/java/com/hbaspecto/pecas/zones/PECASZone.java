...
public class PECASZone extends AbstractZone implements UnitOfLand {
	...
	public static PECASZone createTaz(int zoneIndex) {
		...
		if (zoneIndex >= zones.length || zoneIndex < 0) {
			throw new Error(
					"Need to index zones consecutively within the allocated array size");
		}
		if (zones[zoneIndex] != null) {
			throw new Error("Attempt to create zone with index" + zoneIndex
					+ " more than once");
		}
		return new PECASZone(zoneIndex, zoneIndex, "", false);
	}

	public static PECASZone createTaz(int zoneIndex, int zoneUserNumber,
			String zoneName, boolean external) {
		final AbstractZone zones[] = getAllZones();
		if (zoneIndex >= zones.length || zoneIndex < 0) {
			throw new Error(
					"Need to index zones consecutively within the allocated array size");
	...
	public static void setUpZones(TableDataSet ztab) {
		...
		if (externalColumn == -1) {
			logger
					.warn("No External column in PECASZonesI -- flows will be allowed between all zone pairs and histograms will include all zones");
		}
		else {
			logger
					.info("External column found in PECASZonesI -- flows will be disallowed between pairs of external zones");
		}
	...
	/**
	 * The buyingCommodityZUtilities is a table of the buying ZUtilities for this
	 * zone. The items are stored in a hastable, so they can be looked up by the
	 ...
	 * double unnamed = theZutilityOfACommodityInAZone.getUtility(); if (selling)
	 ...
	 * TravelUtilityCalculatorInterface tp, boolean withRouteChoice) { // message
	 * #2.5.1 to
	 ...
	 * ); if (selling) { return ((CommodityZUtility)sellingCommodityZUtilities.get
	 ...
	 * commodities Commodity[] coms = new Commodity[numTestComs]; for (int i = 0;
	 * i < numTestComs; i++) { coms[i] =
	 ...
	 * coms[i].setDefaultSellingDispersionParameter(new Parameter(1.0)); } // set
	 * up BuyingZUtilities, SellingZUtilities and Exchanges
	 * 
	 ...
	 * szu.addExchange(xc); } } for (int z = 0; z < numTestZones; z++) { PECASZone
	 * t = (PECASZone)allZonesCopy[z]; if (c < numTestComs / 2) {
	 ...
	 * GridCell(allZonesCopy[z], (float)10.0, dt1, (float) 10000.0, Math.random()
	 * * 50, zs); // 10 acres, 10000 // sq ft of space, up to 50 years old gc =
	 * new GridCell(allZonesCopy[z], (float) 10.0, dt2, (float) 10000.0,
	 * Math.random() * 50, zs); // 10 acres, 10000 sq ft of space, up to 50 years
	 * old } } }
	 */
