        ...
        implements UnitOfLand
{
    ...
    /**
     * The buyingCommodityZUtilities is a table of the buying ZUtilities for this zone. The items are stored in a hastable, so they can be looked up
     ...
     */
    /*
     * public double calcZUtility(Commodity c, boolean selling) throws ChoiceModelOverflowException { // message #1.2.3.1.3.1 to
     * theZutilityOfACommodityInAZone:com.pb.models.pecas.CommodityZutility // double unnamed = theZutilityOfACommodityInAZone.getUtility(); if
     * (selling) { return ((Alternative) sellingCommodityZUtilities.get(c)).getUtility(1); } else { return ((Alternative)
     * buyingCommodityZUtilities.get(c)).getUtility(1); } }
     */
     ...
     */
    /*
     * public double calcZUtilityForPreferences(Commodity c, boolean selling, TravelUtilityCalculatorInterface tp, boolean withRouteChoice) { //
     * message #2.5.1 to zUtilityOfLabourOrConsumption:com.pb.models.pecas.CommodityZutility // double unnamed =
     * zUtilityOfLabourOrConsumption.calcUtilityForPreferences(TravelPreferences ); if (selling) { return
     * ((CommodityZUtility)sellingCommodityZUtilities.get (c)).calcUtilityForPreferences(tp, withRouteChoice); } else { return ((CommodityZUtility
     * )buyingCommodityZUtilities.get(c)).calcUtilityForPreferences(tp, withRouteChoice); } }
     */

    /*
     * public static void createTestTazAndExchange(ZoningSchemeInterface zs) { final int numTestZones = 10; final int numTestComs = 4; final int
     * numGridCellsPerTypePerZone = 24; // set up zones AbstractZone.createTazArray(numTestZones); for (int i = 0; i < numTestZones; i++) { PECASZone
     * t = new PECASZone(i,i,""); } // set up commodities Commodity[] coms = new Commodity[numTestComs]; for (int i = 0; i < numTestComs; i++) {
     * coms[i] = Commodity.createOrRetrieveCommodity(String.valueOf(i),'a'); coms[i].setSellingUtilityCoefficients(1.0, 1.0, 1.0);
     * coms[i].setBuyingUtilityCoefficients(1.0,1.0,1.0); coms[i].setDefaultBuyingDispersionParameter(new Parameter(1.0));
     * coms[i].setDefaultSellingDispersionParameter(new Parameter(1.0)); } // set up BuyingZUtilities, SellingZUtilities and Exchanges
     * 
     * Commodity.setUpExchangesAndZUtilities(""); AbstractZone[] allZonesCopy = AbstractZone.getAllZones(); /* change this section of code to match
     * RunPA for (int c = 0; c < numTestComs; c++) { for (int z = 0; z < numTestZones; z++) { SellingZUtility szu = new SellingZUtility(coms[c],
     * (PECASZone)allZonesCopy[z]); // constructor has to set up links in both directions BuyingZUtility bzu = new BuyingZUtility(coms[c],
     * (PECASZone)allZonesCopy[z]); szu.setDispersionParameter(0.2); bzu.setDispersionParameter(0.2); Exchange xc = new Exchange(coms[c],
     * (PECASZone)allZonesCopy[z]); if (c < numTestComs / 2) // half of commodities exchanged in consumption zone { bzu.addExchange(xc); } else {
     * szu.addExchange(xc); } } for (int z = 0; z < numTestZones; z++) { PECASZone t = (PECASZone)allZonesCopy[z]; if (c < numTestComs / 2) {
     * CommodityZUtility czu = (CommodityZUtility)t.sellingCommodityZUtilities.get(coms[c]); czu.addAllExchanges(); } else { CommodityZUtility czu =
     * (CommodityZUtility)t.buyingCommodityZUtilities.get(coms[c]); czu.addAllExchanges(); } } } /* end of code section to change to match RunPA
     * 
     * 
     * 
     * //set up grid cells; Iterator it = zs.allowedDevelopmentTypes(); DevelopmentTypeInterface dt1 = (DevelopmentTypeInterface) it.next();
     * DevelopmentTypeInterface dt2 = (DevelopmentTypeInterface) it.next(); for (int z = 0; z < numTestZones; z++) { for (int g = 0; g <
     * numGridCellsPerTypePerZone; g++) { GridCell gc = new GridCell(allZonesCopy[z], (float)10.0, dt1, (float) 10000.0, Math.random() * 50, zs); //
     * 10 acres, 10000 // sq ft of space, up to 50 years old gc = new GridCell(allZonesCopy[z], (float) 10.0, dt2, (float) 10000.0, Math.random() *
     * 50, zs); // 10 acres, 10000 sq ft of space, up to 50 years old } } }
     */

    /*
     * public void addSellingZUtility(SellingZUtility aSellingCommodityZUtility, Commodity com) { sellingCommodityZUtilities.put(com,
     * aSellingCommodityZUtility); }
     */

    /*
     * public void addBuyingZUtility(BuyingZUtility aBuyingCommodityZUtility, Commodity com) { buyingCommodityZUtilities.put(com,
     ...
     */
    /*
     * public Hashtable getSellingCommodityZUtilities() { return sellingCommodityZUtilities; }
     */
     ...
     */
    /*
     * public Hashtable getBuyingCommodityZUtilities() { return buyingCommodityZUtilities; }
     */
