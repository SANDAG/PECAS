...
public class GridCellSynthesizer {
	...
	public static void main(String[] args) {
		final ResourceBundle ldRb = ResourceUtil.getResourceBundle("synth");
		final GridCellSynthesizer me = new GridCellSynthesizer();
		me.initialize(ldRb);
	 ...
	 * lookup table AmountOfDevelopment - empl. spaces, acres or sqft depending on
	 * devtype. YearBuilt - year of development ZoningScheme - 1 of 9 categories,
	 * see lookup table.
	 */
	...
	TableDataSetIndexedValue minMaxLookup = new TableDataSetIndexedValue(
			"IntensityRangeI", new String[] { "DevelopmentType" }, new String[0],
			new String[1][1], new int[1][0], "MinIntensity");
	...
	public void initialize(ResourceBundle ldRb) {
		...
		// check to see if the Employment, Residential Acres and Land table
		// should be read using
		// using a JDBC reader or CSVFileReader. (By default, use CSVFileReader)
		final String readerType = ldRb.getString("reader.type");

		TableDataReader myReader = null;

		if (readerType.equalsIgnoreCase("jdbc")) {
			final JDBCConnection myConnection = new JDBCConnection(
					ldRb.getString("land.database"), ldRb.getString("land.jdbcDriver"),
			...
			((CSVFileWriter) myWriter).setMyDecimalFormat(new GeneralDecimalFormat(
					"0.#########E0", 10000000, .001));
		}
		...
		if (readerType.equalsIgnoreCase("jdbc")) {
			...
			final CSVFileReader referenceReader = new CSVFileReader();
		...
		final TableDataSet zoningSchemesTable = referenceData
				.getTableDataSet("ZoningSchemesI");
		final int codeColumn = zoningSchemesTable
				.checkColumnPosition("ZoningSchemeCode");
		final int nameColumn = zoningSchemesTable
				.checkColumnPosition("ZoningScheme");
		zoningSchemeCodes = new TreeMap();

		for (int i = 1; i <= zoningSchemesTable.getRowCount(); i++) {
			zoningSchemeCodes.put(
					new Integer((int) zoningSchemesTable.getValueAt(i, codeColumn)),
					zoningSchemesTable.getStringValueAt(i, nameColumn));
		}

		zoningSchemeSequentialLookupFromCode = new TreeMap();
		zoningSchemeCodeLookupFromSequential = new int[zoningSchemeCodes.keySet()
				.size()];
		final Iterator it = zoningSchemeCodes.keySet().iterator();
		int number = 0;
		while (it.hasNext()) {
		...
		// devTypes are numbered 1-12, the associated zoning codes are
		// associated with indices 1-12.
		// devTypeZoningCorrespondence = new
		// int[]{0,1,2,5,4,5,7,7,8,9,12,12,12};

		yrBuiltProbTable = landData.getTableDataSet(ldRb
				.getString("year.built.probabilities"));
	...
	void buildZoningChoices() {
		final int zoneColumn = zoningTDSxTAZ.checkColumnPosition("TAZ");
		final int dTypeColumn = zoningTDSxTAZ
				.checkColumnPosition("DevelopmentType");
		zoningChoices = new HashMap<String, ObservedChoiceProbabilities>();
		final String[][] zoningStringArray = new String[1][1];
		final TableDataSetIndexedValue zoningCodeLookup = new TableDataSetIndexedValue(
				"ZoningSchemesI", new String[] { "ZoningScheme" }, new String[0],
				zoningStringArray, new int[1][0], "ZoningSchemeCode");
		zoningCodeLookup.setValueMode(TableDataSetIndexedValue.AVERAGE_MODE);
		for (int r = 1; r <= zoningTDSxTAZ.getRowCount(); r++) {
			final String devTypeName = zoningTDSxTAZ.getStringValueAt(r, dTypeColumn);
			final Integer dtInt = developmentTypeCodes.get(devTypeName);
			if (dtInt == null) {
				logger.fatal("Development type name " + devTypeName
						+ " is invalid in ZoningProbabilityTables");
				throw new RuntimeException("Development type name " + devTypeName
						+ " is invalid in ZoningProbabilityTables");
			}
			final int devType = dtInt.intValue();
			final String devTypeString = String.valueOf(devType);
			final int taz = (int) zoningTDSxTAZ.getValueAt(r, zoneColumn);
			ObservedChoiceProbabilities probs = zoningChoices.get(taz + "&"
					+ devTypeString);
			if (probs == null) {
			...
			for (int c = 1; c <= zoningTDSxTAZ.getColumnCount(); c++) {
				if (c != zoneColumn && c != dTypeColumn
						&& !zoningTDSxTAZ.getColumnLabel(c).equalsIgnoreCase("DefaultTAZ")) {
					final String zoningName = zoningTDSxTAZ.getColumnLabel(c);
					zoningStringArray[0][0] = zoningName;
					zoningCodeLookup.setStringValues(zoningStringArray);
					final int zoningCode = (int) zoningCodeLookup
							.retrieveValue(referenceData);
					final ZoningAlternative alt = new ZoningAlternative(zoningCode,
							zoningTDSxTAZ.getValueAt(r, c));
					probs.addAlternative(alt);
				}
			}
		}

		for (final ObservedChoiceProbabilities p : zoningChoices.values()) {
	...
	class LandCategory {
		 ...
		 * initialize it yet. You'll need to call LandCategory.initialize(...) later
		 * 
		 * @param code
		...
		/**
		 * Use this constructor to create a LandCategory and assign a quantity of
		 * observed land based on a TableDataSet, but not yet initialize it. Call
		 * LandCategory.initialize(...) later.
		 * 
		 * @param code
		 *          the code representing the development type
		 * @param landTdis
		 *          the TableDataSetIndexedValue where the retrieveValue() will get
		 ...
		 *          the code representing the development type
		 * @param inventoryTdis
		 *          used to retrieve the inventory of space to be applied to the
		 *          land (column name already set)
		 * @param landTdis
		 *          used to retrieve the quantity of land on which the inventory is
		 *          to be applied, using column names specified in landCategories
		 ...
		 * @param serviceLevel
		 *          the service level to be applied per unit of inventory.
		 */
		LandCategory(char code, TableDataSetIndexedValue inventoryTdis,
				TableDataSetIndexedValue landTdis, String[] landCategories,
				String myTitle) {
			title = myTitle;
			...
			this.code = code;
			final float space = inventoryTdis.retrieveValue(landData);
			final String[][] stringValues = new String[1][1];
			stringValues[0][0] = names[code];
			minMaxLookup.setStringValues(stringValues);
			minMaxLookup.setMyFieldName("ServiceLevel");
			final float serviceLevel = minMaxLookup.retrieveValue(landData);
			minMaxLookup.setMyFieldName("MinIntensity");
			final float minIntens = minMaxLookup.retrieveValue(landData);
			minMaxLookup.setMyFieldName("MaxIntensity");
			final float maxIntens = minMaxLookup.retrieveValue(landData);
		 ...
		 * @param landTdis
		 *          used to retrieve the quantity of land on which the inventory is
		 *          to be applied (column name already set)
		 */
		LandCategory(char code, TableDataSetIndexedValue inventoryTdis,
		 ...
		 *          the code representing the development type
		 * @param landTdis
		 *          landTdis used to retrieve the quantity of land on which the
		 *          inventory is to be applied, using column names specified in
		 *          landCategories
		 * @param landCategories
		 *          a list of the land categories to be used.
		 */
		public LandCategory(char c, TableDataSetIndexedValue landTdis,
				String[] landCategories, String myTitle) {
			title = myTitle;
			observedLand = 0;
			for (int i = 0; i < landCategories.length; i++) {
				landTdis.setMyFieldName(landCategories[i]);
		...
		void initialize(String name, float spaceParam, double minIntensity,
				double maxIntensity, double serviceLevel) {
			...
			if (intensity < minIntensity) {
				intensity = minIntensity; // the intensity is now set at what we
				// want.
			...
			gridCounts.setValueAt(zoneRow, gridCounts.checkColumnPosition(name),
					cells);
			intensities.setValueAt(zoneRow, gridCounts.checkColumnPosition(name),
					(float) intensity);
		}
		 ...
		 * percentage of total available cells to borrow. If there are no cells to
		 * borrow, the method will return and the intensities will stay as they are.
		 */
		private double borrowSomeAgForestCells() {

			final float agLand = landCategories[0].observedLand;
			final float forLand = landCategories[1].observedLand;
			double landToBorrow = space / intensity - observedLand; // remember
			// that the
			// intensity
			// has now
			// been set
			...
			if (code != 50) {
				...
				if (landToBorrow > agLand + forLand) {
					logger
							.warn("Not enough Agriculture land and Forest land to borrow for "
									+ toString());
					landToBorrow = agLand + forLand;
				}
				landCategories[0].observedLand -= agLand / (agLand + forLand)
						* landToBorrow;
				landCategories[1].observedLand -= forLand / (agLand + forLand)
						* landToBorrow;
				return landToBorrow;
			}
			else { // just try to borrow Forest
				if (forLand == 0) {
					logger.warn("No Forest land to borrow for agricultural use in "
							+ toString());
					return 0;
				}
				if (landToBorrow > forLand) {
					logger
							.warn("Not enough Forest land to borrow for agricultural use in "
									+ toString());
		...
		private boolean createSomeVacantCells() {
			...
			final double amtToMakeVacant = cells * gridCellSize - space / intensity;

			vacantCells = (int) Math.round(amtToMakeVacant / gridCellSize);

			if (vacantCells > cells || vacantCells <= 0) {
				return false; // TODO - log this as a potential data problem or
				// rounding issue.
			}

			// TODO the new vacant cells should have a zoning consistent with
			// what we thought their use was
	...
	public void synthesizeGridCells() {
		...
		// can't use the indexing function built into TableDataSet because the
		// TAZ numbers are way too huge, would eat up to much memory
		// instead use TableDataSetIndexedValue
		final int[][] keyValues = new int[1][1];
		final String[][] stringKeyValues = new String[1][0];
		final String[] stringKeyColumns = new String[0];
		final String[] tazKeyColumn = new String[] { "TAZ" };
		final String[] inventoryKeyColumn = new String[] { "FloorspaceZone" };

		// ATTENTION: It matters in the code below that vacant is the last
		// column!!
		final String[] resultColumnNames = new String[] { "TAZ", "Ag", "ForestPro",
				"Office", "HvyInd", "LtInd", "CommRetAccom", "K12", "GovtInst", "Mil",
				"ResUHi", "ResUMed", "ResULow", "ResRHi", "ResRLow", "Vac", };
		gridCounts = TableDataSet.create(
				new float[landQuantityTable.getRowCount()][resultColumnNames.length],
				resultColumnNames);
		gridCounts.setName("gridcounts");
		intensities = TableDataSet.create(
				new float[landQuantityTable.getRowCount()][resultColumnNames.length],
				resultColumnNames);
		intensities.setName("intensities");

		for (zoneRow = 1; zoneRow <= landQuantityTable.getRowCount(); zoneRow++) {
			taz = (int) landQuantityTable.getValueAt(zoneRow,
					landQuantityTable.checkColumnPosition("TAZ"));
			keyValues[0][0] = taz;
			int cellsInTAZ = 0;
			...
			// I made landCategories static so that the LandCategory class can
			// see (and change) the amount of Forest/Protected Cells.
			landCategories = new LandCategory[resultColumnNames.length - 1];

			// Get the Land database table.
			final TableDataSetIndexedValue landTdiv = new TableDataSetIndexedValue(
					landQuantityTableName, stringKeyColumns, tazKeyColumn,
					stringKeyValues, keyValues, "Agricultural Land");
			landTdiv.setErrorOnMissingValues(true);

			// Initially for Ag and For/Protected we are just going to get the
			// observed land. Once we finish with the
			// other devTypes (that may borrow some or all of the AG and
			// FOREST/PROTECTED LAND, then we will figure
			// out the intensity of the Ag land and borrow any remaining
			// FOREST/PROTECTED if the AG intensities are way too high or create
			// vacant AG
			// if the AG intensities are too low. We will also calculated the
			// number of cells.
			// ATTENTION: The order of these categories matters!!

			// ag - devType 50
			landCategories[0] = new LandCategory((char) 50, landTdiv, "Ag in " + taz); // this
																																									// constructors
																																									// will
																																									// set
																																									// the
																																									...
																																									// code
																																									// and
			// the observed land total

			// forest/protected and Airport/Port - devType60
			String[] landCategoryStrings = new String[] {
					"Forestry and protected land", "Airport/Port Land" };
			landCategories[1] = new LandCategory((char) 60, landTdiv,
					landCategoryStrings, "Forestry/Protected/Airport in " + taz);
			// old way without airports landCategories[1] = new
			// LandCategory((char) 60,landTdiv); //this constructor will set the
			// devType code and the observed land total.

			landTdiv.setMyFieldName("Vacant land");
			landCategories[14] = new LandCategory((char) 82, "Vacant in " + taz);

			// Now we need the Inventory table because we are going to start
			// populating the grid.
			// Get floorspace inventory table for non-residential lands
			final TableDataSetIndexedValue inventoryTdiv = new TableDataSetIndexedValue(
					inventoryTableName, stringKeyColumns, inventoryKeyColumn,
					stringKeyValues, keyValues, "Office_SFB");
			inventoryTdiv.setErrorOnMissingValues(true);

			// office - devType22
			inventoryTdiv.setMyFieldName("Office_SFB");
			landTdiv.setMyFieldName("Office Land");
			landCategories[2] = new LandCategory((char) 22, inventoryTdiv, landTdiv,
					"Office in " + taz);

			// heavy industry land - devType32
			inventoryTdiv.setMyFieldName("HvyInd_ES");
			landTdiv.setMyFieldName("Heavy industrial Land");
			landCategories[3] = new LandCategory((char) 32, inventoryTdiv, landTdiv,
					"HvyIndustry in " + taz);

			// light industry land - devType31
			inventoryTdiv.setMyFieldName("LtInd_ES");
			landTdiv.setMyFieldName("Light industrial Land");
			landCategories[4] = new LandCategory((char) 31, inventoryTdiv, landTdiv,
					"Light Industry in " + taz);

			// commercial retail- devType21
			inventoryTdiv.setMyFieldName("CommRetAccom_SFB");
			landCategoryStrings = new String[] { "Commercial Land", "Retail Land",
					"Accommodation" };
			landCategories[5] = new LandCategory((char) 21, inventoryTdiv, landTdiv,
					landCategoryStrings, "CommRetAccom in " + taz);

			// K12 land - devType23
			inventoryTdiv.setMyFieldName("K12_ES");
			landTdiv.setMyFieldName("K-12 school land");
			landCategories[6] = new LandCategory((char) 23, inventoryTdiv, landTdiv,
					"K12 in " + taz);
			//
			// //govtInst - devType24
			inventoryTdiv.setMyFieldName("GovtInst_SFB");
			landCategoryStrings = new String[] { "Government Land",
					"Higher Educ Land", "Healthcare Institution Land" };
			landCategories[7] = new LandCategory((char) 24, inventoryTdiv, landTdiv,
					landCategoryStrings, "GovInst in " + taz);

			// military - devType40
			inventoryTdiv.setMyFieldName("Military_Space");
			landTdiv.setMyFieldName("Military base land");
			landCategories[8] = new LandCategory((char) 40, inventoryTdiv, landTdiv,
					"Military in " + taz);
			...
			landCategories[9] = new LandCategory((char) 11, inventoryTdiv, landTdiv,
					"ResRLow in " + taz);
			...
			landCategories[10] = new LandCategory((char) 12, inventoryTdiv, landTdiv,
					"ResRHi in " + taz);

			// Residential Urban Low - devType15
			inventoryTdiv.setMyFieldName("ResULow_SQFT");
			landTdiv.setMyFieldName("Low");
			landCategories[11] = new LandCategory((char) 15, inventoryTdiv, landTdiv,
					"ResULow in " + taz);

			// Residential Urban Medium - devType16
			inventoryTdiv.setMyFieldName("ResUMed_SQFT");
			landTdiv.setMyFieldName("Med");
			landCategories[12] = new LandCategory((char) 16, inventoryTdiv, landTdiv,
					"ResUMed in " + taz);

			// Residential Urban high - devType17
			inventoryTdiv.setMyFieldName("ResUHi_SQFT");
			landTdiv.setMyFieldName("High");
			landCategories[13] = new LandCategory((char) 17, inventoryTdiv, landTdiv,
					"ResUHi in " + taz);

			// Get the Ag land and Ag floorspace and calculate cells and
			// intensity
			inventoryTdiv.setMyFieldName("Ag_ES");
			final float agSpace = inventoryTdiv.retrieveValue();
			...
			final String[] id1Array = new String[cellsInTAZ];
			final String[] id2Array = new String[cellsInTAZ];
			final float[] landSizeArray = new float[cellsInTAZ];
			final int[] devTypeArray = new int[cellsInTAZ];
			final float[] quantityArray = new float[cellsInTAZ];
			final int[] yearBuiltArray = new int[cellsInTAZ];
			final int[] zoningArray = new int[cellsInTAZ];
			final float[] servicing = new float[cellsInTAZ];
			final int[] serviceCostCode = new int[cellsInTAZ];
			final float[] constructionCostMultiplier = new float[cellsInTAZ];

			int row = 0;

			// First see if we need to adjust the probabilities to match the
			// total quantity of zoned land

			final double[][] balances = new double[landCategories.length][zoningSchemeCodes
					.size()];
			final double[][] balancingTargets = new double[2][];
			...
			for (int d = 0; d < landCategories.length - 1; d++) {
				final int probabilityTableToUse = (int) zoningProbabilityMap
						.getIndexedValueAt(taz, 2);
				final ObservedChoiceProbabilities p = zoningChoices
						.get(probabilityTableToUse + "&" + landCategories[d].getCode());
				if (p == null) {
					throw new RuntimeException("No probability table for zone "
							+ probabilityTableToUse + " and development type "
							+ landCategories[d].getCode());
				}
				final ArrayList alts = p.getAlternatives();
				final double[] probs = p.getProbabilities();
				for (int alt = 0; alt < alts.size(); alt++) {
					final ZoningAlternative zAlt = (ZoningAlternative) alts.get(alt);
					final int zoningCode = zAlt.getCode();
					final Integer sequentialScheme = (Integer) zoningSchemeSequentialLookupFromCode
							.get(new Integer(zoningCode));
					if (sequentialScheme == null) {
						logger.error("No zoning code " + zoningCode + " in ZoningSchemes");
						throw new RuntimeException("No zoning code " + zoningCode
								+ " in ZoningSchemes");
					}
					final int col = sequentialScheme.intValue();
					balances[d][col] += landCategories[d].cells * gridCellSize
							* probs[alt];
			...
			final int[][] zoningAcresKeys = new int[1][1];
			zoningAcresKeys[0][0] = taz;
			NDimensionalMatrixDouble balancesMatrix = new NDimensionalMatrixDouble(
					"balancingAct", balances);
			try {
				final TableDataSetIndexedValue zoningAcres = new TableDataSetIndexedValue(
						"LandAcresxZoningI", new String[0], new String[] { "TAZ" },
						new String[1][0], zoningAcresKeys, "ACRES");
				zoningAcres.setErrorOnMissingValues(true);

				final Iterator it = zoningSchemeCodes.keySet().iterator();
				while (it.hasNext()) {
					final Integer schemeCode = (Integer) it.next();
					final String schemeName = (String) zoningSchemeCodes.get(schemeCode);
					zoningAcres.setMyFieldName(schemeName);
					final double value = zoningAcres.retrieveValue(landData);
					final int col = ((Integer) zoningSchemeSequentialLookupFromCode
							.get(schemeCode)).intValue();
					balancingTargets[1][col] = value;
				}
				final NDimensionalMatrixBalancerDouble balancer = new NDimensionalMatrixBalancerDouble(
						balancesMatrix, balancingTargets);
				balancer.balance();
			...
			final double[] rowTotals = balancesMatrix.collapseToVectorAsDouble(0);

			for (int d = 0; d < landCategories.length - 1; d++) { // We will
				// treat
				// "VACANT"
				// that is
				// the
				...
				for (int n = 0; n < landCategories[d].cells; n++) {
					...
					if (zoning >= zoningSchemeCodeLookupFromSequential.length) {
						logger.error("Unrealistic zoned land totals for TAZ " + taz
								+ " Applying unbalanced zoning for devtype "
								+ names[landCategories[d].getCode()]);
						final int probabilityTableToUse = (int) zoningProbabilityMap
								.getIndexedValueAt(taz, 2);
						final ObservedChoiceProbabilities p = zoningChoices
								.get(probabilityTableToUse + "&" + landCategories[d].getCode());
					...
					// TODO if intensity is sampled than servicing needs to be
					// sampled too
					servicing[row] = (float) (landCategories[d].servicesPerUnit
							* landCategories[d].intensity * gridCellSize);
				...
				for (int v = 0; v < landCategories[d].vacantCells; v++) {
					...
					yearBuiltArray[row] = 1990; // default for vacant land is
					// 1990
					final int probabilityTableToUse = (int) zoningProbabilityMap
							.getIndexedValueAt(taz, 2);
					final ObservedChoiceProbabilities p = zoningChoices
							.get(probabilityTableToUse + "&"
									+ String.valueOf(devTypeArray[row]));
					final ZoningAlternative z = (ZoningAlternative) p.chooseAlternative();
					zoningArray[row] = z.getCode(); // ZoningScheme
					// TODO if intensity is ampled than servicing needs to be
					// sampled too
			...
			final TableDataSet gridCellDataTable = new TableDataSet();
			...
			gridCellDataTable.appendColumn(constructionCostMultiplier,
					"ConstructFactor");
	...
	private int selectYearBuilt(int zoneNumber) throws RuntimeException {
		...
		// take all elements of the row except the first and second element
		// which is the
		// TAZ number and the TAZName. Check that the probs add up to 1 while
		// you are at it.
		final float[] row = yrBuiltProbTable.getIndexedRowValuesAt(zoneNumber); // returns
		// row
		// values
		// starting
		// in
		// position
		// 0
		final float[] probabilities = new float[row.length - 2];
		float sum = 0.0f;
		for (int i = 0; i < row.length - 2; i++) {
			probabilities[i] = row[i + 2]; // skip the TAZ and TAZName column.
			sum += probabilities[i]; // I am anticipating some rounding issues
			// so will
			// throw runtime if rounding error is
			// greater than .05 (meaning sum
			// is 99.95 or less instead of 1.
		}
		if (1.0 - sum >= 0.5) {
			throw new RuntimeException("Probability Sum is  " + sum
					+ " - check data - should add to 1.0");
		}

		final int index = DistributionSelector
				.getMonteCarloSelection(probabilities);
		// if(logger.isDebugEnabled()) logger.debug("index chosen was " +
		// index);
		final String intervalHeader = yrBuiltProbTable.getColumnLabel(index + 3); // the
																																							// index
																																							// starts
																																							// at
																																							// 1
																																							// and
																																							// doesn't
		// take into account the TAZ or
		// TAZName column in the table.

		// if(logger.isDebugEnabled()) logger.debug("Interval Header is " +
		// intervalHeader);
		final double lowerBound = Double.parseDouble(intervalHeader.substring(0,
				intervalHeader.indexOf('-')));
		final double upperBound = Double.parseDouble(intervalHeader
				.substring(intervalHeader.indexOf('-') + 1));

		yrBuilt = (int) DistributionSelector.selectFromUniformDistribution(
