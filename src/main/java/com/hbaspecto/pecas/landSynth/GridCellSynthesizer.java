...
public class GridCellSynthesizer
{
    ...
    public void initialize(ResourceBundle ldRb)
    {
        ...
        {
            final JDBCConnection myConnection = new JDBCConnection(ldRb.getString("land.database"),
                    ldRb.getString("land.jdbcDriver"), ResourceUtil.getProperty(ldRb,
                            "land.databaseUser", ""), ResourceUtil.getProperty(ldRb,
                            "land.databasePassword", ""));
        ...
        } else
        {
            ...
            ((CSVFileWriter) myWriter).setMyDecimalFormat(new GeneralDecimalFormat("0.#########E0",
                    10000000, .001));
        }
        ...
        final TableDataSet zoningSchemesTable = referenceData.getTableDataSet("ZoningSchemesI");
        final int codeColumn = zoningSchemesTable.checkColumnPosition("ZoningSchemeCode");
        final int nameColumn = zoningSchemesTable.checkColumnPosition("ZoningScheme");
    ...
    void buildZoningChoices()
    {
        ...
        final int dTypeColumn = zoningTDSxTAZ.checkColumnPosition("DevelopmentType");
        ...
        final TableDataSetIndexedValue zoningCodeLookup = new TableDataSetIndexedValue(
                "ZoningSchemesI", new String[] {"ZoningScheme"}, new String[0], zoningStringArray,
                new int[1][0], "ZoningSchemeCode");
        ...
        for (int r = 1; r <= zoningTDSxTAZ.getRowCount(); r++)
        {
            ...
            for (int c = 1; c <= zoningTDSxTAZ.getColumnCount(); c++)
            {
                        ...
                        && !zoningTDSxTAZ.getColumnLabel(c).equalsIgnoreCase("DefaultTAZ"))
                {
                    ...
                    final int zoningCode = (int) zoningCodeLookup.retrieveValue(referenceData);
    ...
    class LandCategory
    {
        ...
        LandCategory(char code, TableDataSetIndexedValue inventoryTdis,
                TableDataSetIndexedValue landTdis, String[] landCategories, String myTitle)
        {
            title = myTitle;
         ...
         *            a list of the land categories to be used.
         */
        public LandCategory(char c, TableDataSetIndexedValue landTdis, String[] landCategories,
                String myTitle)
        {
            title = myTitle;
            observedLand = 0;
        ...
        void initialize(String name, float spaceParam, double minIntensity, double maxIntensity,
                double serviceLevel)
        {
            ...
            intensities
                    .setValueAt(zoneRow, gridCounts.checkColumnPosition(name), (float) intensity);
        }
        ...
        private double borrowSomeAgForestCells()
        {
            ...
            if (code != 50)
            {
                ...
                {
                    logger.warn("Not enough Agriculture land and Forest land to borrow for "
                            + toString());
            ...
            { // just try to borrow Forest
                if (forLand == 0)
                {
                    logger.warn("No Forest land to borrow for agricultural use in " + toString());
                    return 0;
                }
                if (landToBorrow > forLand)
                {
                    logger.warn("Not enough Forest land to borrow for agricultural use in "
                            + toString());
    ...
    public void synthesizeGridCells()
    {
        ...
        // column!!
        final String[] resultColumnNames = new String[] {"TAZ", "Ag", "ForestPro", "Office",
                "HvyInd", "LtInd", "CommRetAccom", "K12", "GovtInst", "Mil", "ResUHi", "ResUMed",
                "ResULow", "ResRHi", "ResRLow", "Vac",};
        ...
        for (zoneRow = 1; zoneRow <= landQuantityTable.getRowCount(); zoneRow++)
        {
            ...
            final TableDataSetIndexedValue landTdiv = new TableDataSetIndexedValue(
                    landQuantityTableName, stringKeyColumns, tazKeyColumn, stringKeyValues,
                    keyValues, "Agricultural Land");
            ...
            String[] landCategoryStrings = new String[] {"Forestry and protected land",
                    "Airport/Port Land"};
            landCategories[1] = new LandCategory((char) 60, landTdiv, landCategoryStrings,
                    "Forestry/Protected/Airport in " + taz);
            ...
            final TableDataSetIndexedValue inventoryTdiv = new TableDataSetIndexedValue(
                    inventoryTableName, stringKeyColumns, inventoryKeyColumn, stringKeyValues,
                    keyValues, "Office_SFB");
            inventoryTdiv.setErrorOnMissingValues(true);

            // office - devType22
            inventoryTdiv.setMyFieldName("Office_SFB");
            landTdiv.setMyFieldName("Office Land");
            landCategories[2] = new LandCategory((char) 22, inventoryTdiv, landTdiv, "Office in "
                    + taz);
            ...
            landCategories[6] = new LandCategory((char) 23, inventoryTdiv, landTdiv, "K12 in "
                    + taz);
            //
            // //govtInst - devType24
            inventoryTdiv.setMyFieldName("GovtInst_SFB");
            landCategoryStrings = new String[] {"Government Land", "Higher Educ Land",
                    "Healthcare Institution Land"};
            ...
            landCategories[8] = new LandCategory((char) 40, inventoryTdiv, landTdiv, "Military in "
                    + taz);
            ...
            landCategories[9] = new LandCategory((char) 11, inventoryTdiv, landTdiv, "ResRLow in "
                    + taz);
            ...
            landCategories[10] = new LandCategory((char) 12, inventoryTdiv, landTdiv, "ResRHi in "
                    + taz);

            // Residential Urban Low - devType15
            inventoryTdiv.setMyFieldName("ResULow_SQFT");
            landTdiv.setMyFieldName("Low");
            landCategories[11] = new LandCategory((char) 15, inventoryTdiv, landTdiv, "ResULow in "
                    + taz);

            // Residential Urban Medium - devType16
            inventoryTdiv.setMyFieldName("ResUMed_SQFT");
            landTdiv.setMyFieldName("Med");
            landCategories[12] = new LandCategory((char) 16, inventoryTdiv, landTdiv, "ResUMed in "
                    + taz);

            // Residential Urban high - devType17
            inventoryTdiv.setMyFieldName("ResUHi_SQFT");
            landTdiv.setMyFieldName("High");
            landCategories[13] = new LandCategory((char) 17, inventoryTdiv, landTdiv, "ResUHi in "
                    + taz);
            ...
            final double[][] balances = new double[landCategories.length][zoningSchemeCodes.size()];
            ...
            {
                final int probabilityTableToUse = (int) zoningProbabilityMap.getIndexedValueAt(taz,
                        2);
                final ObservedChoiceProbabilities p = zoningChoices.get(probabilityTableToUse + "&"
            ...
            NDimensionalMatrixDouble balancesMatrix = new NDimensionalMatrixDouble("balancingAct",
                    balances);
            try
            {
                final TableDataSetIndexedValue zoningAcres = new TableDataSetIndexedValue(
                        "LandAcresxZoningI", new String[0], new String[] {"TAZ"}, new String[1][0],
                        zoningAcresKeys, "ACRES");
                ...
                while (it.hasNext())
                {
                    ...
                    final int col = ((Integer) zoningSchemeSequentialLookupFromCode.get(schemeCode))
                            .intValue();
            ...
            { // We will
              ...
              // differently.
                ...
                for (int v = 0; v < landCategories[d].vacantCells; v++)
                {
                    ...
                    // 1990
                    final int probabilityTableToUse = (int) zoningProbabilityMap.getIndexedValueAt(
                            taz, 2);
                    final ObservedChoiceProbabilities p = zoningChoices.get(probabilityTableToUse
    ...
    private int selectYearBuilt(int zoneNumber) throws RuntimeException
    {
        ...
        final int index = DistributionSelector.getMonteCarloSelection(probabilities);
        ...
        final double upperBound = Double.parseDouble(intervalHeader.substring(intervalHeader
                .indexOf('-') + 1));
