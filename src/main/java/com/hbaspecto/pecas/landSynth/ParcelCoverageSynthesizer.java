...
public class ParcelCoverageSynthesizer extends AssignSquareFeetToParcel
		implements Runnable {
	...
	public static void main(String[] args) {
		try {
			...
			logger
					.fatal("Can't read properties file, it should be on the command line");
		...
		final LoadingQueue<Integer> zoneNumberQueue = new LoadingQueue<Integer>(
				10000);
	...
	public static Collection<Integer> getZoneNumbers() {
		...
		try {
			...
			final ResultSet zoneSet = getSqFtStatement
					.executeQuery("SELECT distinct " + floorspaceZoneColumnName
							+ " FROM " + sqftInventoryTableName);
			while (zoneSet.next()) {
	...
	protected boolean putParcelsInArray(Integer taz) throws SQLException {
		...
		final Statement stmt = localConn.createStatement(
		...
		try {
			...
			logger
					.warn("Couldn't set the maxRows property in the database, manually check to see if all parcels are being processed.");
		}
		...
		if (bufferSize == 0) {
			...
			// column names
			result = stmt.executeQuery("SELECT * FROM " + parcelTableName
					+ " where \"" + parcelZoneColumnName + "\"=" + taz);
		}
		else {
			...
			final String query = "SELECT * FROM " + parcelTableName
					...
					+ "), (SELECT st_buffer(t." + tazGeomName + ", " + bufferSize
					+ ") FROM " + tazTableName + " t WHERE t.\"" + tazNumberName + "\"="
					+ taz + "))";
		...
		try {
			...
			while (result.next()) {
				...
				for (final List<ParcelInterface> aParcelArray : sortedParcelLists
						.values()) {
					// add it to each list
					aParcelArray.add(p);
				}
	...
	public void initialize() {
		...
		ParcelInMemory.spaceAmountColumnName = ResourceUtil.checkAndGetProperty(
				props, "SpaceAmountField");
		final boolean intSpaceTypeCode = ResourceUtil.getBooleanProperty(props,
				"IntegerSpaceTypeCode", true);
		if (intSpaceTypeCode) {
			ParcelInMemory.spaceTypeIntColumnName = ResourceUtil.checkAndGetProperty(
					props, "SpaceTypeField");
		}
		else {
			ParcelInMemory.spaceTypeStringColumnName = ResourceUtil
					.checkAndGetProperty(props, "SpaceTypeField");
		}
