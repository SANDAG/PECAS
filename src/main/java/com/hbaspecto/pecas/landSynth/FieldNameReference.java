...
public class FieldNameReference {
	...
	public Object getField(String fieldName, int[] intValues,
			String[] stringValues, double[] doubleValues, boolean[] booleanValues,
			long[] longValues) {
		if (integerFieldNames.contains(fieldName)) {
			return Integer
					.valueOf(intValues[integerFieldNames.lastIndexOf(fieldName)]);
		}
		else if (doubleFieldNames.contains(fieldName)) {
			return Double.valueOf(doubleValues[doubleFieldNames
					.lastIndexOf(fieldName)]);
		}
		else if (booleanFieldNames.contains(fieldName)) {
			return Boolean.valueOf(booleanValues[booleanFieldNames
					.lastIndexOf(fieldName)]);
		}
			...
			final String msg = "Can't find field name " + fieldName;
			logger.fatal(msg);
