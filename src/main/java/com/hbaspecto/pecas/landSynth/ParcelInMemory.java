 ...
 * 
 */
public class ParcelInMemory
        implements ParcelInterface
{
    ...
    {
        return "Parcel: " + getId() + ",Coverage type: " + getCoverage() + ", Qnty: "
                + getQuantity();
    }
    ...
    static Logger            logger                       = Logger.getLogger(ParcelInMemory.class
                                                                  .getName());
    ...
    {
        final Object fieldValue = fieldNameReference.getField(fieldName, intValues, stringValues,
                doubleValues, booleanValues, longValues);
        if (fieldValue == null)
        {
            return null;
        }
    ...
    private void getRegularValuesFromDatabase()
    {
        ...
        taz = (Integer) fieldNameReference.getField(zoneColumnName, intValues, stringValues,
                doubleValues, booleanValues, longValues);
        amountOfLand = ((Double) fieldNameReference.getField(landAreaColumnName, intValues,
                stringValues, doubleValues, booleanValues, longValues)).floatValue();
        amountOfSpace = ((Double) fieldNameReference.getField(spaceAmountColumnName, intValues,
                stringValues, doubleValues, booleanValues, longValues)).floatValue();
        initialFAR = 0;
        if (initialFARColumnName != null)
        {
            initialFAR = ((Double) fieldNameReference.getField(initialFARColumnName, intValues,
                    stringValues, doubleValues, booleanValues, longValues)).floatValue();
        }
        if (spaceTypeIntColumnName != null)
        {
            coverage = (Integer) fieldNameReference.getField(spaceTypeIntColumnName, intValues,
    ...
    public void putPecasFieldsBackToDatabase(ParcelUpdater updater)
    {
        ...
        } else
        {
            ...
            updater.requestUpdateStringCoverage(getId(), String.valueOf(coverageChar),
                    getQuantity());
        }
    }
    ...
    {
        final int index = coverageTypeLookupArray[intCoverageType - firstSpaceTypeID];
        return index;
            ...
            String sdMatchCoeffSpaceTypeTableName, String sdMatchCoeffSpaceTypeColName)
    {
        try
        {
            ...
            final Statement getSpaceTypesStmt = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY,
            ...
            ResultSet spaceTypesSet = getSpaceTypesStmt
                    .executeQuery("SELECT min(space_type_id) as min, max(space_type_id) as max, count(*) as count "
                            ...
                            + " as space_type_id FROM " + sdMatchCoeffSpaceTypeTableName + " ) p");
            ...
            spaceTypesSet = getSpaceTypesStmt.executeQuery(" SELECT distinct space_type_id "
                    + " FROM ( " + "SELECT distinct " + floorspaceSpaceTypeColName
                    + " as space_type_id FROM " + FloorspaceInventoryTable + " UNION "
                    + "SELECT distinct " + sdMatchCoeffSpaceTypeColName + " as space_type_id FROM "
                    + sdMatchCoeffSpaceTypeTableName + " ) p" + " ORDER BY space_type_id");
