...
public class MSSQLServerLandInventory extends SimpleORMLandInventory {
	...
	public MSSQLServerLandInventory(ResourceBundle rb, String landDatabaseDriver,
			...
			String logFileNameAndPath, String schema) throws SQLException {
	...
	public void applyDevelopmentChanges() {
		...
		try {
			...
			final String exactpath = ResourceUtil.checkAndGetProperty(rbSD,
					"Exactpath");
			...
			statement.execute("BULK INSERT development_events FROM '" + exactpath
					+ "DevelopmentEvents.csv" + "' WITH (FIRSTROW = 2,FORMATFILE='"
					+ exactpath + "development_events.xml');");

			logger
					.info("Now applying changes to parcel file. NOTE PSEUDOPARCELLING IS IMPLEMENTED");

			String strUpdate = "UPDATE "
					+ Parcels_gen.meta.getTableName()
					+ " "
					+ "SET	space_quantity   = d.new_space_quantity, "
					+ "       space_type_id    = d.new_space_type_id, "
					+ "       year_built       = d.new_year_built, "
					+ "       land_area        = d.land_area, "
					+ // to update the new area of the remaining parcel
					...
					+ "  	   AND (d.event_type = 'C' OR " + "			d.event_type = 'D' OR "
					+ "           d.event_type = 'A' OR "
					+ "           d.event_type = 'L' OR "
					+ "           d.event_type = 'R' );\n";

			strUpdate += "INSERT INTO " + Parcels_gen.meta.getTableName() + " "
					+ "    	SELECT  parcel_id, " + "		new_pecas_parcel_num, "
					+ "		new_year_built, " + "		taz, " + "		new_space_type_id, "
					+ "		new_space_quantity, " + "		land_area, "
					+ "		available_services, " + "		new_is_derelict, "
					+ "		new_is_brownfield " + "   	FROM development_events d "
					+ "    	WHERE " + "	     d.event_type = 'CS' "
					+ "	OR   d.event_type = 'AS' " + "	OR   d.event_type = 'RS' "
					+ "	OR   d.event_type = 'DS' " + "	OR   d.event_type = 'LS' "
					+ "   	; \n";
			...
			strUpdate += " INSERT INTO parcel_cost_xref "
					...
					+ " AND ( " + "     	 event_type = 'CS' "
					+ "	OR   event_type = 'AS' " + "	OR   event_type = 'RS' "
					+ "	OR   event_type = 'DS' " + "	OR   event_type = 'LS' ) " + "	; \n";
	...
	protected void createParcelsTemp(int year) {
		try {
			...
			statement.execute("SELECT fp.pecas_parcel_num as parcel_id, "
					+ // TODO: In Cali there is no parcel_id. We use
						// pecasParcelNum
					"fp.pecas_parcel_num, " + "fp.year_built, fp.taz, "
					+ "fp.space_type_id, " + "fp.space_quantity, " + "fp.land_area, "
					+ "fp.available_services_code, " + "zxref.zoning_rules_code, "
					+ "costxref.cost_schedule_id, " + "feexref.fee_schedule_id, "
					+ "fp.is_derelict, " + "fp.is_brownfield, "
					+ "CEILING((rand(fp.pecas_parcel_num))* "
					+ N
					+ " ) as randnum "
					+ "INTO parcels_temp "
					+ "FROM "
			...
			if (fetchParcelsByTaz) {
				...
				statement
						.execute("CREATE INDEX randnum_idx  ON parcels_temp (randnum)");
			}
			...
			throw new RuntimeException("Couldn't create temporary table in database",
					e);
		}
	}
	...
	public DevelopmentLog getDevelopmentLogger() {
		if (msSQLDevelopmentLogger != null) {
