...
public interface LandInventory {
	 ...
	 * no next object. If any of the methods with currentZone,id2 parameters have
	 * been called, the internal counter will have been changed to point to
	 ...
	 * 
	 * @param coverageCode
	 *          character representing the PECAS description of the current use
	 *          (building type)
	 */
	 ...
	 * 
	 * @param quantity
	 *          quantity of space (or other improvements) currently built on the
	 *          land
	 */
	public abstract void putQuantity(double quantity);

	/**
	 * Set the construction year for the current grid/parcel -- used to calculate
	 * effective age
	 * 
	 * @param yearBuilt
	 */
	...
	/**
	 * Get the construction year for the current grid/parcel -- used to calculate
	 * effective age.
	 * 
	 * @param currentZone
	 * @param id2
	 ...
	 * If land units and space units are consistent, then Floor Area Ratio can be
	 * calculated as getQuantity(currentZone,id2)/getSize(currentZone,id2). Note
	 * that there is no setSize() method -- PECAS does not need to be able to
	 * change the physical layout of the parcels or grid cells, except through the
	 * splitParcel() method.
	 * 
	 ...
	 * associated with a parcel. These need to be stored separately, in a separate
	 * table in a database or in memory. The grid file only refers to the
	 * numerical uniqueID of the zoning regulations.
	 * 
	 * @return index used in the ZoningSchemesI table in the PECAS input database
	 ...
	 */
	public abstract double getPrice(int coverageCode, int currentYear,
			int baseYear);
	 ...
	 * current parcel/grid. Many grids are water or protected areas that can never
	 * be developed. For parcel data, it is often advisable to remove protected
	 * parcels before running PECAS, but for grid data it is often advisable to
	 * have a complete grid coverage, but to mask out certain parcels by setting
	...
	/**
	 * The maximum size of a parcel. If a parcel is larger than this size, it will
	 * be treated in the monte-carlo simulation as multiple parcels of size
	 * getSize()/((int)getSize()/getMaxParcelSize()), and if development occurs on
	 * some parts but not others, SD will split the parcel into pseudoparcels by
	...
	/**
	 * Split the current parcel into an old one and a new one. The new one will be
	 * of size newLandSize, the old one will be modified to be of size
	 * getSize()-newLandSize.
	 * 
	 * @param parcelSizes
	 ...
	 */
	public abstract ParcelInterface splitParcel(double parcelSizes)
	...
	/**
	 * This method is in case the land inventory database queues up changes to be
	 * applied all at once; calling this method tells it that you need the changes
	 * to be applied
	 */
	...
	public abstract void applyPriceSmoothing(TableDataFileReader reader,
