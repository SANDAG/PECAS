...
public class DevelopNewAlternative extends DevelopmentAlternative {
	...
	private boolean setUpParameters() {
		...
		final TransitionCosts transitionCost = tempSession.mustFind(
				TransitionCosts.meta, costScheduleID,
	...
	public double getUtilityNoSizeEffect() throws ChoiceModelOverflowException {
		...
		if (Double.isNaN(result)) {
			...
			logger.error("Trhjp (utility per unit land)= " + utilityPerLand
					+ "; Thjp=" + utilityPerSpace + "; " + minimumFAR + "<FAR<"
					+ maximumFAR + " landsize=" + landArea);
			throw new ChoiceModelOverflowException(
					"NAN utility for DevelopmentAlternative Trhjp (utility per unit land)= "
							+ utilityPerLand + "; Thjp=" + utilityPerSpace + "; "
							+ minimumFAR + "<FAR<" + maximumFAR + " landsize=" + landArea);
		}
	...
	double getUtilityPerUnitSpace(TransitionCosts transitionCost,
		...
		double rent = ZoningRulesI.land.getPrice(
				theNewSpaceTypeToBeBuilt.getSpaceTypeID(), ZoningRulesI.currentYear,
	...
	double getUtilityPerUnitLand(TransitionCostCodes costCodes,
		...
		if (oldSpaceType != LandInventory.VACANT_ID) {
			...
			cost += oldSpaceTypeCosts.get_DemolitionCost()
					* ZoningRulesI.land.getQuantity() / ZoningRulesI.land.getLandArea();
		}
	...
	public void doDevelopment() {
		...
		if (size > ZoningRulesI.land.getMaxParcelSize()) {
			 ...
			 * (newDevQuantity*dt.getServicingRequirement()); if (servicingNeeded
			 * >servicing) ZoningRulesI.land.putServiceLevel(servicingNeeded);
			 */
			final int servicingNeeded = zoningReg.get_ServicesRequirement();
			...
			ZoningRulesI.land.getDevelopmentLogger().logDevelopment(
					ZoningRulesI.land, oldDT, oldDevQuantity, oldYear, oldIsDerelict,
					oldIsBrownfield);
		}
	}
	...
	public Matrix getExpectedTargetDerivativesWRTParameters(
		...
		final double expectedAddedSpace = 0; // Never any added space in a
		// develop new alternative.
		final double expectedNewSpace = expectedFAR
				* ZoningRulesI.land.getLandArea();
