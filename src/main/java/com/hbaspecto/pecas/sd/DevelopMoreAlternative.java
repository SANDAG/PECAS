...
class DevelopMoreAlternative extends DevelopmentAlternative {
	...
	private boolean setUpParameters() {
		...
		double minFAR = Math.max(myDt.get_MinIntensity(),
		...
		final TransitionCosts transitionCost = tempSession.mustFind(
				TransitionCosts.meta, costScheduleID, myDt.get_SpaceTypeId());
	...
	public double getUtilityNoSizeEffect() throws ChoiceModelOverflowException {
		...
		double result = getCompositeUtility(dispersion, landArea, utilityPerSpace,
	...
	public void doDevelopment() {
		...
		if (size > ZoningRulesI.land.getMaxParcelSize()) {
			...
			if (zoningReg != null) {
				if (Math.min(myDt.get_MaxIntensity(),
						zoningReg.get_MaxIntensityPermitted())
						* ZoningRulesI.land.getLandArea() > oldDevQuantity) {
			...
			if (zoningReg != null) {
				if (Math.min(zoningReg.get_MaxIntensityPermitted(),
						myDt.get_MaxIntensity())
						* ZoningRulesI.land.getLandArea() > oldDevQuantity) {
					newDevQuantity = (float) sampleIntensity()
			...
			// keeps track of the total amount of development added for a
			// spacetype.
			myDt.cumulativeAmountOfDevelopment += ZoningRulesI.land.getQuantity()
					- oldDevQuantity;
			ZoningRulesI.land.getDevelopmentLogger().logAddition(ZoningRulesI.land,
					oldDevQuantity, oldYear);
		}
	}
	...
	private double getUtilityPerUnitNewSpace(TransitionCosts transitionCost,
			DevelopmentFees df) {
		final double rent = ZoningRulesI.land.getPrice(myDt.getSpaceTypeID(),
				ZoningRulesI.currentYear, ZoningRulesI.baseYear);
	...
	private double getUtilityPerUnitNewSpace(TransitionCosts transitionCost,
			DevelopmentFees df, double rent) {
	...
	private double getUtilityPerUnitLand(TransitionCostCodes costCodes,
		 ...
		 * ZoningRulesI.land.get_FeeScheduleId(), spaceType.get_SpaceTypeId()); cost
	...
	private double sampleIntensity() {
		...
		return sampleIntensityWithinRanges(dispersion, landArea, utilityPerSpace,
				utilityPerLand, intensityPoints, perSpaceAdjustments,
				perLandAdjustments);
	}
	...
	public Vector getExpectedTargetValues(List<ExpectedValue> ts)
		...
		if (!canBuild) {
			...
			final double expectedFAR = getExpectedFAR(dispersion, landArea,
					utilityPerSpace, utilityPerLand, intensityPoints,
	...
	public Vector getUtilityDerivativesWRTParameters(List<Coefficient> cs)
		...
		final Vector results = getUtilityDerivativesWRTParameters(dispersion,
				landArea, utilityPerSpace, utilityPerLand, intensityPoints,
				perSpaceAdjustments, perLandAdjustments);
	...
	public Matrix getExpectedTargetDerivativesWRTParameters(
		...
		final double expectedFAR = getExpectedFAR(dispersion, landArea,
				...
				perLandAdjustments);
		final double expectedAddedSpace = expectedFAR
				* ZoningRulesI.land.getLandArea();
		final double expectedNewSpace = 0; // Never any new space in a develop
		// more alternative.
		...
		int i = 0;
		for (final ExpectedValue value : ts) {
			dTdE.set(i, 0, value.getModelledTotalNewDerivativeWRTAddedSpace(
		...
		final Vector results = getExpectedFARDerivativesWRTParameters(dispersion,
				landArea, utilityPerSpace, utilityPerLand, intensityPoints,
				perSpaceAdjustments, perLandAdjustments);
