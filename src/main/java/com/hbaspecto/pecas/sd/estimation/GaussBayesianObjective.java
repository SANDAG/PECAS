...
public class GaussBayesianObjective implements ObjectiveFunction {
	...
	public GaussBayesianObjective(DifferentiableModel model,
			List<Coefficient> coeffs, List<EstimationTarget> targets,
			Matrix targetVariance, Vector priorMean, Matrix priorVariance) {
	...
	public double getValue() throws OptimizationException {
		final Vector modelledValues = myModel.getTargetValues(myTargets,
	...
	public Vector getGradient(Vector params) throws OptimizationException {
		...
		targetErrors.add(-1, getMyTargetValues());
		targetErrors = myInverseTargetVariance.mult(targetErrors,
				targetErrors.copy());
		...
		currentGradient = myInversePriorVariance.multAdd(paramErrors,
				currentGradient);
	...
	public Matrix getHessian(Vector params) throws OptimizationException {
		final Matrix jacobian = myModel.getJacobian(myTargets, params);
		Matrix varianceTimesJacobian = new DenseMatrix(jacobian.numRows(),
				jacobian.numColumns());
		varianceTimesJacobian = myInverseTargetVariance.mult(jacobian,
				varianceTimesJacobian);
	...
	public void logParameters(Logger logger) {
		...
		for (final Coefficient coeff : myCoeffs) {
			String line = "Parameter " + coeff.getName() + ": prior mean = "
					+ myMean.get(i) + ", current value = "
					+ currentParameterValues.get(i);
	...
	public void logTargetAndObjective(Logger logger) {
		...
		logger.info("Current value of objective function = "
				+ currentObjectiveValue);
		if (previousValuesExist) {
			logger.info("Previous value of objective function = "
		...
		for (final EstimationTarget target : myTargets) {
			String line = "Target " + target.getName() + ": target value = "
					+ target.getTargetValue() + ", modelled value = "
					+ currentModelledValues[i];
			if (previousValuesExist) {
