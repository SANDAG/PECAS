...
public class EstimationMatrix {
	...
	public EstimationMatrix(List<ExpectedValue> eVals,
		...
		expValues = new BackMapList<ExpectedValue>(eVals);
	...
	public void addExpectedValueComponentApplicableToCurrentParcel(
		...
		int i = 0;
		int j = 0;
		for (final ExpectedValue t : expValues) {
	...
	/**
	 * Adds the specified values, element by element, to the cumulative totals for
	 * the derivatives.
	 * 
	 * @param component
	 * @throws IndexOutOfBoundsException
	 *           if the given matrix is the wrong size.
	 */
	...
	/**
	 * Returns an immutable list holding the targets. The list has a back-map from
	 * target objects to indices, so calls to <code>indexOf</code> and
	...
	public List<ExpectedValue> getTargetsApplicableToCurrentParcel() {
		final List<ExpectedValue> sublist = new ArrayList<ExpectedValue>();
		for (final ExpectedValue t : expValues) {
			if (t.appliesToCurrentParcel()) {
				sublist.add(t);
			}
	...
	/**
	 * Returns an immutable list holding the coefficients. The list has a back-map
	 * from coefficient objects to indices, so calls to <code>indexOf</code> and
	 * <code>contains</code> have constant time performance.
	 * 
	 * @return An immutable list of targets.
	 */
	...
	private class BackMapList<E> implements List<E> {
		...
		private final HashMap<E, int[]> backmap;
		...
		private BackMapList(List<E> list) {
			...
			while (it.hasNext()) {
				...
				if (backmap.containsKey(next)) {
					final int[] indices = backmap.get(next);
		...
		public boolean equals(Object o) {
			...
			final BackMapList other = (BackMapList) o;
