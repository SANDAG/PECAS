...
public class EstimationDataSet {
	...
	private final NumberFormat nf = new GeneralDecimalFormat("#.####E0", 1E7,
			1E-2);
	...
	public void compileEstimationRow(LandInventory l) {
		...
		for (final Integer newSpaceTypeID : sortedSpaceTypesIds) {
			...
			if (!newST.isVacant()) {
				...
				for (int i = 0; i < NUM_SAMPLES; i++) {
					addDevelopNewAlternativeInfo(l, newSpaceTypeID, i, NUM_SAMPLES,
							storeQntySamples);
				}
			}
		}
	...
	private void addDoNothingAlternativeInfo(LandInventory l) {
		...
		final SpaceTypesI dt = SpaceTypesI
				.getAlreadyCreatedSpaceTypeBySpaceTypeID(l.getCoverage());
		if (!dt.isVacant() && !l.isDerelict()) {
			...
			rent = l.getPrice(dt.getSpaceTypeID(), ZoningRulesI.currentYear,
	...
	private void addDemolishAlternativeInfo(LandInventory l) {
		...
		if (!dt.isVacant()) {
			demolitionCost = dt.getDemolitionCost(l.get_CostScheduleId())
	...
	private void addDerelictAlternativeInfo(LandInventory l) {
		...
		final SpaceTypesI dt = SpaceTypesI
				.getAlreadyCreatedSpaceTypeBySpaceTypeID(l.getCoverage());
		int isPermissible = 1;
		if (!zoning.get_DerelictionPossibilities() || l.isDerelict()
				|| dt.isVacant()) {
			isPermissible = -1;
		}
		estimationRow.add(new Integer(isPermissible)); // Alternative
	...
	private void addRenovateAlternativeInfo(LandInventory l) {
		 ...
		 * tempSession.mustFind(TransitionCostCodes.meta, l.get_CostScheduleId());
	...
	private void addAddMoreAlternativeInfo(LandInventory l, int sampleNumber,
			int numberOfSamples) {

		estimationRow.add(new Integer(l.getCoverage()));

		final ZoningRulesI zoningRules = ZoningRulesI
				.getZoningRuleByZoningRulesCode(l.getSession(), l.getZoningRulesCode());
		final SpaceTypesI dt = SpaceTypesI
				.getAlreadyCreatedSpaceTypeBySpaceTypeID(l.getCoverage());
		int isPermissible = 1;
		...
		if (!zoningRules.get_AdditionPossibilities() || dt.isVacant()
				|| l.isDerelict() || isParcelFull) {
			isPermissible = -1;
		}
		estimationRow.add(new Integer(isPermissible)); // Alternative
		// permissible (Zoning)
		...
		final int age = ZoningRulesI.currentYear - l.getYearBuilt();
		final double rentOfExistingSpace = spaceValue
				* dt.getRentDiscountFactor(age) * l.getQuantity() / l.getLandArea();
		final int ageOfNew = 0;

		// Note: spaceValue = 0 if the parcel is derelict and/or isVacant
		final double rentOfNew = spaceValue * dt.getRentDiscountFactor(ageOfNew)
				* addedQuantity / l.getLandArea();
		final double rent = rentOfNew + rentOfExistingSpace;

		final double maintenanceCost = dt.getAdjustedMaintenanceCost(age)
				* l.getQuantity() / l.getLandArea()
				+ dt.getAdjustedMaintenanceCost(ageOfNew) * addedQuantity
				/ l.getLandArea();
		...
		if (!dt.isVacant()) {
			final TransitionCostCodes costCodes = tempSession.mustFind(
					TransitionCostCodes.meta, costScheduleID);
			final TransitionCosts transitionCost = tempSession.mustFind(
					TransitionCosts.meta, costScheduleID, dt.get_SpaceTypeId());

			double costPerUnitOfSpace = 0;
			costPerUnitOfSpace += transitionCost.get_AdditionCost();
			if (zoningPermissions != null) {
				if (zoningPermissions.get_AcknowledgedUse()) {
					costPerUnitOfSpace += zoningPermissions
							.get_PenaltyAcknowledgedSpace();
				}
			}
	...
	private void addDevelopNewAlternativeInfo(LandInventory l,
			int newSpaceTypeId, int sampleNumber, int numberOfSamples,
			boolean storeQntySamples) {
		...
		if (zoningPermissions != null) {
			maximumFAR = Math.min(theNewSpaceType.get_MaxIntensity(),
					zoningPermissions.get_MaxIntensityPermitted());
			minimumFAR = Math.max(theNewSpaceType.get_MinIntensity(),
					zoningPermissions.get_MinIntensityPermitted());
		}
		else {
			isPermissible = -1;
		}
		...
		final int age = 0;
		rent *= theNewSpaceType.getRentDiscountFactor(age) * quantity
				/ l.getLandArea();
		final double maintenanceCost = theNewSpaceType
				.getAdjustedMaintenanceCost(age) * quantity / l.getLandArea();
		...
		final TransitionCostCodes costCodes = tempSession.mustFind(
				TransitionCostCodes.meta, costScheduleID);
		final TransitionCosts transitionCost = tempSession
				.mustFind(TransitionCosts.meta, costScheduleID,
						theNewSpaceType.get_SpaceTypeId());

		double cost = transitionCost.get_ConstructionCost() * quantity
				/ l.getLandArea();
		if (zoningPermissions != null) {
			if (zoningPermissions.get_AcknowledgedUse()) {
		...
		if (!oldSpaceType.isVacant()) {
			final TransitionCosts oldSpaceTypeCosts = tempSession.mustFind(
					TransitionCosts.meta, costScheduleID, oldSpaceType.get_SpaceTypeId());
