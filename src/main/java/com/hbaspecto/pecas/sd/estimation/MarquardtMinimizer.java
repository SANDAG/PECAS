...
public class MarquardtMinimizer
{
    ...
    private Vector doOneIteration(BeforeIterationCallback before) throws OptimizationException
    {
            ...
            try
            {
                ...
                if (newobj < currentObjective)
                {
                    ...
                    final Matrix newHessian = obj.getHessian(parameters).add(
                            getPenaltyFunctionHessian(parameters));
                    final Double hessianNorm = new Double(newHessian.norm(Matrix.Norm.One));
    ...
    {
        void finishedIteration(int iteration);
    }
     ...
     *            not converged.
     * @param before
     *            A callback to execute each iteration once the gradient/hessian
     *            is computed but before the new parameters are chosen.
     * @param after
            ...
            Vector epsilon)
    {
        ...
        {
            final double diff = Math.abs(currentParameters.get(i) - previousParameters.get(i));
    ...
    public Vector minimize(Vector epsilon, int maxIterations, Vector penaltyEpsilon,
            int penaltyMaxIterations) throws OptimizationException
    {
        ...
        while (!converged && penaltyIteration < maxIterations)
        {
            ...
            final boolean thisIterationConverged = checkConvergence(parameters, previousParameters,
                    penaltyEpsilon);
            previousParameters = parameters;
            converged = thisIterationConverged && lastTwoIterationsConverged;
            lastTwoIterationsConverged = thisIterationConverged;
