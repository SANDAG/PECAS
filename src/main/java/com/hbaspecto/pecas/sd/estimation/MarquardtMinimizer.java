...
public class MarquardtMinimizer {
     ...
     * @param initialGuess Values of the objective function's parameters to
     *        use as an initial guess.
     * @throws OptimizationException if the initial guess causes an error in
     *         the objective function.
     */
    public MarquardtMinimizer(ObjectiveFunction objective, Vector initialGuess) throws OptimizationException {
    ...
    /**
     * Readies the optimizer for a new constrained optimization, using the given initial
     * guess. This method is similar to <code>reset</code> except that it also resets the
     * constraint looseness and number of penalty function iterations.
     * @param initialGuess The new initial guess.
     * @throws OptimizationException if the initial guess causes an error in the objective
     *         function.
     */
     ...
     * @param minStepSize The minimum step size allowed for an iteration.
     * @param maxStepSize The maximum step size allowed for an iteration.
     * @param initialMarquardtFactor The factor at which the diagonal matrix B in the Marquardt
     *        method is weighted compared to the Hessian.
     */
    public void setOptimizationParameters(double minStepSize, double maxStepSize, double initialMarquardtFactor) {
        minstep = minStepSize;
        maxstep = maxStepSize;
        ...
        while(!foundValidStep) {
            ...
            try {
                ...
                if(newobj < currentObjective) {
                    ...
                    Matrix newHessian = obj.getHessian(parameters).add(getPenaltyFunctionHessian(parameters));
                    ...
                    if (hessianNorm.isInfinite() || hessianNorm.isNaN()) {
                    	...
                    	lambda = 2* lambda;
                    	logger.info("***Step looked good but hessian had infinitiy or NaN in it so we have to back up, lambda set to "+lambda);
...
//    }
    ...
    /**
     * Performs iterations of the Marquardt method until convergence occurs. Convergence
     * is deemed to have occurred if the coefficients in three successive iterations differ by less than
     * <code>epsilon</code>. This method will find the minimum for a single weighting of
     * any penalty functions; use <code>minimize()</code> to do the full solution, including
     * reducing the weighting of the penalty function until the solution is found.
     * @param epsilon The threshold at which the optimization is deemed to have converged. There is a
     *        separate epsilon for each coefficient, so that coefficients with different orders of
     *        magnitude can be handled properly; the epsilons must be given in the same order as the
     *        coefficients.
     * @param maxIterations The maximum number of iterations - after that number of iterations,
     *        the method will return its results even if it has not converged.
     * @return The values of the parameters at the optimum.
     */
    public Vector iterateToConvergence(Vector epsilon, int maxIterations, EachIterationCallback callBacker) {
        iteration = 0;
        ...
        try {
            while(!converged && iteration < maxIterations) {
                ...
                boolean thisIterationConverged = checkConvergence(parameters, previousParameters, epsilon);
    ...
    private boolean checkConvergence(Vector currentParameters, Vector previousParameters, Vector epsilon) {
     ...
     * Finds the minimum of the objective function, subject to the constraints provided using
     * the <code>addConstraint</code> method. This method calls <code>iterateToConvergence</code>
     * repeatedly while making the constraints "looser" until that method's output changes by less
     * than <code>penaltyEpsilon</code> for three consecutive iterations. The methods <code>lastRunConverged</code>,
     * <code>lastRunMaxIterations</code>, and <code>lastRunInvalidStep</code> refer to the termination
     * cause for the last call to <code>iterateToConvergence</code> (with the loosest constraint),
     * since it is that iteration's termination that is relevant for this method (e.g. if the last
     * iteration returns normally, this indicates a valid solution, even if an earlier iteration
     * terminated because of an exception). Note that if no constraints have been added, this method
     * does exactly the same thing as <code>iterateToConvergence</code>.
     * @param epsilon The <code>epsilon</code> parameter to pass to <code>iterateToConvergence</code>.
     * @param maxIterations The <code>maxIterations</code> parameter to pass to <code>iterateToConvergence</code>.
     * @param penaltyEpsilon The threshold at which the minimum is deemed to have been found.
     * @param penaltyMaxIterations The maximum number of calls allowed to <code>iterateToConvergence</code>.
     * @return The values of the parameters at the optimum.
     * @throws OptimizationException if the current parameters are invalid.
     */
    public Vector minimize(Vector epsilon, int maxIterations, Vector penaltyEpsilon, int penaltyMaxIterations, EachIterationCallback callBack) throws OptimizationException {
        if(constraints.size() == 0)
            return iterateToConvergence(epsilon, maxIterations, callBack);
        ...
        while(!converged && penaltyIteration < maxIterations) {
            ...
            boolean thisIterationConverged = checkConvergence(parameters, previousParameters, penaltyEpsilon);
            previousParameters = parameters;
            converged = thisIterationConverged && lastTwoIterationsConverged;
            lastTwoIterationsConverged = thisIterationConverged;
        }
    ...
    /**
     * Returns the current value of the objective function after the most recent iteration
     * or reset.
    ...
    /**
     * Returns the number of penalty function iterations performed since the last call to
    ...
    /**
     * Determines whether the last execution of <code>iterateToConvergence</code> converged
     * on a solution.
     * @return True if the optimizer converged on a solution, and false otherwise
     *         (including if <code>iterateToConvergence</code> has never been invoked.
     */
    ...
    /**
     * Determines whether the last execution of <code>iterateToConvergence</code> hit the
    ...
    /**
     * Determines whether the last execution of <code>minimize</code> converged on a
     * solution
    ...
    /**
     * Determines whether the last execution of <code>minimize</code> hit the maximum
     * number of iterations before finding a solution.
     * @return True if the optimizer ran out of iterations, and false otherwise
     *         (including if <code>minimize</code> has never been invoked.
     */
    ...
    /**
     * Determines whether the last execution of <code>iterateToConvergence</code> halted
     * because a valid step (i.e. one that decreased the objective function) could not be found.
     * In particular, returns true if the objective function threw an exception for every step tried.
     * @return True if the optimizer converged on a solution, and false otherwise
     *         (including if <code>iterateToConvergence</code> has never been invoked.
     */
