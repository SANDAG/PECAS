...
public class ExpectedTargetModel implements DifferentiableModel {
    ...
    public ExpectedTargetModel(List<Coefficient> coeffs, LandInventory landInventory) {
    ...
    private void calculateAllValues(List<EstimationTarget> targets, Vector params) {
        ...
        List<ExpectedValue> expValueObjects = convertToExpectedValueObjects(targets);
        EstimationMatrix matrix = new EstimationMatrix(expValueObjects, myCoeffs);
        land.setToBeforeFirst();
        while(land.advanceToNext()) {
            ZoningRulesI currentZoningRule = ZoningRulesI.getZoningRuleByZoningRulesCode(land.getSession(), land.getZoningRulesCode());
        ...
        for(int j = 0; j < myCoeffs.size(); j++) {
            double derivative = myCoeffs.get(j).getInverseTransformationDerivative();
            for(int i = 0; i < targets.size(); i++) {
    ...
    private List<ExpectedValue> convertToExpectedValueObjects(List<EstimationTarget> targets) {
    ...
    public void printCurrentDerivatives(BufferedWriter writer) throws IOException {
    	...
    	{
    		writer.write(lastTargetValues.get(i).getName() + "," + lastTargetValues.get(i).getTargetValue());
    		for(int j = 0; j < myCoeffs.size(); j++)
