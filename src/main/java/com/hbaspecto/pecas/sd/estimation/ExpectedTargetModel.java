        ...
        implements DifferentiableModel
{
    ...
    private void calculateAllValues(List<EstimationTarget> targets, Vector params)
    {
        ...
        final EstimationMatrix matrix = new EstimationMatrix(expValueObjects, myCoeffs);
        land.setToBeforeFirst();
        while (land.advanceToNext())
        {
            final ZoningRulesI currentZoningRule = ZoningRulesI.getZoningRuleByZoningRulesCode(
        ...
        for (int j = 0; j < myCoeffs.size(); j++)
        {
            final double derivative = myCoeffs.get(j).getInverseTransformationDerivative();
            for (int i = 0; i < targets.size(); i++)
            {
