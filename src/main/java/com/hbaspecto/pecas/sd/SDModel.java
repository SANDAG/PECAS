 ...
 *         inventory is used. it is an absrtact class that has some common code
 *         that can be used by lot of different implementation.
 */
public abstract class SDModel {
	...
	public void runSD(int currentYear, int baseYear, ResourceBundle rb) {
		...
		// TableDataSet crossTabbed=
		// TableDataSetCrosstabber.crossTabDataset(outputDatabase,
		// "FloorspaceI", "FloorspaceZone", "Commodity", "Quantity");
	...
	protected static void initZoningScheme(int currentYear, int baseYear) {
		...
		if (ResourceUtil.getProperty(rbSD, "DevelopmentDispersionParameter") != null) {
			logger
					.warn("DevelopmentDispersionParameter is set in sd.properties, this is no longer the place to put SD dispersion parameters");
		}
		...
		final double amortizationFactor = interestRate * compounded
				/ (compounded - 1);
		ZoningRulesI.amortizationFactor = ResourceUtil.getDoubleProperty(rbSD,
				"AmortizationFactor", amortizationFactor);
		ZoningRulesI.servicingCostPerUnit = ResourceUtil.getDoubleProperty(rbSD,
	...
	/**
	 * Runs through the inventory simulating development NOTE only works with
	 * TableDataSetLandInventory, if your land inventory is of some other type
	 * you'll have to write your own code to iterate through all the land.
	 */
	public abstract void simulateDevelopment();

	/*
	 * { TableDataSetLandInventory tdsLand = (TableDataSetLandInventory) land;
	 * ZoningScheme.openLogFile(logFilePath); ZoningScheme currentScheme = null;
	 * int zoneNumberColumn = zoneNumbers.checkColumnPosition("AlphaZone");
	 * tdsLand.setToBeforeFirst(); while (tdsLand.advanceToNext()) { // long
	 * zoneNumber = (long) zoneNumbers.getValueAt(row,zoneNumberColumn); // long[]
	 * parcelNumbers = tdsLand.getId2Numbers(zoneNumber); // for (long
	 * parcelSequence=0;parcelSequence<parcelNumbers.length;parcelSequence++) { //
	 * long parcel = parcelNumbers[(int) parcelSequence]; currentScheme =
	 * ZoningScheme.getZoningSchemeByIndex(land.getZoningRulesCode()); if
	 * (currentScheme == null) { ZoningScheme.logBadZoning(land); } else {
	 * currentScheme.doDevelopment(land); } } // might need this next line just to
	 * be safe. tdsLand.writeAndReclaimMemory(); ZoningScheme.closeLogFile();
	 * 
	 * }
	 */
	public void writeOutInventoryTable(TableDataSet landTypes) {
		final TableDataSet landInventoryTable = land.summarizeInventory();
		landInventoryTable.setName("FloorspaceI");
		outputDatabase.addTableDataSet(landInventoryTable);
		// TableDataSet crossTabbed =
		// TableDataSetCrosstabber.crossTabDataset(outputDatabase,
		// landInventoryTable.getName(), "TAZ", "Commodity", "Quantity");
	...
	public SpaceTypesI[] setUpDevelopmentTypes() {
		...
		final SQuery<SpaceTypesI> devQry = new SQuery<SpaceTypesI>(
				SpaceTypesI_gen.meta);
		final List<SpaceTypesI> dtypes = session.query(devQry);

		final SpaceTypesI[] d = new SpaceTypesI[dtypes.size()];
		final SpaceTypesI[] dTypes = dtypes.toArray(d);
	...
	// Integer floorspaceZone = new Integer( (int)
	// alphaZoneTable.getValueAt(zRow, "AlphaZone"));
	...
	// if (pecasZone != null) {
	// // don't add in bogus records -- there might be land use zones that
	// aren't covered by the spatial IO model
	...
	// e.printStackTrace();
	// throw new
	// RuntimeException("Put base year and time interval on command line" +
	// "\n For example, 1990 1");
	// }
	// myLD.setUp();
