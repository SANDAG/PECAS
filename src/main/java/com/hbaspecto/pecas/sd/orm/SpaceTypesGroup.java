...
public class SpaceTypesGroup
        extends SpaceTypesGroup_gen
        implements java.io.Serializable
{
    ...
    {
        /*
         * The following query gets all the commodities in a group together with their total production (TPc). SELECT commodity, sum(internal_bought)
         * as sum from exchange_results WHERE LUZ in (SELECT luz_number from luzs) AND commodity in (SELECT cc_commodity_name FROM
         * construction_commodities WHERE space_types_group_id = spaceGroupID) GROUP BY commodity
         */
        ...
        } else
        {
            ...
            SSessionJdbc session = SSessionJdbc.getThreadLocalSession();
            Iterator<ConstructionCommodities> cc = ConstructionCommodities
            ...
            while (cc.hasNext())
            {
                ...
                double tp = ((Double) session.rawQuerySingle(
                        " SELECT sum(internal_bought) as sum  from exchange_results "
                                + " WHERE LUZ in (SELECT luz_number from luzs) AND commodity = ? ",
                        false, new String(cc_instance.get_CcCommodityName()))).doubleValue();
                target += tp * cc_instance.get_ConvertingFactor();
            }

            // Now find space types in the group;
            SQuery<SpaceTypesI> query = new SQuery<SpaceTypesI>(SpaceTypesI.meta).eq(
                    SpaceTypesI.SpaceTypeGroupId, spaceGroupID);

            SQueryResult<SpaceTypesI> sts = session.query(query);
            for (SpaceTypesI st : sts)
            {
                SiteSpecTotals ss = session.find(SiteSpecTotals.meta, st.get_SpaceTypeId(),
                        getCurrentYear());
                if (ss == null)
                {
