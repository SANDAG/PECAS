...
public abstract class SiteSpecTotals_gen extends SRecordInstance implements
	...
	public SiteSpecTotals_gen() {
		if (meta == null) {
			final String msg = SiteSpecTotals_gen.class.getName()
	...
	public static void init(ResourceBundle rb) {
		...
		meta = new SRecordMeta<SiteSpecTotals>(SiteSpecTotals.class,
				ResourceUtil
						.getProperty(rb, "sdorm.sitespec_totals", "sitespec_totals"));

		// Columns in table

		SpaceTypeId = new SFieldInteger(meta, ResourceUtil.getProperty(rb,
				"sdorm.sitespec_totals.space_type_id", "space_type_id"),
				new SFieldFlags[] { SFieldFlags.SPRIMARY_KEY, SFieldFlags.SMANDATORY });

		YearEffective = new SFieldInteger(meta, ResourceUtil.getProperty(rb,
				"sdorm.sitespec_totals.year_effective", "year_effective"),
				new SFieldFlags[] { SFieldFlags.SPRIMARY_KEY, SFieldFlags.SMANDATORY });

		SpaceQuantity = new SFieldDouble(meta, ResourceUtil.getProperty(rb,
				"sdorm.sitesec_totals.space_quantity", "space_quantity"));
	}
