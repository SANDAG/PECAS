 ...
 ***/
public abstract class Parcels_gen extends SRecordInstance implements
	...
	public Parcels_gen() {
		if (meta == null) {
			final String msg = Parcels_gen.class.getName()
	...
	public static void init(ResourceBundle rb) {
		...
		// Columns in table
		ParcelId = new SFieldString(meta, ResourceUtil.getProperty(rb,
				"sdorm.parcels.parcel_id", "parcel_id"), 2147483647);

		PecasParcelNum = new SFieldLong(meta, ResourceUtil.getProperty(rb,
				"sdorm.parcels.pecas_parcel_num", "pecas_parcel_num"),
		...
		Taz = new SFieldInteger(meta, ResourceUtil.getProperty(rb,
				"sdorm.parcels.taz", "taz"));
		...
		LandArea = new SFieldDouble(meta, ResourceUtil.getProperty(rb,
				"sdorm.parcels.land_area", "land_area"));

		AvailableServicesCode = new SFieldInteger(meta, ResourceUtil.getProperty(
				rb, "sdorm.parcels.available_services_code", "available_services_code"));
	...
	public Tazs get_TAZS(SSessionJdbc ses) {
		try {
			 ...
			 **/
			return ses.findOrCreate(Tazs_gen.meta, new Object[] { get_Taz(), });
		}
		catch (final SException e) {
			if (e.getMessage().indexOf("Null Primary key") > 0) {
				return null;
