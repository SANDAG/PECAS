        ...
        implements java.io.Serializable
{
    ...
    public static void init(ResourceBundle rb)
    {
        ...
        meta = new SRecordMeta<ExchangeResults>(ExchangeResults.class, ResourceUtil.getProperty(rb,
                "sdorm.sdprices", "exchange_results"));

        // Columns in table
        Commodity = new SFieldString(meta, ResourceUtil.getProperty(rb, "sdorm.sdprices.commodity",
                "commodity"), 2147483647, new SFieldFlags[] {SFieldFlags.SPRIMARY_KEY,
                SFieldFlags.SMANDATORY});

        Luz = new SFieldInteger(meta, ResourceUtil.getProperty(rb, "sdorm.sdprices.luz", "luz"),
                new SFieldFlags[] {SFieldFlags.SPRIMARY_KEY, SFieldFlags.SMANDATORY});

        Price = new SFieldDouble(meta,
                ResourceUtil.getProperty(rb, "sdorm.sdprices.price", "price"));
