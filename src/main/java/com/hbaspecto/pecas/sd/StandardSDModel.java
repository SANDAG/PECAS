...
public class StandardSDModel extends SDModel {
    ...
    public void setUp() {
		...
		"InputDatabase");
		JDBCConnection inputPBConnection = new JDBCConnection(
				inputDatabaseSpecifier, inputDatabaseDriver, ResourceUtil
				.getProperty(rbSD, "InputDatabaseUser", ""),
				ResourceUtil.getProperty(rbSD, "InputDatabasePassword", ""));
		JDBCTableReader jdbcInputTableReader = new JDBCTableReader(
				inputPBConnection);
		JDBCTableWriter jdbcInputTableWriter = new JDBCTableWriter(
				inputPBConnection);
		excelInputDatabase = ResourceUtil.getBooleanProperty(rbSD,
				"ExcelInputDatabase",false);
		jdbcInputTableReader
		.setMangleTableNamesForExcel(excelInputDatabase);
		jdbcInputTableWriter
		.setMangleTableNamesForExcel(excelInputDatabase);
		...
		TableDataSetCollection inputDatabase = new TableDataSetCollection(
    ...
    public void simulateDevelopment() {
		...
		while (land.advanceToNext()) {
			...
			currentZoningRule = ZoningRulesI.getZoningRuleByZoningRulesCode(land.getSession(), land.getZoningRulesCode());
	...
	public void calibrateModel(EstimationReader reader, int baseY, int currentY, double epsilon, int maxits) {
		//ZoningRulesI.openLogFile(logFilePath);
		baseYear = baseY;
		...
		try {
			...
			ObjectiveFunction theObjective = new GaussBayesianObjective(theModel, coeffs, targets, targetVariance, means, variance);
		...
		try {
			writer = new BufferedWriter(new FileWriter("perturbed.csv"));
			// Write target names across the top.
			for(EstimationTarget target: targets)
				writer.write("," + target.getName());
			writer.newLine();
			// Write unperturbed target values.
			try {
				targetValues = theModel.getTargetValues(targets, means);
				writer.write("Unperturbed");
				for(int i = 0; i < targetValues.size(); i++)
					writer.write("," + targetValues.get(i));
				writer.newLine();
			} catch(OptimizationException e) {}

			// Perturb each coefficient in turn.
			for(int i = 0; i < means.size(); i++) {
					Vector perturbed = means.copy();
					perturbed.set(i, perturbed.get(i) + delta);
				try {
					targetValues = theModel.getTargetValues(targets, perturbed);
					writer.write(coeffs.get(i).getName());
					for(int j = 0; j < targetValues.size(); j++)
						writer.write("," + targetValues.get(j));
					writer.newLine();
				} catch(OptimizationException e) {}
			}
		} catch(IOException e) {}
			...
			try {
				...
				MarquardtMinimizer theMinimizer = new MarquardtMinimizer(theObjective, new DenseVector(reader.readStartingValues(coeffs)));
				...
				if(theMinimizer.lastRunConverged()) {
					logger.info("SD parameter estimation converged on a solution: " + paramsAsString);
					logger.info("Initial objective function = " + initialObj);
					logger.info("Optimal objective function = " + theMinimizer.getCurrentObjectiveValue());
					logger.info("Convergence after " + theMinimizer.getNumberOfIterations() + " iterations");
				}
				else {
					int numits = theMinimizer.getNumberOfIterations();
					logger.info("SD parameter estimation stopped after " + numits + " iteration" + (numits == 1? "" : "s") + " without finding a solution");
				...
				logger.info("Target values at optimum: " + Arrays.toString(Matrices.getArray(optimalTargets)));
			} catch(OptimizationException e) {
