...
public class RenovateAlternative extends DevelopmentAlternative {
	...
	private double getUtilityPerUnitSpace() {
		...
		final double rent = ZoningRulesI.land.getPrice(getExistingSpaceType()
				.getSpaceTypeID(), ZoningRulesI.currentYear, ZoningRulesI.baseYear)
		...
		if (ZoningRulesI.land.isDerelict()) {
			...
			cost = getExistingSpaceType().getRenovationCost(
					ZoningRulesI.land.get_CostScheduleId())
					* ZoningRulesI.amortizationFactor
					+ getExistingSpaceType().getAdjustedMaintenanceCost(age);
		}
	...
	public Vector getExpectedTargetValues(List<ExpectedValue> ts)
		...
		for (final ExpectedValue value : ts) {
			if (value instanceof RenovationTarget) {
				...
				result.set(i, redevT.getModelledRenovateQuantityForParcel(
