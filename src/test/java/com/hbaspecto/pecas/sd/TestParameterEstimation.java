...
public class TestParameterEstimation {
	...
	private static void setUpTestInventory(String url, String user,
		...
		// replace that data.
		statement.execute("UPDATE exchange_results "
				+ "SET price=42000, internal_bought=42000");
	...
	public static void setUpResources() throws Exception {
		...
		final JDBCConnection inputPBConnection = new JDBCConnection(url, driver,
				user, password);
		...
		final String landDatabaseDriver = ResourceUtil.checkAndGetProperty(rb,
				"LandJDBCDriver");
		try {
		...
		if (ResourceUtil.getBooleanProperty(rb, "UseYearSubdirectories", true)) {
			outputTableReader.setMyDirectory(ResourceUtil.getProperty(rb,
	...
	public void testTargetValuesWithFullMatrix() throws Exception {
		final CSVEstimationReader reader = new CSVEstimationReader(
				"testconfig\\targets.csv", false, "testconfig\\parameters.csv", false);
	...
	public void testTargetValuesDiagonal() throws Exception {
		final CSVEstimationReader reader = new CSVEstimationReader(
				"testconfig\\targetssimple.csv", true,
				"testconfig\\parameterssimple.csv", true);
		testTargetValues(reader);
	}
	...
	public void testTargetValues(EstimationReader reader) throws Exception {
		...
		final DifferentiableModel minimodel = new ExpectedTargetModel(minicoeffs,
				land);
	...
	public void checkTargets(List<EstimationTarget> targets) {
		 ...
		 * double[] {90, 60099}; double[] spaceTypeIntensityTargets = new double[]
		 * {0.0003, 0.102};
		 */
		final double[][] spaceTypeTazTargets = new double[][] { { 2, 0, 39, 22 },
				{ 0, 36057, 684, 11855 } };
		final double[] redevelopmentIntoSpaceTypeTargets = new double[] { 167,
				42797 };
		...
		for (int i = 0; i < spacetypes.length; i++) {
			...
			assertEquals(redevelopmentIntoSpaceTypeTargets[i],
	...
	public void checkCoeffs(List<Coefficient> coeffs) {
		...
		for (int i = 0; i < spacetypes.length; i++) {
			for (int j = 0; j < spacetypesnv.length; j++) {
				assertEquals("trans-" + spacetypes[i] + "-" + spacetypesnv[j], coeffs
						.get(c).getName());
				c++;
			}
		}
	...
	public void checkPriorMeans(double[] means) {
		...
		for (int i = 0; i < spacetypes.length; i++) {
			value = SpaceTypesI
					.getAlreadyCreatedSpaceTypeBySpaceTypeID(spacetypes[i])
					.get_AboveStepPointAdjustment();
			assertEquals(value, means[n]);
			n++;
		}
		for (int i = 0; i < spacetypes.length; i++) {
			value = Math.log(SpaceTypesI.getAlreadyCreatedSpaceTypeBySpaceTypeID(
					spacetypes[i]).get_GyDispersionParameter());
			assertEquals(value, means[n]);
			n++;
		}
		for (int i = 0; i < spacetypes.length; i++) {
		...
		for (int i = 0; i < spacetypes.length; i++) {
			value = Math.log(SpaceTypesI.getAlreadyCreatedSpaceTypeBySpaceTypeID(
					spacetypes[i]).get_GkDispersionParameter());
			assertEquals(value, means[n]);
			n++;
		}
		for (int i = 0; i < spacetypes.length; i++) {
			value = Math.log(SpaceTypesI.getAlreadyCreatedSpaceTypeBySpaceTypeID(
					spacetypes[i]).get_GwDispersionParameter());
			assertEquals(value, means[n]);
			n++;
		}
		for (int i = 0; i < spacetypes.length; i++) {
		...
		for (int i = 0; i < spacetypes.length; i++) {
			value = Math.log(SpaceTypesI.getAlreadyCreatedSpaceTypeBySpaceTypeID(
					spacetypes[i]).get_IntensityDispersionParameter());
			assertEquals(value, means[n]);
			n++;
		}
		for (int i = 0; i < spacetypes.length; i++) {
		...
		for (int i = 0; i < spacetypes.length; i++) {
			value = Math.log(SpaceTypesI.getAlreadyCreatedSpaceTypeBySpaceTypeID(
					spacetypes[i]).get_NewTypeDispersionParameter());
			assertEquals(value, means[n]);
			n++;
		}
		for (int i = 0; i < spacetypes.length; i++) {
		...
		for (int i = 0; i < spacetypes.length; i++) {
			value = Math.log(SpaceTypesI.getAlreadyCreatedSpaceTypeBySpaceTypeID(
					spacetypes[i]).get_NochangeDispersionParameter());
			assertEquals(value, means[n]);
			n++;
		}
		for (int i = 0; i < spacetypes.length; i++) {
			value = Math.log(SpaceTypesI.getAlreadyCreatedSpaceTypeBySpaceTypeID(
					spacetypes[i]).get_GzDispersionParameter());
			assertEquals(value, means[n]);
			n++;
		}
		for (int i = 0; i < spacetypes.length; i++) {
		...
		for (int i = 0; i < spacetypes.length; i++) {
			for (int j = 0; j < spacetypesnv.length; j++) {
				value = SpaceTypesI.getAlreadyCreatedSpaceTypeBySpaceTypeID(
						spacetypes[i]).getTransitionConstantTo(sess, spacetypesnv[j]);
				assertEquals(value, means[n]);
				n++;
			}
		}
	...
	public void checkPriorVariance(double[][] variance) {
		 ...
		 * result[18][18] = 2070.25; result[19][19] = 12.25; result[20][20] = 1E+99;
		 * result[21][21] = 100; result[22][22] = 100; result[23][23] = 1E+99;
		 * result[24][24] = 0.01; result[25][25] = 0.01; result[26][26] = 0.01;
		 * result[27][27] = 2410.81; result[28][28] = 118.81; result[29][29] =
		 * 2905.21; result[30][30] = 0.01; result[31][31] = 0.01; result[32][32] =
		 * 0.01; result[33][33] = 1E-12; result[34][34] = 1E-12; result[35][35] =
		 * 1E-12; result[36][36] = 0.01; result[37][37] = 0.01; result[38][38] =
		 * 0.01; result[39][39] = 0.01; result[40][40] = 0.01; result[41][41] =
		 * 0.01; result[42][42] = 686.44; result[43][43] = 3.5344; result[44][44] =
		 * 1E+99; result[45][45] = 686.44; result[46][46] = 3.5344; result[47][47] =
		 * 1E+99; result[48][48] = 0.2304; result[49][49] = 0.1296; result[50][50] =
		 * 1E-12; result[51][51] = 4; result[52][52] = 2.25; result[53][53] = 1E-12;
		 * result[54][54] = 74.4769; result[55][55] = 148.84; result[56][56] =
		 * 2.5281; result[57][57] = 91.7764; result[58][58] = 1413.76;
		 * result[59][59] = 2401;
		 */
